...
const FORBIDDEN_DETAIL_KEY =
	/(?:transcript|chain[_. -]*of[_. -]*thought|private[_. -]*reasoning|(?:^|[_. -])reasoning(?:$|[_. -])|(?:^|[_. -])thoughts?(?:$|[_. -])|^(?:stdout|stderr|output|prompt)(?:tail|text|line)?$)/i;
...
function containsForbiddenDetail(value: unknown): boolean {
	...
	if (value !== null && typeof value === "object") {
		return Object.entries(value as Record<string, unknown>).some(
			([key, nested]) =>
				FORBIDDEN_DETAIL_KEY.test(key) || containsForbiddenDetail(nested),
		);
	}
	return false;
}
...
function isJsonDetailValue(value: unknown, seen = new Set<object>()): boolean {
	...
	if (Array.isArray(value))
		return value.every((item) => isJsonDetailValue(item, seen));
...
export const TraceDetailSchema = z
	...
	.superRefine((detail, ctx) => {
		if (containsForbiddenDetail(detail)) {
			ctx.addIssue({
				code: "custom",
				message: "trace detail cannot contain transcripts or private reasoning",
			});
		}
		...
		if (length === undefined) {
			ctx.addIssue({
				code: "custom",
				message: "trace detail must be JSON serializable",
			});
		} else if (length > TRACE_MAX_DETAIL_CHARS) {
			ctx.addIssue({
				code: "too_big",
				maximum: TRACE_MAX_DETAIL_CHARS,
				inclusive: true,
				origin: "string",
				message: `trace detail exceeds ${TRACE_MAX_DETAIL_CHARS} encoded characters`,
			});
		}
	});
		...
		for (const [index, event] of artifact.events.entries()) {
			if (event.sequence !== expected) {
				ctx.addIssue({
					...
					message: "trace event sequences must be contiguous and ordered",
				});
			}
			expected += 1;
			...
			if (event.runId !== artifact.runId || (!isChildEvent && event.taskId !== artifact.taskId)) {
				ctx.addIssue({
					code: "custom",
					path: ["events", index],
					message: "trace event identity does not match its artifact",
				});
			}
		}
	});
...
export function capTraceText(
	value: string,
	max = TRACE_MAX_TEXT_CHARS,
): string {
	if (max <= 0) return "";
...
function phaseForTraceType(type: TraceEventType): TraceEvent["phase"] {
	...
	if (type.startsWith("artifact") || type.endsWith(".delivered"))
		return "artifact";
...
export function traceEventFromGateway(
	...
	switch (event.type) {
		...
		case "task.routed":
			type = "task.routed";
			detail = { planMode: event.detail.planMode };
			break;
		...
		case "verify.completed":
			...
			detail = {
				...
				...(event.detail.evidence === undefined
					? {}
					: { evidence: boundVerificationEvidence(event.detail.evidence) }),
			};
			break;
		...
		case "merge.completed":
			type = "artifact.accepted";
			detail = { commitId: capTraceText(event.detail.commitId) };
			break;
		case "merge.conflict":
			type = "artifact.rejected";
			detail = {
				conflicts: event.detail.conflicts.map((conflict) =>
					capTraceText(conflict),
				),
			};
			break;
		case "permission.requested":
			type = "failure";
			detail = {
				requestId: capTraceText(event.requestId),
		...
		case "task.failed":
			type = "task.failed";
			detail = {
				cause: capTraceText(event.detail.cause),
				stage: event.detail.stage ?? "internal",
				code: event.detail.code ?? "unclassified",
			};
			break;
		case "task.escalated":
			type = "task.escalated";
...
export function writeTraceArtifact(
	trace: TraceArtifact,
	artifactsDir: string,
): TraceWriteResult {
	...
	if (
		...
		/^[A-Za-z0-9][A-Za-z0-9._:-]*$/.test(trace.runId)
	) {
		...
		if (temporary !== undefined) {
			try {
				...
				if ((cleanupError as NodeJS.ErrnoException).code !== "ENOENT") {
					return {
						ok: false,
						error: `${failure}; temporary trace cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`,
					};
				}
			}
		}
