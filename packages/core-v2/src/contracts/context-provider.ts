...
import { z } from "zod";
import type { ContextHandle, ContextRetrievalOptions } from "../context/retrieval.ts";

export const ContextProviderIdentitySchema = z.object({
	id: z.string().min(1).max(128),
	version: z.string().min(1).max(128),
}).strict();
export type ContextProviderIdentity = z.infer<typeof ContextProviderIdentitySchema>;
...
export const ContextArtifactHandleSchema = z.object({
	id: z.string().min(1).max(256),
	kind: z.enum(["file", "symbol"]),
	path: z.string().min(1).max(512).refine((value) => !value.startsWith("/") && !value.split("/").includes(".."), "handle path must be repository-relative"),
	symbol: z.string().max(256).optional(),
	language: z.string().min(1).max(64),
	status: z.enum(["indexed", "binary", "unsupported", "oversized", "unreadable"]),
