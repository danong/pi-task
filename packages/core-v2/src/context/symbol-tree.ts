...
const SYMBOL_PATTERNS: Record<string, RegExp> = {
	...
	javascript: /^\s*(?:export\s+)?(?:default\s+)?(?:async\s+)?(?:function|class|const|let|var)\s+([A-Za-z_$][\w$]*)/,
	kotlin: /^\s*(?:fun|class|interface|object|typealias|val|var)\s+([A-Za-z_]\w*)/,
	...
	swift: /^\s*(?:public\s+|private\s+|internal\s+|static\s+)*(?:func|class|struct|enum|protocol)\s+([A-Za-z_]\w*)/,
	typescript: /^\s*(?:export\s+)?(?:default\s+)?(?:abstract\s+)?(?:async\s+)?(?:function|class|interface|type|enum|const|let|var)\s+([A-Za-z_$][\w$]*)/,
};
...
function extractSymbols(
	...
	for (const line of text.split(/\r?\n/)) {
		...
		if (symbol === undefined || Buffer.byteLength(symbol, "utf8") > limits.maxSymbolBytes)
			continue;
...
export function scanSymbolTree(options: ScanSymbolTreeOptions): SymbolTree {
	...
	if (!statSync(root).isDirectory()) throw new Error(`symbol tree root is not a directory: ${options.root}`);
	const limits: SymbolTreeLimits = {
		maxFileBytes: positiveInteger(options.maxFileBytes, DEFAULT_MAX_FILE_BYTES, "maxFileBytes"),
		maxSymbolsPerFile: positiveInteger(options.maxSymbolsPerFile, DEFAULT_MAX_SYMBOLS_PER_FILE, "maxSymbolsPerFile"),
		maxSymbolBytes: positiveInteger(options.maxSymbolBytes, DEFAULT_MAX_SYMBOL_BYTES, "maxSymbolBytes"),
	};
