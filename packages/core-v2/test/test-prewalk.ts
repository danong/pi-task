...
import type { SessionHandle, SessionHostEvent } from "../src/sessions/host.ts";
...
export async function runTests(): Promise<void> {
	...
	try {
		// ─── Cost model: long task + big context → swap wins ────────────
		{
			...
			check(
				d.stayCostUsd > d.swapCostUsd,
				"stay projected more expensive when swapping wins",
			);
		}
		...
		// ─── Identical pricing → never swap (per-turn saving ≤ 0) ────────
		{
			...
			check(
				!d.swap && d.breakEvenTurns === Number.POSITIVE_INFINITY,
		...
		// ─── Wiring: fires once on the first successful edit ─────────────
		{
			...
			check(
				setModels === 1 && swaps === 1,
				`swap fired exactly once on the first successful edit (got ${setModels}/${swaps})`,
			);
		...
		// ─── Wiring: unprofitable decision → no swap ─────────────────────
		{
			...
			attachPrewalk(fake, {
				...
				decide: () =>
					decidePrewalkSwap({
						contextTokensAtSwap: 1_200,
						remainingTurnsEstimate: 1,
						pricing: STAY_WINS,
					}),
			});
	...
	console.log(
		"✓ prewalk: break-even model both directions, single-fire wiring, default-off respected",
	);
}
