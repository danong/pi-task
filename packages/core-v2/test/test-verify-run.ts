...
import { runVerification, VERIFY_OUTPUT_TAIL_CHARS, VERIFY_TIMEOUT_EXIT_CODE } from "../src/verify/run.ts";
...
export async function runTests(): Promise<void> {
	...
	try {
		// ─── Passing + vacuous ────────────────────────────────────────
		{
			...
			check(pass.passed && pass.commands.length === 1 && pass.commands[0]?.exitCode === 0,
				`"true" passes with one recorded command, got ${JSON.stringify(pass)}`);

			const empty = await runVerification([], { cwd: dir });
			check(empty.passed && empty.commands.length === 0, "empty command list passes vacuously");
		}

		// ─── Failures: exit codes, aggregation, order ─────────────────
		{
			const result = await runVerification(["true", "exit 3", "false"], { cwd: dir });
			check(!result.passed, "a failing command fails the suite");
			check(result.commands.length === 3, "every command runs — failures do not stop the suite");
			const failed = result.commands.filter((c) => c.exitCode !== 0);
			check(failed.length === 2, `two failures aggregated, got ${failed.length}`);
			check(failed[0]?.command === "exit 3" && failed[0]?.exitCode === 3,
				"failures keep command order with correct exit codes");
			check(failed[1]?.command === "false" && failed[1]?.exitCode === 1, '"false" exits 1');
		}
		...
		// ─── Output tails captured + capped ───────────────────────────
		{
			...
			check((cmd?.stdoutTail ?? "").includes("out-marker"), "stdout tail captured");
			check((cmd?.stderrTail ?? "").includes("err-marker"), "stderr tail captured");

			const flood = `printf '${"x".repeat(VERIFY_OUTPUT_TAIL_CHARS * 4)}'; exit 1`;
			const capped = await runVerification([flood], { cwd: dir });
			check(capped.commands[0]?.stdoutTail.length === VERIFY_OUTPUT_TAIL_CHARS,
				`stdout tail capped at ${VERIFY_OUTPUT_TAIL_CHARS}, got ${capped.commands[0]?.stdoutTail.length}`);
			check(capped.commands[0]?.stderrTail.length === 0, "empty stderr stays empty");
		}

		// ─── Per-command timeout: hung sleep killed ───────────────────
		{
			const hung = await runVerification(["sleep 30"], { cwd: dir, commandTimeoutMs: 200 });
			const cmd = hung.commands[0];
			check(cmd?.timedOut === true, "hung sleep reports timedOut");
			check(cmd?.exitCode === VERIFY_TIMEOUT_EXIT_CODE, `timeout exits ${VERIFY_TIMEOUT_EXIT_CODE}`);
		...
		// ─── Bounded grace: in-flight command finishes past the wall ──
		{
			...
			check(cmd !== undefined && cmd.exitCode === 0 && !cmd.timedOut,
	...
	console.log("✓ verify/run: pass/vacuous, exit-code aggregation, output caps, per-command timeout, wall, grace");
}
