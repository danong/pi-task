	...
	while (stack.length > 0) {
		...
		for (const e of readdirSync(dir, { withFileTypes: true })) {
			...
			if (e.isDirectory()) stack.push(full);
				...
				snap.set(full, statSync(full).mtimeMs ^ statSync(full).size);
		}
	}
...
export function runTests(): Promise<void> {
	...
	try {
		// ─── Read-only behavior (R1) ─────────────────────────────────
		{
			...
			check(
				out.kind === "report",
				`ungated survey produces a report (got ${out.kind})`,
			);
			if (out.kind !== "report") throw new Error("survey unexpectedly denied");
			const after = snapshotTree(fx.root);
			check(
				after.size === before.size,
				`no files created or removed by survey (${before.size} → ${after.size})`,
			);
			for (const [path, sig] of before) {
				check(after.get(path) === sig, `unmutated: ${path}`);
			}
			check(
				out.report.summary.gateDecision === "ungated",
				"no gate → decision recorded as ungated",
			);
			// node_modules never entered: its file is invisible to findings
			// AND to the scanned count.
			check(
				out.report.summary.filesScanned === 4,
				`skip-list respected (scanned ${out.report.summary.filesScanned}, want 4)`,
			);
			check(
				!findingLocations(out.report.findings).includes(
		...
		// ─── Bundle + continuation inputs fold into typed findings ────
		{
			...
			const out = runWorkflowSurvey({
				...
				bundleCandidates: [
					fx.files[1]!,
					join(fx.root, "never-bundled.ts"),
					join(fx.root, "missing.ts"),
				],
				continuation: {
			...
			check(
				cats.includes("bundle-candidate"),
				"candidate dropped by builder surfaces as finding",
			);
			check(
				cats.includes("continuation-budget"),
				"over-budget continuation surfaces as finding",
			);
			check(
			...
			check(
				out.report.findings.some(
					(f) =>
						f.location === join(fx.root, "missing.ts") && f.severity === "warn",
				),
		...
		// ─── Bounded + deterministic output (R3) ──────────────────────
		{
			...
			check(
				full.report.summary.truncated === false,
				"default budget does not truncate small fixture",
			);
			check(
			...
			check(
				JSON.stringify(locs) === JSON.stringify([...locs].sort()),
				`findings sorted by location (got ${JSON.stringify(locs)})`,
			);

			// Cap: maxFindings=1 truncates honestly.
			const capped = runWorkflowSurvey({
				root: fx.root,
				budget: { ...budget, maxFindings: 1 },
			});
			if (capped.kind !== "report")
				throw new Error("survey unexpectedly denied");
			check(capped.report.findings.length === 1, "maxFindings cap applied");
			check(
				capped.report.summary.truncated === true,
				"truncation flagged in summary",
			);
			check(
				capped.report.findings[0]?.severity ===
					full.report.findings[0]?.severity &&
					capped.report.findings[0]?.location ===
						full.report.findings[0]?.location,
			...
			writeFileSync(
				join(fx2root, "src", "b.ts"),
				"export const b = 2;\n",
				"utf-8",
			);
			writeFileSync(
				join(fx2root, "src", "a.ts"),
				"export const a = 1;\n",
				"utf-8",
			);
			writeFileSync(join(fx2root, "README.md"), "readme\n", "utf-8");
			writeFileSync(
				join(fx2root, "node_modules", "pkg", "index.js"),
				"module.exports = {};\n",
				"utf-8",
			);
			const reversed = runWorkflowSurvey({ root: fx2root, budget });
			if (reversed.kind !== "report")
				throw new Error("survey unexpectedly denied");
			...
			const normalize = (s: string): string =>
				s
					...
					.join("\n");
			check(
				normalize(renderSurveyReport(full.report)) ===
					normalize(renderSurveyReport(reversed.report)),
		...
		// ─── Idempotent re-run (R2) ───────────────────────────────────
		{
			...
			check(
				r3.report.summary.filesScanned === r1.report.summary.filesScanned + 1,
				"added file increments scanned count",
			);
			check(
				renderSurveyReport(r3.report) !== render1,
				"changed tree → different render",
			);
		}
		...
		// ─── Human-gate surface (R3) ──────────────────────────────────
		{
			...
			check(
				denied.kind === "denied",
				`denied gate → typed denial (got ${denied.kind})`,
			);
			check(asked === 1, "gate consulted exactly once");
			check(
				denied.kind === "denied" &&
					denied.reason.includes(SURVEY_PERMISSION_ACTION),
			...
			gateway.on("permission.requested", (event) => {
				if (event.type === "permission.requested")
					seen.push({ action: event.action, requestId: event.requestId });
			});
			const gate = gatewaySurveyGate(gateway, "t-survey", "sess-1", () => true);
			const granted = runWorkflowSurvey({ root: fx.root }, gate);
			check(
				granted.kind === "report",
				`granted gate → report (got ${granted.kind})`,
			);
			if (granted.kind === "report") {
				check(
					granted.report.summary.gateDecision === "granted",
					"granted decision recorded on summary",
				);
			}
			check(
				seen.length === 1 && seen[0]?.action === SURVEY_PERMISSION_ACTION,
		...
		{
			const missing = runWorkflowSurvey({
				root: join(parent, "does-not-exist"),
			});
			if (missing.kind !== "report")
				throw new Error("survey unexpectedly denied");
			check(
				missing.report.summary.filesScanned === 0,
				"missing root scans zero files",
			);
			check(
				missing.report.findings.some((f) =>
					f.message.includes("directory unreadable"),
				),
	...
	if (errors.length > 0) {
		throw new Error(
			`${errors.length} workflow-survey failure(s):\n- ${errors.join("\n- ")}`,
		);
	}
