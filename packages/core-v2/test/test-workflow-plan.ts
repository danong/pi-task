...
import { DEFAULT_MAX_FAN_OUT, DagCycleError, synthesizeDag, type DagNode } from "../src/workflow/dag.ts";
import {
	GateError,
	planApprovalStatus,
	recordPlanApproval,
...
function specWith(dependsOn?: string[]): string {
	const deps = dependsOn === undefined ? "" : `\n## Depends On\n${dependsOn.map((d) => `- ${d}`).join("\n")}\n`;
	return `## Goal\ng\n\n## Requirements\n- R1: x\n\n## Verification\n- true\n${deps}`;
}
...
export async function runTests(): Promise<void> {
	...
	// ─── R1: side-effect-free spec validation ──────────────────────
	{
		...
		check(parsed.verificationCommands.length === 2, "verification commands parsed");
		check(JSON.stringify(parsed.dependsOn) === JSON.stringify(["setup-spec"]), "depends_on linkage field parsed");

		// Typed failures per missing/malformed dimension.
		check(expectTypedError(() => validateSpec("## Requirements\n- x\n\n## Verification\n- true\n")) === "missing_goal",
			"missing goal fails typed");
		check(expectTypedError(() => validateSpec("## Goal\ng\n\n## Verification\n- true\n")) === "missing_requirements",
			"empty requirements fail typed");
		check(expectTypedError(() => validateSpec("## Goal\ng\n\n## Requirements\n- x\n")) === "missing_verification",
			"missing verification fails typed");
		check(expectTypedError(() => validateSpec(specWith().replace("- true", "- echo 'unterminated"))) === "bad_verification_command",
			"unterminated quote fails typed");

		// Bash-line well-formedness is syntax-only and never executes.
		check(isWellFormedBashLine("true && false"), "compound command accepted syntactically");
		check(!isWellFormedBashLine(""), "empty command rejected");
		check(!isWellFormedBashLine("echo \\"), "trailing line-continuation rejected");
		check(!isWellFormedBashLine('echo "open'), "unterminated double quote rejected");
		check(!isWellFormedBashLine("echo 'open"), "unterminated single quote rejected");
		check(!isWellFormedBashLine("echo `open"), "unterminated backtick rejected");
		check(isWellFormedBashLine(`echo "it's closed"`), "nested quote closed correctly accepted");

		// depends_on validated where present: absent section tolerated.
		check(parseDependsOn("## Goal\ng\n").length === 0, "absent depends_on parses as empty");
	}

	// ─── R2: pure DAG synthesis ────────────────────────────────────
	{
		check(DEFAULT_MAX_FAN_OUT > 0, "default fan-out guard is a positive sane limit");

		const dag = synthesizeDag([node("b", ["a"]), node("a"), node("c", ["a", "b"])]);
		const pos = new Map(dag.order.map((id, i) => [id, i]));
		check(dag.order.length === 3, "all nodes ordered exactly once");
		check(pos.get("a")! < pos.get("b")! && pos.get("a")! < pos.get("c")! && pos.get("b")! < pos.get("c")!,
			`topological order respects dependencies, got ${dag.order.join(",")}`);

		// Determinism: same input list → same order across calls.
		const again = synthesizeDag([node("b", ["a"]), node("a"), node("c", ["a", "b"])]);
		check(again.order.join(",") === dag.order.join(","), "synthesis is deterministic");
		...
		check(cyclePath.length >= 3, `cycle surfaced with a path, got ${cyclePath.join(" → ")}`);
		check(expectTypedError(() => synthesizeDag([node("a", ["a"])])) === "cycle", "self-edge detected as cycle");

		// Fan-out guard is configurable.
		check(expectTypedError(() =>
			synthesizeDag([node("x", ["d1", "d2", "d3"]), node("d1"), node("d2"), node("d3")], { maxFanOut: 2 }),
		) === "fan_out_exceeded", "fan-out over the configured limit fails typed");
		check(synthesizeDag([node("x", ["d1", "d2", "d3"]), node("d1"), node("d2"), node("d3")]).order.length === 4,
			"same graph passes under the default guard");

		check(expectTypedError(() => synthesizeDag([node("a", ["ghost"])])) === "unknown_dependency",
			"dangling dependency fails typed");
		check(expectTypedError(() => synthesizeDag([node("a"), node("a")])) === "duplicate_id",
	...
	// ─── R3: human gate — dry vs real persistence on the ledger seam ──
	{
		...
		try {
			...
			check(store.db.prepare("PRAGMA user_version").get()!.user_version === LEDGER_SCHEMA_VERSION,
			...
			check(refused.includes("plan first") && refused.includes("--approve"),
			...
			check(planApprovalStatus(store, "dag-1").status === "pending", "dry plan records pending approval");
			let stillRefused = "";
			try {
				requirePlanApproved(store, "dag-1");
			} catch (err) {
			...
			check(stillRefused.includes("NOT approved") && stillRefused.includes("--approve"),
			...
			check(planApprovalStatus(store, "dag-1").status === "approved", "--approve flips the row to approved");
			let threw = false;
			try {
				requirePlanApproved(store, "dag-1");
			} catch {
			...
			recordPlanApproval(store, { dagId: "dag-1", approved: false });
			check(planApprovalStatus(store, "dag-1").status === "pending", "re-plan without approval revokes");

			// Per-DAG isolation on the shared ledger.
			recordPlanApproval(store, { dagId: "dag-2", approved: true });
			check(planApprovalStatus(store, "dag-1").status === "pending" && planApprovalStatus(store, "dag-2").status === "approved",
			...
			check(planApprovalStatus(reopened, "dag-2").status === "approved", "approval row persists across reopen");
			reopened.close();
		} finally {
	...
	console.log("✓ workflow-plan: side-effect-free validation, DAG order/cycle/fan-out, dry-vs-real human gate");
}
