...
//
// Every probe is a VALUE statement (not a bare type alias): tsc skips
// unreferenced local type aliases, so compile-time assertions must be
// anchored to a checked statement to enforce anything. The assertion
// itself fails to compile whenever the probe resolves to false.
void (true as Expect<SurfaceEvent extends { type: string } ? true : false>);
void (true as Expect<
	IsAssignable<{ type: "Receipt"; receipt: TaskReceipt }, SurfaceEvent>
>);
void (true as Expect<
	IsAssignable<{ type: "TurnDelta"; text: string }, SurfaceEvent>
>);
void (true as Expect<
	IsAssignable<
		{
...
function makeBundle(taskId: string, model: ModelAssignment): ExecutionBundle {
	...
	for (let i = 0; i < 3; i++) {
		files.push({
			...
			astOutline:
				i % 2 === 0 ? "export function fn" : `export const v = 1; // ${i}`,
...
export async function runTests(): Promise<void> {
	...
	// ─── Schema round-trips ────────────────────────────────────────────
	{
		const assignment = ModelAssignmentSchema.parse({
			model: "fast/m",
			lane: "flex",
		});
		check(
			assignment.model === "fast/m" && assignment.lane === "flex",
			"ModelAssignment round-trip",
		);
		check(
			ModelAssignmentSchema.parse({}).model === undefined,
			"ModelAssignment fully-optional round-trip",
		);

		const bundle = makeBundle("t1", { lane: "flex" });
		const parsedBundle = ExecutionBundleSchema.parse(bundle);
		check(
			parsedBundle.taskId === bundle.taskId &&
				parsedBundle.targetFiles.length === 3,
			"ExecutionBundle round-trip",
		);
		const jsonRoundTrip = JSON.parse(
			JSON.stringify(parsedBundle),
		) as ExecutionBundle;
		...
		check(
			yieldParsed.deviations.length === 0,
			"Yield round-trip (empty deviations OK)",
		);
	...
	// ─── Deterministic serialization (NFR-4 / NFR-3) ──────────────────
	{
		...
		check(
			h1 === h2,
			"HandoffBundle serialization ignores ledger-only precedingSessionId",
		);
		check(
			!h1.includes("sess-A") && !h1.includes("precedingSessionId"),
			"ledger-only field absent from serialization",
		);
		...
		check(
			es1 === es2,
			"ExecutionBundle serialization ignores ledger-only timestamps/run/session ids",
		);
		check(
			!es1.includes("2026") && !es1.includes("runId"),
			"no ledger-only literal serializes into a prompt",
		);

		// Prompt-bound differences DO change the bytes (bundle is not lost).
		const e3 = { ...makeBundle("t1", {}), goal: "Ship a different feature." };
		check(
			serializeForPrompt(ExecutionBundleSchema, e3) !== es1,
			"prompt-bound difference changes the bytes",
		);
	}
	...
	// ─── ControlSurface subscription levels ───────────────────────────
	{
		...
		const receiptsOnly = LEVEL_EVENTS.receipts.filter(
			(e) => !LEVEL_EVENTS.digest.includes(e),
		);
		check(receiptsOnly.length === 0, "receipts ⊆ digest");
		check(
			LEVEL_EVENTS.receipts.every((e) => full.includes(e)),
			"receipts ⊆ delta",
		);
	}
	...
	// ─── Per-seam smoke tests (Hermetic fakes, FR-11 discipline) ──────
	{
		...
		const wsDriver: import("../src/contracts/workspace-driver.ts").WorkspaceDriver =
			{
				...
				createWorkspace(taskId, parentBranch) {
					states.push("provisioning");
					return Promise.resolve({
						taskId,
						hostPath: `/tmp/ws/${taskId}`,
						branchName: parentBranch ?? "task",
		...
		check(
			ws.status === "provisioning" && ws.hostPath.startsWith("/tmp/ws/"),
			"workspace created",
		);
		check(
			(await wsDriver.mergeWorkspace(ws)).success === true,
			"workspace merged",
		);
		await wsDriver.cleanupWorkspace(ws);
		check(
			states.join(",") === "provisioning,merging,cleaning_up",
			"workspace lifecycle order",
		);
		...
		const res = await envDriver.exec("true", ["a", "b"], {
			timeoutMs: 1000,
			readOnly: true,
		});
		check(
			res.exitCode === 0 &&
				calls.length === 1 &&
				calls[0]!.args.join(",") === "a,b",
			"env exec recorded+timeout honored",
		);
		...
		const compressor: import("../src/contracts/context-compressor.ts").ContextCompressor =
			{
				name: "test-regex",
				isSupported: () => Promise.resolve(true),
				generateOutline: (_file, opts) =>
					Promise.resolve({
						outline: "export function a(){}".slice(0, opts.maxTokens),
						truncated: false,
						cursor: null,
					}),
				extractSymbols: () => Promise.resolve("a"),
			};
		const page = await compressor.generateOutline("a.ts", { maxTokens: 5 });
		check(
			page.outline.length <= 5 && !page.truncated,
			"compressor respects maxTokens",
		);
		...
		const verifyDriver: import("../src/contracts/verification-driver.ts").VerificationDriver =
			{
				name: "test-verify",
				runVerification: (_ctx, commands): Promise<VerificationResult> =>
					Promise.resolve({
		...
		check(
			vres.passed === false && vres.commands.length === 2,
		...
		check(
			transformed && out.taskId === "t1",
			"plugin transform hook runs on real bundle",
		);
					...
					const stream2: SurfaceStream = {
						events: (async function* () {
							for (const e of events) {
								...
								if (allowed.includes(e.type)) yield e;
							}
						})(),
						send: () => undefined,
						close: () => undefined,
		...
		const receipts = await (async () => {
			const got: string[] = [];
			for await (const e of surface.connect("s", "receipts").events)
				got.push(e.type);
			return got;
		})();
		...
		check(
			!receipts.includes("StatusSnapshot"),
			"receipts level excludes daemon-state snapshots",
		);
	}
