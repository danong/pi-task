...
import type { WatchdogAction, WatchedEventType } from "../src/guards/watchdogs.ts";
...
function isAbort(action: WatchdogAction): action is Extract<WatchdogAction, { kind: "abort" }> {
...
export async function runTests(): Promise<void> {
	...
	// ─── (a) settle-without-yield ────────────────────────────────────
	{
		...
		for (const eventType of EVENT_TYPES) {
			...
			check(first.kind === "continue" && second.kind === "continue",
		...
		for (const nudged of [false, true]) {
			...
			check(withYield.kind === "continue", `settled + yielded → continue (nudged=${nudged})`);
		}

		// First settle without yield → nudge (exactly once).
		const first = decideSettleAction("settled", false, false);
		check(first.kind === "nudge", `first settle without yield → nudge, got ${first.kind}`);
		check(first.kind === "nudge" && first.text === DEFAULT_SETTLE_NUDGE_TEXT && first.text.length > 0,
		...
		check(second.kind === "abort", `second settle without yield → abort, got ${second.kind}`);
		check(isAbort(second) && second.reason === "settled_without_yield",
			"second-settle abort names the settled_without_yield reason");
		check(isAbort(second) && second.message === settledWithoutYieldMessage() && second.message.includes("yield"),
	...
	// ─── (b) no-progress ─────────────────────────────────────────────
	{
		...
		const below = decideNoProgressAction({ ...base, nowMs: 1_000 + windowMs - 1 });
		check(below.kind === "continue", "activity inside the window → continue");

		const atBoundary = decideNoProgressAction({ ...base, nowMs: 1_000 + windowMs });
		check(isAbort(atBoundary), "silence of exactly the window → abort (inclusive boundary)");
		check(isAbort(atBoundary) && atBoundary.reason === "no_progress", "no-progress abort names its reason");

		const beyond = decideNoProgressAction({ ...base, nowMs: 1_000 + windowMs * 3 });
		...
		const inTool = decideNoProgressAction({ ...base, nowMs: 1_000 + windowMs * 99, inFlightTool: true });
		check(inTool.kind === "continue", "in-flight tool suppresses the no-progress abort");

		// The abort message names the cause and the window.
		const message = noProgressMessage(windowMs);
		check(isAbort(beyond) && beyond.message === message, "abort message comes from the pure builder");
		check(message.includes(String(windowMs)) && message.includes(formatDurationMs(windowMs)),
	...
	// ─── (c) wall clock ──────────────────────────────────────────────
	{
		...
		const below = decideWallAction({ ...base, nowMs: 5_000 + wallTimeoutMs - 1 });
		check(below.kind === "continue", "inside the wall budget → continue");

		const atBoundary = decideWallAction({ ...base, nowMs: 5_000 + wallTimeoutMs });
		check(isAbort(atBoundary), "exactly the wall budget → abort (inclusive boundary)");
		check(isAbort(atBoundary) && atBoundary.reason === "wall_timeout", "wall abort names its reason");

		const beyond = decideWallAction({ ...base, nowMs: 5_000 + wallTimeoutMs * 2 });
		check(isAbort(beyond) && beyond.message === wallTimeoutMessage(wallTimeoutMs), "past the wall → abort");
		check(wallTimeoutMessage(wallTimeoutMs).includes(String(wallTimeoutMs))
			&& wallTimeoutMessage(wallTimeoutMs).includes(formatDurationMs(wallTimeoutMs)),
	...
	// ─── (d) per-tool timeout ────────────────────────────────────────
	{
		...
		const below = decideToolTimeoutAction({ ...base, nowMs: 42_000 + timeoutMs - 1 });
		check(below.kind === "continue", "tool inside its bound → continue");

		const atBoundary = decideToolTimeoutAction({ ...base, nowMs: 42_000 + timeoutMs });
		check(isAbort(atBoundary), "tool at exactly its bound → abort (inclusive boundary)");
		check(isAbort(atBoundary) && atBoundary.reason === "tool_timeout", "tool abort names its reason");

		const hung = decideToolTimeoutAction({ ...base, nowMs: 42_000 + timeoutMs * 5, toolName: "edit" });
		check(isAbort(hung) && hung.message === toolTimeoutMessage(timeoutMs, "edit"), "past the bound → abort");
		check(toolTimeoutMessage(timeoutMs, "edit").includes("edit")
	...
	{
		check(formatDurationMs(0) === "0ms", `0ms formats, got ${formatDurationMs(0)}`);
		...
		check(formatDurationMs(120_000) === "2m", "exact minutes drop the remainder");
	...
	// ─── purity / determinism ────────────────────────────────────────
	{
		const samples: WatchdogAction[] = [
			...
			decideNoProgressAction({ nowMs: 10, lastActivityMs: 0, windowMs: 5, inFlightTool: false }),
			decideWallAction({ nowMs: 10, startedAtMs: 0, wallTimeoutMs: 5 }),
			decideToolTimeoutAction({ nowMs: 10, startedAtMs: 0, timeoutMs: 5, toolName: "bash" }),
		];
		...
		check(JSON.stringify(samples) === JSON.stringify(replay), "identical inputs decide identically");
	}

	if (errors.length > 0) {
		throw new Error("test-watchdogs failed:\n  ✗ " + errors.join("\n  ✗ "));
	}
	console.log("✓ watchdogs: settle/no-progress/wall/tool-timeout pure decisions, messages, determinism");
}
