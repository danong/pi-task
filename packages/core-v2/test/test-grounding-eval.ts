...
export function runTests(): Promise<void> {
	...
	// ─── Config enumeration: every plan mode covered, ids unique ────────
	{
		...
		for (const mode of ["cold", "prewalk", "bundle", "fork"] as const) {
			check(
				modes.has(mode),
				`plan mode ${mode} reachable in GROUNDING_CONFIGS`,
			);
		}
	...
	// ─── Strong-model gate: default excludes, flag includes ─────────────
	{
		...
		check(
			gated.every((c) => !isStrongConfig(c)),
			"default selection has no strong configs",
		);
		check(
			filterConfigs({ includeStrong: true }).length > gated.length,
		...
		try {
			filterConfigs({
				includeStrong: false,
				configFilter: ["daemon-prewalk-strong"],
			});
		} catch (err) {
		...
		const narrowed = filterConfigs({
			includeStrong: true,
			configFilter: ["bare", "daemon-bundle"],
		});
		check(
			narrowed.length === 2 &&
				narrowed.every((c) => c.id === "bare" || c.id === "daemon-bundle"),
	...
	// ─── Plan assembly: cross product, tier baselines, gating flags ────
	{
		...
		check(
			plan.rows.length === filterConfigs({ includeStrong: false }).length * 2,
			"plan is configs × specs",
		);
		check(
			plan.rows.every((r) => !r.gated),
			"ungated selection marks nothing gated",
		);

		// bench-good entry preferred over default for SPEC_A.
		const a = plan.rows.find((r) => r.specId === "spec-a")!;
		check(
			a.expectedDurationMs === 8_000 && a.expectedCostUsd === 0.02,
			"tier baseline wins when present",
		);
		const b = plan.rows.find((r) => r.specId === "spec-b")!;
		check(
			b.expectedDurationMs === 20_000 && b.expectedCostUsd === 0.005,
			"missing tier falls back to default baseline",
		);
		check(
			baselineForSpec(SPEC_B, "no-such-tier").durationMs === 20_000,
			"baselineForSpec falls back to default for unknown tiers",
		);
		check(
			Math.abs(
				plan.totalExpectedDurationMs -
					plan.rows.reduce((s, r) => s + r.expectedDurationMs, 0),
			) < 1e-9,
		...
		check(
			strongPlan.rows.some((r) => r.gated),
			"strong rows flagged gated in the plan",
		);

		const lines = renderGroundingPlan(plan, { gatedIncluded: false });
		const text = lines.join("\n");
		check(
			text.includes("dry run") && text.includes("planned runs"),
			"plan header rendered",
		);
		check(text.includes("--allow-strong"), "plan states how to ungate");
		check(
			formatEvalDuration(33_207) === "33.2s",
			`duration fmt small (${formatEvalDuration(33_207)})`,
		);
		check(
			formatEvalDuration(65_000).startsWith("1m"),
			`duration fmt large (${formatEvalDuration(65_000)})`,
		);
	}
	...
	// ─── Aggregation: COR from sums, rates exact, nulls respected ──────
	{
		...
		const b = aggs.get("b")!;
		check(
			b.bundleHitRate !== null && Math.abs(b.bundleHitRate - 0.5) < 1e-9,
	...
	{
		const recs = [
			record({ configId: "n", costUsd: 0.1, filesChanged: 2 }),
		...
		check(
			perFile !== null && Math.abs(perFile - 0.15 / 5) < 1e-12,
			`USD/file = total cost ÷ total files (${perFile})`,
		);
		check(
			costPerFileChanged(recs, "m") === null,
			"zero-diff config → null (nothing to normalize)",
		);
		check(costPerFileChanged(recs, "absent") === null, "unknown config → null");
		const agg = aggregateRecords(recs).get("n")!;
		check(
			agg.costPerFileChangedUsd !== null &&
				Math.abs(agg.costPerFileChangedUsd - 0.03) < 1e-12,
	...
	// ─── NFR-4: violations counted, retry rate over known outcomes ─────
	{
		check(
			!isCacheAffinityViolation(
				record({ retriedWithHandoff: false, cacheHitOnRetry: false }),
			),
		...
		check(
			agg.retryCacheHitRate !== null &&
				Math.abs(agg.retryCacheHitRate - 0.5) < 1e-9,
	...
	// ─── Wins/loses + summary rendering ────────────────────────────────
	{
		...
		check(
			buildWinsLoses([]).length === 0,
			"empty aggregates → empty winners table",
		);
		...
		check(
			...
			"summary carries the metric table header",
		);
		check(
			text.includes("cheap") && text.includes("thorough"),
			"summary lists both configs",
		);
		check(
			text.includes("Where each mode wins / loses"),
			"summary carries the wins/loses section",
		);
		check(text.includes("NFR-4"), "summary surfaces cache-affinity accounting");
		check(
			text.includes("extensions/task/bench-regression.ts"),
			"summary points readers at the fixture/spec owner file",
		);

		const empty = renderSummaryLines([], [], []);
		check(
			empty.some((l) => l.includes("No evidence recorded yet")),
	...
	// ─── Evidence store round-trip (real fs, temp dir only) ────────────
	{
		...
		try {
			...
			check(
				records.length === 2 && corrupt === 1,
				`JSONL round-trip skips corrupt lines (${records.length}/${corrupt})`,
			);
			check(
				loadRecords(join(dir, "missing")).records.length === 0,
			...
			check(
				lines.length > 0 &&
					lines[0]!.startsWith("# Suite-03 grounding evaluation"),
