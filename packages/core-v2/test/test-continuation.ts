...
function indicesOf(
	...
	kept: readonly ContinuationEntry[],
): number[] {
...
export function runTests(): Promise<void> {
	...
	{
		check(
			recencyToolScorer([], 1000).length === 0,
			"recencyTool: empty input → empty output",
		);
		check(
			uniformScorer([], 1000).length === 0,
			"uniform: empty input → empty output",
		);
		check(
			pruneContinuation([], 1000).length === 0,
			"pruneContinuation: empty input → empty output",
		);
	}
	...
	{
		const out = recencyToolScorer(
			[entry("user", "task"), entry("toolResult", "r", "read")],
			0,
		);
		check(
			out.length >= 1 && out.length <= 2,
			`zero-budget keeps ≤2 entries (got ${out.length})`,
		);
	...
	// ─── R4: budget respected — greedy keep never exceeds budget ────────
	{
		const entries = Array.from({ length: 40 }, () =>
			entry("toolResult", `x`.repeat(160), "read"),
		);
		// each ≈ 44 tokens; 40 × 44 ≈ 1760 total. Budget 400 → subset.
		const out = recencyToolScorer(entries, 400);
		check(
			out.length < entries.length,
			`budget forces pruning (${out.length}/${entries.length})`,
		);
		check(
			spentTokens(out) <=
				400 +
					estimateEntryTokens(entries[entries.length - 1] as ContinuationEntry),
	...
	// ─── R4: ordering preserved — kept entries appear in original order ──
	{
		...
		for (const scorer of [recencyToolScorer, uniformScorer]) {
			...
			check(
				bigBudget.map((e) => e.content).join("|") ===
	...
	{
		check(
			listScorers().includes("recencyTool"),
			"registry exposes recencyTool",
		);
		check(listScorers().includes("uniform"), "registry exposes uniform");
		check(
			selectScorer("uniform") === uniformScorer,
			"selectScorer resolves uniform by name",
		);
		check(
			selectScorer("recencyTool") === recencyToolScorer,
			"selectScorer resolves recencyTool by name",
		);
		let threw = false;
		try {
		...
		const viaPruneUniform = pruneContinuation(entries, 9, {
			scorer: "uniform",
		});
		const viaPruneRecency = pruneContinuation(entries, 9, {
			scorer: "recencyTool",
		});
		check(
			JSON.stringify(indicesOf(entries, viaPruneUniform)) !==
				JSON.stringify(indicesOf(entries, viaPruneRecency)),
	...
	// ─── R3: retry signal changes the outcome vs first attempt ──────────
	{
		const entries = Array.from({ length: 12 }, (_, i) =>
			i % 3 === 0
				? entry("toolResult", `r${i} `.repeat(8), "read")
				: entry("assistant", `t${i} `.repeat(8)),
		);
		...
		for (const scorer of [recencyToolScorer, uniformScorer]) {
			...
			check(
				spentTokens(retried) <= budget + 60,
				`${scorer.name}: retry output bounded`,
			);
			...
			check(
				retried.some((e) => e.role === "toolResult"),
	...
	if (errors.length > 0) {
		return Promise.reject(
			new Error(`continuation tests failed:\n  ✗ ${errors.join("\n  ✗ ")}`),
		);
	}
	console.log(
		"✓ continuation: budgets respected, order preserved, tool-result invariant, retry-aware scorers, config-driven selection",
	);
