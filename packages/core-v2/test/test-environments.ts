...
export async function runTests(): Promise<void> {
	...
	try {
		...
		const ctx = { hostPath: dir } as Parameters<
			HostEnvironmentDriver["resolvePath"]
		>[0];
		...
		// ─── resolvePath: identity on the host rung ──────────────────────
		{
			...
			check(
				resolved.effectivePath === dir && resolved.inContainer === false,
				"host resolvePath is identity",
			);
		}
		...
		// ─── exec: exit codes, cwd, env ──────────────────────────────────
		{
			...
			const env = await host.exec("printenv", ["V2_PROBE"], {
				cwd: dir,
				env: { V2_PROBE: "yes" },
			});
		...
		{
			const slow = await host.exec("sleep", ["5"], {
				cwd: dir,
				timeoutMs: 250,
			});
			check(
				slow.timedOut && slow.exitCode === 124,
				"hung command killed with 124",
			);
		}

		// ─── output caps ─────────────────────────────────────────────────
		{
			const big = await host.exec("head", ["-c", "20000", "/dev/zero"], {
				cwd: dir,
			});
			check(
				big.stdout.length <= ENV_OUTPUT_TAIL_CHARS,
				`stdout capped (got ${big.stdout.length})`,
			);
		}

		// ─── default timeout constant is sane ────────────────────────────
		check(
			DEFAULT_ENV_COMMAND_TIMEOUT_MS >= 60_000,
			"default command timeout >= 1m",
		);
		...
		// ─── mise: capability detection + wrapper (skips without mise) ───
		{
			...
			check(
				(available && maybe !== null) || (!available && maybe === null),
			...
			if (maybe !== null) {
				...
				check(
					probe.exitCode === 0 && probe.stdout.trim().startsWith("v"),
					"mise exec runs the wrapped command",
				);
			}
	...
	console.log(
		"✓ environments: host exec semantics, timeouts, caps, mise capability detection",
	);
}
