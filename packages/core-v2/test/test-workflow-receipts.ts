...
export function runTests(): Promise<void> {
	...
	// ─── R1: typed per-node summaries ──────────────────────────────
	{
		...
		check(
			n.nodeId === "n-b" && n.verdict === "ship",
			`nodeId/verdict carried (${n.nodeId}/${n.verdict})`,
		);
		check(
			n.inputTokens === 100 &&
				n.outputTokens === 30 &&
				n.cacheReadTokens === 50,
			`per-node token counters carried (${n.inputTokens}/${n.outputTokens}/${n.cacheReadTokens})`,
		);
		check(
			n.cacheWriteTokens === 0,
			`absent cacheWriteTokens treated as 0 (got ${n.cacheWriteTokens})`,
		);
		check(
			Math.abs(n.costUsd - 0.02) < 1e-12 && Math.abs(n.cor - 0.4) < 1e-12,
			"cost/cor carried",
		);
	...
	// ─── R1: aggregate sums over nodes ─────────────────────────────
	{
		...
		check(
			summary.aggregate.nodeCount === 3,
			`nodeCount = 3 (got ${summary.aggregate.nodeCount})`,
		);
		check(
			summary.aggregate.verdictCounts.ship === 1 &&
				summary.aggregate.verdictCounts.escalate === 1 &&
				summary.aggregate.verdictCounts.failed === 1,
			`verdict counts partitioned (${JSON.stringify(summary.aggregate.verdictCounts)})`,
		);
		check(
			summary.aggregate.totalInputTokens === 300,
			`Σinput = 300 (got ${summary.aggregate.totalInputTokens})`,
		);
		check(
			summary.aggregate.totalOutputTokens === 40,
			`Σoutput = 40 (got ${summary.aggregate.totalOutputTokens})`,
		);
		check(
			summary.aggregate.totalCacheReadTokens === 150,
			`ΣcacheRead = 150 (got ${summary.aggregate.totalCacheReadTokens})`,
		);
		check(
			summary.aggregate.totalCacheWriteTokens === 0,
			`ΣcacheWrite = 0 (got ${summary.aggregate.totalCacheWriteTokens})`,
		);
		check(
			Math.abs(summary.aggregate.totalCostUsd - 0.07) < 1e-12,
			`Σcost = 0.07 (got ${summary.aggregate.totalCostUsd})`,
		);
		check(
			summary.aggregate.totalFilesChanged === 4,
			`ΣfilesChanged = 4 (got ${summary.aggregate.totalFilesChanged})`,
		);
		check(
			summary.aggregate.totalTurns === 14,
			`Σturns = 14 (got ${summary.aggregate.totalTurns})`,
		);
	...
	// ─── R1: single escalation digest only when something escalated ──
	{
		...
		check(
			renderEscalationDigest(null) === "no escalations",
			"null digest renders honestly",
		);
		...
		const summary = buildBuildSummary(fixtures);
		check(
			summary.escalationDigest !== null &&
				summary.escalationDigest.includes("n-a"),
	...
	// ─── R2: retry wiring visible, flat fields, no double-counting ──
	{
		...
		check(
			s.aggregate.totalInputTokens === 800,
			`retry cost included once (Σinput 800, got ${s.aggregate.totalInputTokens})`,
		);
		check(
			Math.abs(s.aggregate.totalCostUsd - 0.16) < 1e-12,
			"retry cost summed, not duplicated",
		);
		...
		check(
			nodeSummaryFromReceipt(retry).inputTokens === 300,
			"cap semantics do not alter token counters",
		);

		// Aggregate-from-nodes agrees with buildBuildSummary on same inputs.
		const viaNodes = aggregateFromNodes(
			buildBuildSummary([firstAttempt, retry]).nodes,
		);
		check(
			JSON.stringify(viaNodes) === JSON.stringify(s.aggregate),
			"aggregateFromNodes matches end-to-end path",
		);
	}
	...
	// ─── Empty input: zeroed aggregate, null digest ──────────────────
	{
		...
		check(
			empty.nodes.length === 0 && empty.aggregate.nodeCount === 0,
			"empty receipts → zeroed summary",
		);
		check(
			empty.aggregate.totalCostUsd === 0 && empty.aggregate.cor === 0,
			"empty aggregate zeroes cost/cor",
		);
	...
	// ─── R3: determinism — order-independent, byte-stable bytes ─────
	{
		...
		const shuffled = buildBuildSummary([
			fixtures[2]!,
			fixtures[0]!,
			fixtures[1]!,
		]);
		check(
			serializeBuildSummary(forward) === serializeBuildSummary(shuffled),
		...
		check(
			serializeBuildSummary(again) === serializeBuildSummary(forward),
			"repeated aggregation is byte-identical",
		);
		check(
			!serializeBuildSummary(forward).match(/\d{4}-\d\d-\d\d|T\d\d:/),
			"serialization carries no timestamps by construction",
		);

		// Different receipts DO change the bytes (not a constant function).
		const changed = buildBuildSummary([
			...fixtures,
			receipt({ taskId: "n-d", verdict: "ship" }),
		]);
		check(
			serializeBuildSummary(changed) !== serializeBuildSummary(forward),
	...
	// ─── R3: file-backed retrieval seam without re-running the build ──
	{
		...
		try {
			...
			check(
				raw === serializeBuildSummary(summary) + "\n",
			...
			check(
				serializeBuildSummary(retrieved) === serializeBuildSummary(summary),
				"retrieved summary equals built summary",
			);
			check(
				retrieved.escalationDigest === summary.escalationDigest,
				"digest survives the round-trip",
			);
			check(
				retrieved.aggregate.verdictCounts.escalate === 1,
				"verdict counts survive the round-trip",
			);

			// Digest rendering off a RETRIEVED summary is deterministic too.
			check(
				renderEscalationDigest(retrieved.escalationDigest) ===
			...
			check(
				readFileSync(path, "utf-8") === raw,
				"rewriting the same receipts yields identical file bytes",
			);
		} finally {
	...
	// ─── cacheWriteTokens participates in totals when present ───────
	{
		...
		check(
			agg.totalCacheWriteTokens === 70,
			`ΣcacheWrite counts present values (got ${agg.totalCacheWriteTokens})`,
		);
		...
		check(
			Math.abs(agg.cor - 0.5) < 1e-12,
			`cor recomputes grounding-weighted, not averaged (got ${agg.cor})`,
		);
	}

	if (errors.length > 0) {
		throw new Error(
			`test-workflow-receipts failed:\n  ✗ ${errors.join("\n  ✗ ")}`,
		);
	}
	console.log(
		"✓ workflow-receipts: per-node usage/COR, aggregate sums, escalation digest, deterministic retrieval",
	);
