...
export async function runTests(): Promise<void> {
	...
	const root = join(tmpdir(), "pi-task-v2-artifacts-" + process.pid + "-" + Date.now());
	try {
		// ─── Happy path: exact shape + atomic write ──────────────────
		{
			const artifactsDir = join(root, "nested", "artifacts"); // created on demand
			const event = { type: "toolStart", toolName: "bash", toolCallId: "c1" } as const;
			...
			check(path === join(artifactsDir, "run-1.failure.json"), `path is <dir>/<runId>.failure.json, got ${path}`);

			const parsed = JSON.parse(readFileSync(path!, "utf-8")) as Record<string, unknown>;
			check(parsed.cause === "wall_timeout", "cause round-trips");
			check(typeof parsed.lastEvent === "string" && parsed.lastEvent.includes("bash"),
				"lastEvent serialized from the host event");
			check(parsed.lastTool === "bash — npx tsc --noEmit", "lastTool round-trips");
		...
		// ─── Caps: tail kept, head dropped ───────────────────────────
		{
			...
			const parsed = JSON.parse(readFileSync(path!, "utf-8")) as Record<string, string | undefined>;
			check(parsed.cause?.length === FAILURE_CAUSE_MAX_CHARS,
				`cause capped to ${FAILURE_CAUSE_MAX_CHARS}, got ${parsed.cause?.length}`);
			check(parsed.lastEvent!.length === FAILURE_LAST_EVENT_MAX_CHARS,
				`lastEvent capped to ${FAILURE_LAST_EVENT_MAX_CHARS}, got ${parsed.lastEvent!.length}`);
			check(parsed.lastTool!.length === FAILURE_LAST_TOOL_MAX_CHARS,
				`lastTool capped to ${FAILURE_LAST_TOOL_MAX_CHARS}, got ${parsed.lastTool!.length}`);
			check(parsed.stderrTail!.length === FAILURE_STDERR_TAIL_CHARS,
				`stderrTail capped to ${FAILURE_STDERR_TAIL_CHARS}, got ${parsed.stderrTail!.length}`);

			// Tails keep the END of the string.
			check(capTail("abcdef", 3) === "def", "capTail keeps the tail characters");

			// Object events cap too.
			const bigEvent = { type: "error", message: "m".repeat(FAILURE_LAST_EVENT_MAX_CHARS * 5), code: "prompt_failed" } as const;
			...
			const cappedParsed = JSON.parse(readFileSync(cappedPath!, "utf-8")) as Record<string, string | undefined>;
			check(cappedParsed.lastEvent!.length === FAILURE_LAST_EVENT_MAX_CHARS,
		...
		// ─── Optional fields omitted when absent ─────────────────────
		{
			...
			const path = writeFailureArtifact({ artifactsDir, runId: "run-3", cause: "settled_without_yield" });
			const parsed = JSON.parse(readFileSync(path!, "utf-8")) as Record<string, unknown>;
			check(Object.keys(parsed).join(",") === "cause",
				`only cause present when no diagnostics supplied, got ${Object.keys(parsed)}`);
		}
		...
		// ─── Never throws: unwritable dir → stderr + undefined ───────
		{
			...
			check(result === undefined, "write failure returns undefined instead of throwing");
			check(reported.includes("failed to write failure artifact"),
				`write failure reported to stderr, got: ${reported}`);
			check(!existsSync(blockerPath + ".tmp"), "temp file cleaned up on failure");
		}

		// ─── Long runId is bounded in the filename ───────────────────
		{
			const artifactsDir = join(root, "runid");
			const path = writeFailureArtifact({ artifactsDir, runId: "r".repeat(500), cause: "x" });
			const fileName = path!.split(/[\\/]/).pop()!;
			check(fileName.length <= 128 + ".failure.json".length,
	...
	console.log("✓ guards/artifacts: shape, per-field caps, optional fields, never-throw, runId bound");
}
