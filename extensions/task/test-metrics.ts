...
export function runTests(): Promise<void> {
	...
	check(
		hashSpec("abc") !== hashSpec("abd"),
		"different specs hash differently",
	);
	...
	// ─── generateRunId ───
	{
		...
		check(
			id.startsWith("20260802T1730-"),
			`run id timestamp prefix from injected now, got ${id}`,
		);
		check(
			generateRunId(now) !== id,
			"random suffix makes ids unique even for the same timestamp",
		);
	}
	...
	// ─── countByPriority ───
	{
		...
		check(
			Object.keys(countByPriority([])).length === 0,
			"empty findings → empty counts",
		);
	}
	...
	// ─── buildRunManifest: defaults + pass-through ───
	{
		...
		check(
			m.task.spec_hash === hashSpec(baseInput().specMarkdown),
			"spec_hash = hashSpec(spec)",
		);
		check(m.task.requirements === 1, "task.requirements passed through");
		check(
			m.config.budget === "default",
			`budget defaults to 'default', got ${m.config.budget}`,
		);
		check(
			m.config.swap_trigger === "first-edit",
			`swap_trigger defaults to 'first-edit', got ${m.config.swap_trigger}`,
		);
		check(m.config.checklist === true, "checklist defaults to true");
		check(m.config.review_forked === false, "review_forked passed through");
		check(
			m.config.sandbox === false,
			"sandbox defaults to false when a direct caller omits it",
		);
		check(
			m.config.prewalk_model === "pre/m" &&
				m.config.execute_model === "exe/m" &&
				m.config.review_model === "rev/m",
			"models passed through",
		);
		check(
			m.phases.execute.cost_usd === 0.02 &&
				m.phases.prewalk === null &&
				m.phases.review === null,
			"phases passed through",
		);
		check(
			m.totals.cost_usd === 0.02,
			`totals.cost_usd = execute only (0.02), got ${m.totals.cost_usd}`,
		);
		check(m.totals.duration_ms === 1000, "totals.duration_ms passed through");
		check(
			m.totals.read_duplication_tokens === 0,
			"read_duplication_tokens passed through",
		);
		check(
			Array.isArray(m.totals.session_files) &&
				m.totals.session_files.length === 0,
			"session_files defaults to []",
		);
		// R1/R2: new fields are ABSENT/ZERO when not supplied (backward compatible).
		check(
			m.received_at === undefined &&
				m.dispatched_at === undefined &&
				m.completed_at === undefined,
			"run-lifecycle timestamps absent when not supplied",
		);
		check(
			m.main_session_tokens === 0,
			`main_session_tokens defaults to 0, got ${m.main_session_tokens}`,
		);
		check(
			JSON.stringify(m.totals.files_changed) === "[]",
			`files_changed defaults to [], got ${JSON.stringify(m.totals.files_changed)}`,
		);
		check(
			m.totals.insertions === 0 && m.totals.deletions === 0,
			"insertions/deletions default to 0",
		);
		...
		check(
			!("received_at" in serialized) &&
				!("dispatched_at" in serialized) &&
				!("completed_at" in serialized),
	...
	// ─── buildRunManifest: cost summation across all phases + overrides ───
	{
		...
		check(
			Math.abs(m.totals.cost_usd - 0.1) < 1e-9,
			`totals.cost_usd sums all phases (0.10), got ${m.totals.cost_usd}`,
		);
		check(
			m.config.budget === "full" &&
				m.config.swap_trigger === "custom" &&
				m.config.checklist === false,
		...
		check(
			m.phases.review?.by_priority.P0 === 1 &&
				m.phases.review?.by_priority.P1 === 1 &&
				m.phases.review?.by_priority.P2 === 1,
			"review by_priority preserved",
		);
		check(
			m.phases.fix_loop.iterations === 2 && m.phases.fix_loop.cost_usd === 0.04,
			"fix_loop preserved",
		);
		check(
			m.totals.read_duplication_tokens === 1234,
			"read_duplication_tokens preserved",
		);
		check(
			m.totals.session_files.length === 1 &&
				m.totals.session_files[0] === "/tmp/s/worker-1.jsonl",
	...
	// ─── buildRunManifest: R1 run-lifecycle fields pass through ───
	{
		...
		check(
			m.received_at === "2026-08-05T00:00:00.000Z" &&
				m.dispatched_at === "2026-08-05T00:00:05.000Z" &&
				m.completed_at === "2026-08-05T01:30:00.000Z",
			"run-lifecycle timestamps pass through",
		);
		check(
			m.main_session_tokens === 12345,
			`main_session_tokens pass through, got ${m.main_session_tokens}`,
		);
		check(
			JSON.stringify(m.totals.files_changed) ===
				JSON.stringify(["a.ts", "b/c.ts"]),
			"files_changed pass through",
		);
		check(
			m.totals.insertions === 42 && m.totals.deletions === 7,
			"insertions/deletions pass through",
		);
	}
	...
	// ─── runLatencyMs (R4): wall-clock latency, worker-duration fallback ───
	{
		...
		check(
			runLatencyMs(withTimestamps) === 90_000,
			`latency from timestamps (90s), got ${runLatencyMs(withTimestamps)}`,
		);
		check(
			runLatencyMs(mk({})) === 1000,
			"no timestamps → totals.duration_ms fallback",
		);
		const onlyReceived = mk({ received_at: "2026-08-05T00:00:00.000Z" });
		check(
			runLatencyMs(onlyReceived) === 1000,
			"received_at only → fallback (completed_at missing)",
		);
		const inverted = mk({
		...
		check(
			runLatencyMs(inverted) === 1000,
			"completed < received (inverted) → fallback",
		);
		const unparseable = mk({
			received_at: "not-a-date",
			completed_at: "also-not",
		});
		check(
			runLatencyMs(unparseable) === 1000,
			"unparseable timestamps → fallback",
		);
	}
	...
	// ─── splitPhases ───
	{
		...
		check(
			noSwap.execute.model === "exe/m" &&
				noSwap.execute.turns === 2 &&
				noSwap.execute.tokens_in === 200,
		...
		check(
			split.prewalk !== null && split.prewalk.model === "pre/m",
			"prewalk model",
		);
		check(
			split.prewalk?.turns === 2 && split.prewalk.tokens_in === 220,
			`prewalk turns/tokens, got ${split.prewalk?.turns}/${split.prewalk?.tokens_in}`,
		);
		check(
			split.prewalk?.reads === 1,
			`prewalk reads (turn <= 2), got ${split.prewalk?.reads}`,
		);
		check(
			split.execute.model === "exe/m" &&
				split.execute.turns === 2 &&
				split.execute.tokens_in === 180,
			`execute turns/tokens (400-220), got ${split.execute.turns}/${split.execute.tokens_in}`,
		);
		check(
			split.execute.reads === 2,
			`execute reads (turn > 2), got ${split.execute.reads}`,
		);
		check(
			split.prewalk?.duration_ms === 500 && split.execute.duration_ms === 500,
			"durations split proportionally (2/4)",
		);
		check(
			Math.abs((split.prewalk?.cost_usd ?? 0) - 0.02) < 1e-9,
			"prewalk cost from snapshot",
		);
		check(
			Math.abs(split.execute.cost_usd - 0.02) < 1e-9,
			"execute cost = diff (0.04-0.02)",
		);
		...
		check(
			late.prewalk?.turns === 1 && late.prewalk.edits === 1,
			"swap on last turn → prewalk has the turn + edits",
		);
		check(
			late.execute.turns === 0 &&
				late.execute.tokens_in === 0 &&
				late.execute.edits === 0,
			"execute zeroed",
		);

		// No turns recorded → execute zeroed, prewalk null
		const empty = splitPhases({
			turnUsage: [],
			reads: [],
			swapTurn: null,
			prewalkModel: "p",
		...
		check(
			empty.prewalk === null && empty.execute.turns === 0,
			"no turns → zeroed execute",
		);
	}
	...
	// ─── computeReadDuplication ───
	{
		...
		check(
			dup.tokens === 180,
			`duplication tokens = execute re-reads of shared.ts (100+80), got ${dup.tokens}`,
		);
		check(
			JSON.stringify(dup.files) === JSON.stringify(["shared.ts"]),
			`duplicated files, got ${JSON.stringify(dup.files)}`,
		);
	}
	{
		const none = computeReadDuplication(
			[{ path: "a.ts", approxTokens: 10, turn: 1 }],
			null,
		);
		check(
			none.tokens === 0 && none.files.length === 0,
			"no swap → no duplication",
		);
		const zero = computeReadDuplication([], 2);
		check(
			zero.tokens === 0 && zero.files.length === 0,
			"no reads → no duplication",
		);
	}
	...
	// ─── contextInheritedTokens ───
	{
		...
		check(
			contextInheritedTokens([usage(1, 500)]) === 500,
			"single turn → its input tokens",
		);
		check(
			contextInheritedTokens([usage(1, 500), usage(2, 900)]) === 400,
			"final-turn input delta (900-500)",
		);
	}
	...
	// ─── storage (real fs on a temp dir) ───
	{
		...
		try {
			...
			check(
				path.endsWith(join("proj", "run-abc.json")),
				`manifest path, got ${path}`,
			);
			check(existsSync(path), "manifest file exists");
			const parsed = JSON.parse(readFileSync(path, "utf-8")) as RunManifest;
			check(
				parsed.run_id === "run-abc" && parsed.task.requirements === 1,
				"written manifest round-trips",
			);
	...
	}
	{
		...
		try {
			...
			check(
				saved.length === 1 &&
					saved[0]!.endsWith(join("proj", "run-abc", "worker-1.jsonl")),
				`saved trace path, got ${JSON.stringify(saved)}`,
			);
			check(
				existsSync(saved[0]!) &&
					readFileSync(saved[0]!, "utf-8") === "{} \n{}\n",
				"trace copied with content",
			);
			const empty = copySessionTraces({
				metricsDir,
				project: "proj",
				runId: "run-abc",
				sources: [],
	...
	{
		check(
			deriveProjectName("/tmp/myproj") === "myproj",
			"project from basename",
		);
	...
	// ─── aggregateExecutePhase (R6, parallel aggregate approximation) ───
	{
		...
		check(
			agg.tokens_in === 300 && agg.tokens_out === 120,
			`tokens summed, got ${agg.tokens_in}/${agg.tokens_out}`,
		);
		check(
			Math.abs(agg.cost_usd - 0.03) < 1e-9,
			`cost summed (0.01+0.02), got ${agg.cost_usd}`,
		);
		check(
			agg.reads === 4 && agg.edits === 6,
			`reads/edits summed, got ${agg.reads}/${agg.edits}`,
		);
		check(
			agg.duration_ms === 500,
			"duration_ms = parallel phase WALL time (not worker durations summed)",
		);
		...
		check(
			empty.turns === 0 && empty.tokens_in === 0 && empty.tokens_out === 0,
			"empty workers → zeroed tokens/turns",
		);
		check(
			empty.cost_usd === 0 && empty.reads === 0 && empty.edits === 0,
			"empty workers → zeroed cost/reads/edits",
		);
		check(
			empty.duration_ms === 0 && empty.model === "exe/m",
			"empty workers → duration 0, model kept",
		);
	...
	// buildFailureArtifact: run_id + timestamp + the failure fields.
	{
		...
		check(
			artifact.kind === "worker" &&
				artifact.cause === "no progress" &&
				artifact.turns === 47,
			"artifact carries the failure fields",
		);
		check(
			artifact.run_id.startsWith("20260805T0000"),
			`run_id from now, got ${artifact.run_id}`,
		);
		check(artifact.timestamp === "2026-08-05T00:00:00.000Z", "timestamp ISO");
		check(
			artifact.last_tool?.name === "bash" && artifact.stderr_tail === "err",
			"last tool + stderr tail land",
		);
	}
	...
	{
		const now = new Date("2026-08-05T00:00:00.000Z");
		const injected = buildFailureArtifact({
			kind: "worker",
			now,
			runId: "20260805T0000-feed",
			cause: "x",
		});
		check(
			injected.run_id === "20260805T0000-feed",
			`injected run_id wins, got ${injected.run_id}`,
		);
		const generated = buildFailureArtifact({ kind: "worker", now, cause: "x" });
		check(
			generated.run_id !== "20260805T0000-feed" &&
				generated.run_id.startsWith("20260805T0000"),
	...
	// writeFailureArtifact: atomic write + round-trip on a temp dir.
	{
		...
		try {
			...
			const path = writeFailureArtifact(artifact, {
				metricsDir,
				project: "proj",
			});
			check(
				path.endsWith(join("proj", `${artifact.run_id}.failure.json`)),
				`artifact path, got ${path}`,
			);
			check(existsSync(path), "artifact file exists");
			const parsed = JSON.parse(readFileSync(path, "utf-8")) as FailureArtifact;
			check(
				parsed.kind === "review" && parsed.cause === "timed out",
		...
		try {
			...
			write(
				...
				manifest({
					...
					config: { budget: "full" },
					totals: { cost_usd: 0.02, duration_ms: 60000 },
				}),
			);
			write(
				"alpha",
			...
			writeFileSync(
				join(metricsDir, "alpha", "20260805T0003-abcd.failure.json"),
				"{}",
				"utf-8",
			);
			writeFileSync(
				join(metricsDir, "alpha", "garbage.json"),
				"{not json",
				"utf-8",
			);
			...
			writeFileSync(
				join(metricsDir, "alpha", "20260805T0005-abcd.request.json"),
				"{}",
				"utf-8",
			);
			writeFileSync(
				join(metricsDir, "alpha", "20260805T0005-abcd.live.json"),
				"{}",
				"utf-8",
			);
			writeFileSync(
				join(metricsDir, "alpha", "20260805T0005-abcd.log"),
				"log\n",
				"utf-8",
			);
			...
			write(
				...
				manifest({
					...
					totals: { cost_usd: 0.01, duration_ms: 30000 },
				}),
			);
			...
			check(
				Math.abs(s.totalCostUsd - 0.06) < 1e-9,
				`total cost, got ${s.totalCostUsd}`,
			);
			check(s.p50DurationMs === 60000, `p50, got ${s.p50DurationMs}`);
			check(s.p90DurationMs === 120000, `p90, got ${s.p90DurationMs}`);
			check(
				s.byTier.full?.count === 2 &&
					Math.abs(s.byTier.full.costUsd - 0.05) < 1e-9,
				"byTier full rollup",
			);
			check(s.byTier.economy?.count === 1, "byTier economy rollup");
			check(
				s.byProject.alpha?.count === 2 && s.byProject.beta?.count === 1,
				"byProject rollup",
			);
			check(
				s.rows[0]!.runId === "20260805T0001-abcd" &&
					s.rows[2]!.runId === "20260805T0004-abcd",
				"rows sorted by run id",
			);

			const filtered = summarizeRuns(metricsDir, "alpha");
			check(
				filtered.count === 2 &&
					filtered.byProject.alpha?.count === 2 &&
					!("beta" in filtered.byProject),
			...
			check(
				lines[0]!.includes("3 total") &&
					lines[0]!.includes("2/3 verified") &&
					lines[0]!.includes("1 aborted"),
				`headline: ${lines[0]}`,
			);
			check(
				joined.includes("by tier: full 2") &&
					joined.includes("by project: alpha 2"),
				"rollup lines",
			);
			check(
				joined.includes("✓") && joined.includes("✗"),
				"pass/fail glyphs in recent rows",
			);
			check(joined.includes("1 unreadable manifest"), "unreadable note");

			const empty = renderTaskStats(
				summarizeRuns(join(metricsDir, "nonexistent")),
			);
			check(
				empty.some((l) => l.includes("no task runs recorded yet")),
	...
	// totals.duration_ms.
	{
		...
		try {
			...
			const s = summarizeRuns(metricsDir);
			check(
				s.rows[0]!.durationMs === 150_000,
				`latency from timestamps (150s), got ${s.rows[0]!.durationMs}`,
			);
			check(
				s.rows[1]!.durationMs === 45_000,
				`fallback to totals.duration_ms (45s), got ${s.rows[1]!.durationMs}`,
			);
			// p50/p90 (sorted [45000, 150000]): both come from the latency values.
			check(
				s.p50DurationMs === 45_000,
				`p50 uses latency/fallback, got ${s.p50DurationMs}`,
			);
			check(
				s.p90DurationMs === 150_000,
				`p90 uses latency, got ${s.p90DurationMs}`,
			);
			const lines = renderTaskStats(s);
			check(
				lines[0]!.includes("p50 45s · p90 2m30s"),
				`headline uses latency: ${lines[0]}`,
			);
		} finally {
			rmSync(metricsDir, { recursive: true, force: true });
		}
	...
	// formatDuration: compact human durations.
	{
		...
		check(
			formatDuration(432000) === "7m12s",
			`7m12s, got ${formatDuration(432000)}`,
		);
		check(
			formatDuration(3723000) === "1h2m",
			`1h2m, got ${formatDuration(3723000)}`,
		);
	...
	if (errors.length > 0) {
		return Promise.reject(
			new Error("test-metrics failed:\n  ✗ " + errors.join("\n  ✗ ")),
		);
	}

	// ─── recentCompletions: derived from manifests + failure artifacts (M3) ───
	function testRecentCompletions(errors: string[]): void {
		const check = (cond: boolean, msg: string): void => {
			if (!cond) errors.push(msg);
		};
		...
		try {
			...
			check(
				recent.length === 2,
				`derived 2 completions (sidecar skips), got ${recent.length} = ${recent.map((c) => c.runId).join(",")}`,
			);
			check(
				recent.every((c) => !/\.(request|live|batch)($|\.)/.test(c.runId)),
				"no sidecar / job-state file surfaces as a completion (P1)",
			);
			check(
				recent[0]!.status === "failed" &&
					recent[0]!.project === "demo" &&
					recent[0]!.runId === "run2",
				"failure artifact surfaces as a failed completion",
			);
			const done = recent.find((c) => c.runId === "run1");
			check(
				done?.status === "completed" && done?.channel === "flex",
				`manifest channel surfaces, got ${JSON.stringify(done)}`,
			);
			// limit + empty
			check(recentCompletions(metricsDir, 1).length === 1, "limit respected");
			check(
				recentCompletions(join(metricsDir, "nope")).length === 0,
				"missing dir → empty",
			);
		} finally {
			rmSync(metricsDir, { recursive: true, force: true });
		...
		console.log(
			"✓ recentCompletions: manifests + failures derived, channel surfaced (M3)",
		);
	}
	testRecentCompletions(errors);
	console.log(
		"✓ metrics: hashSpec, generateRunId, countByPriority, buildRunManifest, phase split, aggregation, duplication, storage",
	);
