...
export async function runTests(): Promise<void> {
	...
	console.log("── test-runner: detached dispatch — request assembly, spawn command, status resolution + rendering ──");

	// ─── path helpers ───
	check(requestPathFor("/m", "p", "r1") === join("/m", "p", "r1.request.json"), "request path suffix");
	check(livePathFor("/m", "p", "r1") === join("/m", "p", "r1.live.json"), "live path suffix");

	// ─── buildRunRequest: field mapping + JSON round-trip ───
	{
		const req = requestFixture();
		check(req.run_id === "20260805T0000-abcd", "run_id lands");
		check(req.created_at === "2026-08-05T00:00:00.000Z", "created_at from injectable now");
		check(req.metrics_dir === "/tmp/m" && req.project === "proj", "metrics dir + project land");
		check(req.worker_count === 2, "worker count lands");
		check(req.plan === PLAN && req.options.spec?.includes("## Goal"), "plan + options land");
	...
	// ─── writeRunRequest: durable file, atomic (no .tmp left) ───
	{
		...
		try {
			...
			check(path === requestPathFor(dir, "alpha", req.run_id), "request written at the canonical path");
			check(existsSync(path), "request file exists");
			const parsed = JSON.parse(requireRead(path)) as RunRequest;
			check(parsed.run_id === req.run_id && parsed.options.model === "p/work", "request round-trips from disk");
	...
	// ─── buildRunnerCommand / resolveRunnerSpawn: child invocation ───
	{
		...
		check(
			local.command === "/repo/node_modules/.bin/tsx" &&
				local.args.join(" ") === "/repo/extensions/task/runner.ts --request /m/p/r1.request.json",
		...
		check(
			npx.command === "npx" &&
				npx.args.join(" ") === "tsx /repo/extensions/task/runner.ts --request /m/p/r1.request.json",
		...
		try {
			...
			check(withLocal.command === join(binDir, "tsx"), `local tsx preferred, got ${withLocal.command}`);
	...
	// ─── parseRunnerArgs ───
	{
		...
		check(ok.requestPath === "/m/p/r1.request.json" && !ok.help, "request path parsed");
	...
	// ─── locateRun: run_id → files, across projects ───
	{
		...
		try {
			...
			check(
				byManifest?.project === "alpha" && byManifest.manifestPath === join(dir, "alpha", "20260805T0001-abcd.json"),
				"manifest located",
			);

			mk("beta", "20260805T0002-abcd.live.json", "{}");
			const byLive = locateRun(dir, "20260805T0002-abcd");
			check(byLive?.project === "beta" && byLive.livePath !== undefined, "live file located");

			mk("gamma", "20260805T0003-abcd.failure.json", "{}");
			const byFailure = locateRun(dir, "20260805T0003-abcd");
			check(byFailure?.project === "gamma" && byFailure.failurePath !== undefined, "failure artifact located");

			mk("delta", "20260805T0004-abcd.request.json", "{}");
			const byRequest = locateRun(dir, "20260805T0004-abcd");
			check(byRequest?.project === "delta" && byRequest.requestPath !== undefined, "request located");
			...
			check(winner?.project === "alpha" && winner.manifestPath !== undefined, "manifest project wins over sidecar-only");
		} finally {
			rmSync(dir, { recursive: true, force: true });
		}
	}
	...
	// ─── readRunStatus: precedence manifest > failure > live > starting > unknown ───
	{
		...
		try {
			...
			const manifest: RunManifest = {
				...
				config: { budget: "economy", prewalk_model: "p", execute_model: "p", review_model: "p", swap_trigger: "edit", checklist: true, review_forked: false, shape: "code", sandbox: false },
				task: { spec_hash: "abc", requirements: 2 },
				phases: {
					prewalk: null,
					execute: { model: "p", turns: 5, tokens_in: 100, tokens_out: 50, reads: 3, edits: 2, duration_ms: 10_000, cost_usd: 0.002 },
					verify: { passed: true, commands: 2, duration_ms: 50, source: "worker-tree" },
					review: null,
					fix_loop: { iterations: 0, cost_usd: 0 },
				},
				totals: { cost_usd: 0.002, duration_ms: 12_000, read_duplication_tokens: 0, session_files: [], files_changed: ["a.txt"], insertions: 3, deletions: 1 },
			...
			if (finished.kind === "finished") {
				check(finished.project === "alpha" && finished.manifest.totals.cost_usd === 0.002, "finished carries the manifest");
			}

			mk("beta", "20260805T0002-abcd.failure.json", JSON.stringify({ run_id: "20260805T0002-abcd", kind: "worker", timestamp: "2026-08-05T00:01:00.000Z", cause: "no progress" }));
			...
			if (failed.kind === "failed") check(failed.artifact.cause === "no progress", "failed carries the artifact");
			...
			const live: LiveRunState = {
				...
				plan: PLAN,
				progress_text: "plan(economy): prewalk(p/prewalk) → work(p/work) · goals: make the build green\n1/2 workers done",
			};
			...
			if (liveStatus.kind === "live") check(liveStatus.staleMs === 30_000, `staleMs from heartbeat, got ${liveStatus.staleMs}`);

			// Request-only: spawned but not yet heartbeating.
			const req = requestFixture({ run_id: "20260805T0004-abcd", metrics_dir: dir, project: "delta" });
			writeRunRequest(req);
			const starting = readRunStatus(dir, "20260805T0004-abcd", now);
			check(starting.kind === "starting", "bare request → starting");
			if (starting.kind === "starting") check(starting.request.worker_count === 2, "starting carries the request");
	...
	// ─── renderRunStatus ───
	{
		...
		const startingLines = renderRunStatus(
			{ kind: "starting", runId: "r1", project: "alpha", metricsDir: "/tmp/m", request: requestFixture() },
			now,
		);
		check(
			startingLines.some((l) => l.includes("STARTING")) && startingLines.some((l) => l.includes("first heartbeat")),
		...
		const liveLines = renderRunStatus(
			{ kind: "live", runId: "r1", project: "alpha", metricsDir: "/tmp/m", state: live, staleMs: 30_000 },
			now,
		);
		const liveJoined = liveLines.join("\n");
		check(liveLines[0].includes("RUNNING") && liveLines[0].includes("elapsed 5m"), `live headline: ${liveLines[0]}`);
		check(liveJoined.includes("prewalk(p/prewalk) → work(p/work)"), "live shows the phase chain");
		check(liveJoined.includes("goals: make the build green"), "live shows goals");
		check(liveJoined.includes("1/2 workers done"), "live shows progress text");
		check(!liveJoined.includes("no heartbeat"), "fresh heartbeat → no stale warning");

		const staleLines = renderRunStatus(
			{ kind: "live", runId: "r1", project: "alpha", metricsDir: "/tmp/m", state: live, staleMs: LIVE_STALE_THRESHOLD_MS + 5_000 },
			now,
		);
		check(
			staleLines.some((l) => l.includes("no heartbeat") && l.includes("r1.log")),
		...
		const manifest: RunManifest = {
			run_id: "r1",
			config: { budget: "economy", prewalk_model: "p", execute_model: "p", review_model: "p", swap_trigger: "edit", checklist: true, review_forked: true, shape: "code", sandbox: false },
			task: { spec_hash: "abc", requirements: 2 },
			phases: {
				prewalk: null,
				execute: { model: "p", turns: 5, tokens_in: 100, tokens_out: 50, reads: 3, edits: 2, duration_ms: 10_000, cost_usd: 0.002 },
				verify: { passed: true, commands: 3, duration_ms: 50, source: "worker-tree" },
				review: { model: "p", forked: true, context_inherited_tokens: 10, findings: 2, by_priority: { P1: 2 }, cost_usd: 0.001, personas: ["standards"] },
				fix_loop: { iterations: 0, cost_usd: 0 },
			},
			totals: { cost_usd: 0.003, duration_ms: 42_000, read_duplication_tokens: 0, session_files: [] },
		};
		const doneLines = renderRunStatus(
			{ kind: "finished", runId: "r1", project: "alpha", metricsDir: "/tmp/m", manifest },
			now,
		);
		const doneJoined = doneLines.join("\n");
		check(doneLines[0].includes("DONE ✓"), `finished headline: ${doneLines[0]}`);
		check(doneJoined.includes("verify: ✓ 3 command(s)"), "finished shows verify result");
		check(doneJoined.includes("$0.003") && doneJoined.includes("42s") && doneJoined.includes("tier economy"), "finished shows cost + duration + tier");
		check(doneJoined.includes("review: 2 finding(s) (P1: 2)"), "finished shows findings");
		...
		const failLines = renderRunStatus(
			{
				...
				project: "alpha",
				metricsDir: "/tmp/m",
				artifact: { run_id: "r1", kind: "worker", timestamp: "2026-08-05T00:01:00.000Z", cause: "no progress", tier: "economy" },
			},
			now,
		);
		const failJoined = failLines.join("\n");
		check(failLines[0].includes("FAILED (worker)"), `failed headline: ${failLines[0]}`);
		check(failJoined.includes("cause: no progress"), "failed shows cause");
		check(failJoined.includes("2026-08-05T00:01:00.000Z"), "failed shows timestamp");
	}
	...
	// ─── removeFileBestEffort: live file gone, missing file tolerated ───
	{
		...
		try {
			...
			writeLiveState({ run_id: "r1", project: "p", started_at: "x", heartbeat_at: "y", plan: PLAN, progress_text: "" }, path);
