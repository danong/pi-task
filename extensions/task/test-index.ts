...
import type { WorkerResult, WorkerUsage } from "./worker.ts";
import {
...
function schemaProperties(schema: ReturnType<typeof taskToolSchema>): string[] {
	const json = JSON.parse(JSON.stringify(schema)) as {
...
function tierConfig(
	overrides: Partial<BudgetTierConfig> = {},
): BudgetTierConfig {
	return {
		prewalkModel: null,
		executeModel: "prov/m",
...
function propertyJson(
	...
	const json = JSON.parse(
		JSON.stringify(
			(schema as unknown as { properties: Record<string, unknown> }).properties[
				name
			],
		),
...
function testBudgetSchemaLocking(errors: string[]): void {
	...
	check(
		unlocked.includes("sub_specs"),
		"unlocked schema should have sub_specs",
	);
	check(unlocked.includes("parallel"), "unlocked schema should have parallel");
	check(
		unlocked.includes("review"),
		"unlocked schema should have the review axis override",
	);
	check(
		unlocked.includes("shape"),
		"unlocked schema should have the shape param",
	);
	check(
		unlocked.includes("detach"),
		"unlocked schema should have the detach param",
	);
	check(unlocked.includes("budget"), "unlocked schema should expose budget");

	check(
		locked.includes("spec") &&
			locked.includes("sub_specs") &&
			locked.includes("parallel") &&
			locked.includes("review") &&
			locked.includes("shape"),
		"locked schema keeps the work parameters + review/shape overrides",
	);
	check(
		locked.includes("detach"),
		"locked schema keeps the detach param (it is not a budget override)",
	);
	check(
		!locked.includes("budget"),
		"locked schema must NOT expose budget (model cannot see or override it)",
	);
	...
	check(
		Array.isArray(budgetSchema.enum) &&
			budgetSchema.enum.length === BUDGET_TIERS.length + 1,
		`budget enum should have auto + the built-in tiers, got ${JSON.stringify(budgetSchema.enum)}`,
	);
	check(
		JSON.stringify(budgetSchema.enum) ===
			JSON.stringify(["auto", ...BUDGET_TIERS]),
	...
	const dynTiers: Record<string, BudgetTierConfig> = {
		fast: tierConfig(),
		turbo: tierConfig(),
	};
	const dynSchema = propertyJson(taskToolSchema(false, dynTiers), "budget");
	check(
		JSON.stringify(dynSchema.enum) ===
			JSON.stringify(["auto", "fast", "turbo"]),
		`dynamic enum should be auto + tiers in file order, got ${JSON.stringify(dynSchema.enum)}`,
	);
	check(
		!(dynSchema.enum as unknown[]).includes("full"),
		"a tier not in the loaded set must not appear in the enum",
	);

	// The schema-locking behavior is unchanged over a dynamic set: a locked
	// tier removes the budget param entirely.
	check(
		!schemaProperties(taskToolSchema(true, dynTiers)).includes("budget"),
		"locked schema removes budget even with a dynamic tier set",
	);
	check(
		schemaProperties(taskToolSchema(true, dynTiers)).includes("detach"),
		"detach survives schema locking over a dynamic tier set",
	);

	console.log(
		"✓ taskToolSchema: budget param removed when locked; enum config-driven (dynamic tiers)",
	);
}
...
function testBudgetResolution(errors: string[]): void {
	...
	check(
		isLockedBudget("full") &&
			isLockedBudget("economy") &&
			isLockedBudget("free"),
		"concrete tiers are locked",
	);
	check(
		!isLockedBudget("auto") &&
			!isLockedBudget(undefined) &&
			!isLockedBudget("bogus"),
		"auto/invalid are not locked",
	);

	check(normalizeBudgetMode("free") === "free", "normalize keeps valid modes");
	check(normalizeBudgetMode("auto") === "auto", "normalize keeps auto");
	check(
		normalizeBudgetMode("bogus") === "auto" &&
			normalizeBudgetMode(undefined) === "auto",
	...
	check(
		...
		"locked flag beats the model param",
	);
	check(
		resolveBudgetTier(
			"economy",
	...
	check(
		...
		"param honored when flag is auto",
	);
	check(
		resolveBudgetTier(
			undefined,
	...
	check(
		resolveBudgetMode("auto", undefined, "full") === "full",
		"auto flag + full default → full",
	);
	check(
		resolveBudgetTier("auto", undefined, "full", 3, DEFAULT_BUDGET_TIERS) ===
			"full",
		"auto flag + full default → full regardless of count",
	);
	check(
		resolveBudgetTier("auto", undefined, "full", 8, DEFAULT_BUDGET_TIERS) ===
			"full",
		"auto flag + full default → full for big specs too",
	);
	check(
		resolveBudgetTier("auto", undefined, "auto", 3, DEFAULT_BUDGET_TIERS) ===
			"economy",
		"config auto + 3 reqs → economy",
	);
	check(
		resolveBudgetTier("auto", undefined, "auto", 8, DEFAULT_BUDGET_TIERS) ===
			"full",
		"config auto + 8 reqs → full",
	);
	check(
		resolveBudgetTier("auto", "economy", "full", 8, DEFAULT_BUDGET_TIERS) ===
			"economy",
		"locked param beats config default",
	);
	check(
		resolveBudgetTier("free", "full", "economy", 8, DEFAULT_BUDGET_TIERS) ===
			"free",
		"locked flag beats everything",
	);
	...
	check(
		isLockedBudget("turbo", dynTiers),
		"a new tier is locked over the dynamic set",
	);
	check(
		!isLockedBudget("max", dynTiers) && !isLockedBudget("full", dynTiers),
		"built-ins NOT in the loaded set are not locked",
	);
	check(
		!isLockedBudget("auto", dynTiers) && !isLockedBudget("bogus", dynTiers),
		"auto/invalid never locked",
	);
	check(
		normalizeBudgetMode("turbo", dynTiers) === "turbo",
		"normalize keeps a loaded new tier",
	);
	check(
		normalizeBudgetMode("max", dynTiers) === "auto",
		"normalize rejects a tier outside the set",
	);
	check(
		resolveBudgetTier("turbo", undefined, undefined, null, dynTiers) ===
			"turbo",
		"locked new tier flag wins",
	);
	check(
		resolveBudgetTier(undefined, "turbo", "full", 8, dynTiers) === "turbo",
		"locked new tier param beats a default outside the set",
	);
	check(
		resolveBudgetTier(undefined, undefined, "auto", 3, dynTiers) === "fast",
	...
	check(
		resolveBudgetTier(undefined, undefined, "full", 8, dynTiers) === "fast",
		"config default outside the set → auto → heuristic default tier",
	);
	// The heuristic never auto-selects the max tier.
	const maxSet: Record<string, BudgetTierConfig> = {
		max: tierConfig(),
		ultra: tierConfig(),
	};
	check(
		autoTierForRequirements(3, maxSet) === "ultra",
		"auto never picks the max tier",
	);
	...
	check(
		resolveBudgetTier("bench-good", undefined, "bench-good", 1, loadedSet) ===
			"bench-good",
		"/task-budget lock on a config-only tier beats the auto heuristic",
	);
	check(
		resolveBudgetTier("auto", undefined, "bench-good", 1, loadedSet) ===
			"bench-good",
		"config default naming a config-only tier locks without an explicit lock",
	);

	console.log(
		"✓ resolveBudgetTier: locked flag > locked param > config default; normalize/isLocked; dynamic tier sets",
	);
}
...
function testSubSpecsPrecedence(errors: string[]): void {
	...
	const deriveParallel = (
		subSpecs: string[] | undefined,
		parallel: number | undefined,
	): number =>
		subSpecs && subSpecs.length > 0
			? subSpecs.length
			: Math.max(1, parallel ?? 1);

	check(
		deriveParallel(["s1", "s2", "s3"], 2) === 3,
		"sub_specs wins over parallel (3 > 2)",
	);
	check(
		deriveParallel(["s1"], 4) === 1,
		"sub_specs of length 1 wins over parallel 4",
	);
	...
	console.log(
		"✓ sub_specs precedence: sub_specs.length wins; empty/spec fall back",
	);
}
...
function testSubSpecNormalization(errors: string[]): void {
	...
	const json = JSON.parse(JSON.stringify(taskToolSchema(false))) as {
		required?: string[];
	};
	check(
		!(json.required ?? []).includes("spec"),
		"spec is NOT required (optional when sub_specs is given)",
	);
	...
	const objProps = anyOf?.[1]?.properties;
	check(
		Array.isArray(anyOf) && anyOf.length === 2 && anyOf[0]!.type === "string",
		"sub_specs accepts markdown strings",
	);
	check(
		anyOf?.[1]?.type === "object" &&
			objProps !== undefined &&
			["goal", "requirements", "verification", "context"].every(
				(k) => k in objProps,
			),
	...
	check(
		guardErr.includes("spec") && guardErr.includes("sub_specs"),
		`guard message names both options, got ${guardErr}`,
	);
	...
	const normalized = normalizeSubSpecs([
		md,
		{
			goal: "g2",
			requirements: ["a", "b"],
			verification: ["test -f x", "npm test"],
			context: "see docs/foo",
		},
	]);
	check(
		normalized.length === 2 && normalized[0] === md,
		"string entry passes through unchanged",
	);
	const rendered = parseSpec(normalized[1]!);
	check(rendered.goal === "g2", `object goal renders, got ${rendered.goal}`);
	check(
		JSON.stringify(rendered.requirements) ===
			JSON.stringify(["R1: a", "R2: b"]),
		"object requirements render as R1/R2",
	);
	check(
		JSON.stringify(rendered.verification) ===
			JSON.stringify(["test -f x", "npm test"]),
		"object verification renders verbatim",
	);
	check(
		normalized[1]!.includes("## Context") &&
			normalized[1]!.includes("see docs/foo"),
		"context renders as a ## Context section",
	);
	...
	check(
		objErr.includes("requirements") && objErr.includes("verification"),
		`object guard message, got ${objErr}`,
	);

	// resolveSubSpecs returns normalized strings for the orchestrator.
	const resolved = resolveSubSpecs({
		sub_specs: [{ goal: "g3", requirements: ["r"], verification: ["true"] }],
	});
	check(
		resolved.hasSubSpecs &&
			resolved.subSpecs.length === 1 &&
			resolved.spec === "",
		"resolveSubSpecs normalizes object entries",
	);
	check(
		resolveSubSpecs({ spec: "  s  " }).spec === "  s  " &&
			!resolveSubSpecs({ spec: "  s  " }).hasSubSpecs,
		"spec path passes through",
	);

	console.log(
		"✓ sub_specs normalization: objects ≡ markdown strings; spec optional with sub_specs; guards",
	);
}
...
function testAutoHeuristicAndCounting(errors: string[]): void {
	...
	check(
		autoTierForRequirements(8, { fast: tierConfig(), full: tierConfig() }) ===
			"full",
		"full present → full for large tasks",
	);
	check(
		autoTierForRequirements(3, { fast: tierConfig() }) === "fast",
		"no economy/full → the default tier (first non-max)",
	);
	check(
		autoTierForRequirements(8, { fast: tierConfig() }) === "fast",
		"large task without full → the default tier too",
	);
	check(
		autoTierForRequirements(null, { fast: tierConfig() }) === "fast",
...
`;
	check(
		countSpecRequirements(spec5) === 5,
		`5-item spec counts 5, got ${countSpecRequirements(spec5)}`,
	);
	check(
		countSpecRequirements("no sections here") === null,
		"unparseable spec → null",
	);
...
`;
	check(
		countSubSpecsRequirements([spec2, spec5]) === 7,
		`two sub-specs sum to 7, got ${countSubSpecsRequirements([spec2, spec5])}`,
	);
	check(
		countSubSpecsRequirements(["garbage"]) === null,
		"all-unparseable sub_specs → null",
	);
	check(
		countSubSpecsRequirements([spec2, "garbage"]) === 2,
		"unparseable sub-spec skipped, rest counted",
	);
	check(countSubSpecsRequirements([]) === null, "empty sub_specs → null");

	// resolveBudgetMode: locked flag > locked param > config default.
	check(
		resolveBudgetMode("free", "full") === "free",
		"locked flag beats param",
	);
	check(
		resolveBudgetMode(undefined, "economy") === "economy",
		"locked param honored",
	);
	check(
		resolveBudgetMode(undefined, undefined, "free") === "free",
		"config default applies when nothing locks",
	);
	check(
		resolveBudgetMode("auto", "economy") === "economy",
		"locked param beats an auto flag",
	);
	check(
		resolveBudgetMode(undefined, undefined, "auto") === "auto",
		"config default auto → auto",
	);
	...
	check(
		...
		"config default economy when nothing locks",
	);
	check(
		resolveBudgetTier(undefined, undefined, "auto", 3, DEFAULT_BUDGET_TIERS) ===
			"economy",
		"config auto + 3 reqs → economy",
	);
	check(
		resolveBudgetTier(undefined, undefined, "auto", 8, DEFAULT_BUDGET_TIERS) ===
			"full",
		"config auto + 8 reqs → full",
	);
	check(
		resolveBudgetTier("free", undefined, "full", 8, DEFAULT_BUDGET_TIERS) ===
			"free",
		"locked flag beats heuristic",
	);

	console.log(
		"✓ auto heuristic + lenient counting + config-default resolution (dynamic tiers)",
	);
}
...
function testBudgetOverridePersistence(errors: string[]): void {
	...
	check(
		readBudgetOverride(fakeCtx([budgetEntry("economy")])) === "economy",
		"single override read back",
	);
	check(
		readBudgetOverride(
			fakeCtx([budgetEntry("economy"), budgetEntry("free")]),
		) === "free",
		"latest entry wins",
	);
	check(
	...
	check(
		readBudgetOverride(
			fakeCtx([budgetEntry("economy"), budgetEntry("auto")]),
		) === "auto",
		"auto override persists too",
	);
	check(
		readBudgetOverride(fakeCtx([])) === undefined,
		"no entries → undefined",
	);
	check(
		readBudgetOverride(
			fakeCtx([budgetEntry("economy"), budgetEntry("bogus")]),
		) === "auto",
		"invalid latest normalizes to auto",
	);
	check(
		readBudgetOverride(
			fakeCtx([budgetEntry("free", "pi-task-other"), budgetEntry("economy")]),
		) === "economy",
		"non-budget entries ignored",
	);
	...
	check(
		readSessionTokensBefore(
			fakeCtx([msgEntry("user"), msgEntry("toolResult")]),
		) === 0,
	...
	check(
		readSessionTokensBefore(
			fakeCtx([
				msgEntry("assistant", { totalTokens: 1000 }),
				msgEntry("assistant", { totalTokens: 2500 }),
			]),
		) === 3500,
	...
	check(
		readSessionTokensBefore(
			fakeCtx([msgEntry("assistant", { input: 800, output: 200 })]),
		) === 1000,
		"no totalTokens → input + output fallback",
	);
	check(
		readSessionTokensBefore(fakeCtx([msgEntry("assistant")])) === 0,
		"assistant without usage → 0",
	);
	check(
		readSessionTokensBefore(
			fakeCtx([
				budgetEntry("economy"),
				msgEntry("assistant", { totalTokens: 42 }),
			]),
		) === 42,
	...
	const sessionMode =
		readBudgetOverride(fakeCtx([budgetEntry("economy")])) ??
		normalizeBudgetMode(flag);
	check(
		sessionMode === "economy",
		"session_start resolution: stored override wins over the auto flag",
	);
	const lockedTier = resolveBudgetTier(
		sessionMode,
		undefined,
		"full",
		8,
	...
	check(
		lockedTier === "economy",
		"locked session mode must beat config default + auto heuristic",
	);
	check(
		resolveBudgetTier(flag, undefined, "full", 8, DEFAULT_BUDGET_TIERS) ===
			"full",
		"raw flag alone resolves to full for 8 requirements — the bypass the handler fix removes",
	);
	const plan = buildRunPlan({ tier: lockedTier, executeModel: "m" });
	check(plan.tier === "economy", "plan carries the locked tier");

	console.log(
		"✓ readBudgetOverride: last matching session entry wins; /task-budget lock survives into the task call",
	);
	console.log(
		"✓ readSessionTokensBefore: assistant-message token sums (R1 main-session spend)",
	);
}
...
function testGoalsPersistence(errors: string[]): void {
	...
	check(
		readGoals(fakeCtx([goalsEntry("block algorithmic feeds")])) ===
	...
	check(
		readGoals(
			fakeCtx([
				goalsEntry("ship the feed"),
				goalsEntry("block algorithmic feeds"),
			]),
		) === "block algorithmic feeds",
		"latest goals entry wins",
	);
	check(
		readGoals(fakeCtx([goalsEntry("a"), goalsEntry("b"), goalsEntry("c")])) ===
			"c",
		"last of three wins",
	);
	check(
		readGoals(fakeCtx([goalsEntry("a"), goalsEntry("   ")])) === undefined,
		"whitespace-only latest statement → no goals",
	);
	check(
		readGoals(fakeCtx([goalsEntry("  keep the api small  ")])) ===
			"keep the api small",
		"goals are trimmed on read",
	);
	check(
		readGoals(fakeCtx([goalsEntry("x", "pi-task-budget"), goalsEntry("y")])) ===
			"y",
	...
	console.log(
		"✓ readGoals: last matching pi-task-goals session entry wins; trimmed; user-owned (command is the only writer)",
	);
}
...
function testResultMapping(errors: string[]): void {
	...
	{
		const text = detachedDispatchText(
			"20260805T0000-abcd",
			"proj",
			"/m/proj/20260805T0000-abcd.log",
		);
		check(
			text.includes("Task detached: run 20260805T0000-abcd (project proj)"),
			"detached text names the run + project",
		);
		check(
			text.includes("/task-status 20260805T0000-abcd"),
			"detached text shows the tracking command",
		);
		check(
			text.includes("results/proj/20260805T0000-abcd.json"),
			"detached text names the manifest path",
		);
		check(
			text.includes("/m/proj/20260805T0000-abcd.log"),
			"detached text names the log path",
		);
	}

	const ret: TaskToolReturn = taskResultToToolReturn(fakeResult());
	check(
		ret.success === true && ret.tests === "passing",
		"mapping carries success/tests",
	);
	check(
		ret.commits.length === 1 && ret.commits[0] === "abc123",
		"mapping carries commits",
	);
	check(
		ret.files_changed.includes("hello.txt"),
		"mapping carries files_changed",
	);
	check(
		ret.duration_ms === 65000,
		"mapping carries the run duration (the completion summary's latency fallback)",
	);
	...
	check(
		ret.verification.passed === true && ret.verification.failures.length === 0,
		"mapping carries verification",
	);

	// Parallel run: conflicts + workers surfaced.
	const par = taskResultToToolReturn(
		fakeResult({
			success: false,
			conflicts: ["a.txt"],
			workers: [],
			...
			verification: {
				...
				commands: 1,
				duration_ms: 1,
				failures: [{ command: "x", exitCode: 1, output: "oops" }],
			},
		}),
	);
	check(par.success === false && par.tests === "failing", "failure mapped");
	check(
		par.conflicts !== undefined && par.conflicts.length === 1,
		"conflicts surfaced on parallel runs",
	);
	const firstFailure = par.verification.failures[0]!;
	check(
		firstFailure.exitCode === 1 && firstFailure.output === "oops",
	...
	check(
		withMetrics.metrics === manifest,
		"metrics (RunManifest) passed through",
	);

	// summarizeResult: readable one-line + expanded text.
	const summary = summarizeResult(fakeResult());
	check(
		summary.includes("succeeded") &&
			summary.includes("1 commit") &&
			summary.includes("passing"),
	...
	const caveated = taskResultToToolReturn(
		fakeResult({
			caveat:
				"worker 1 aborted during finalization; verified post-merge — merged commit xyz, 3 file(s) changed",
		}),
	);
	check(
		caveated.success === true,
		"finalization-incomplete recovery maps as a success",
	);
	check(
		caveated.caveat !== undefined &&
			caveated.caveat.includes("aborted during finalization"),
	...
	const caveatSummary = summarizeResult(
		fakeResult({
			caveat:
				"worker 1 aborted during finalization; verified post-merge — merged commit xyz, 3 file(s) changed",
		}),
	...
	check(
		caveatSummary.includes("aborted during finalization") &&
			caveatSummary.includes("verified post-merge"),
		`summary carries the caveat, got: ${caveatSummary}`,
	);

	console.log(
		"✓ taskResultToToolReturn: typed mapping + conflicts/metrics/verification/review_skipped + caveat",
	);
}
...
function testReviewReport(errors: string[]): void {
	...
	check(
		report.startsWith("Review: fix — 2 finding(s)."),
		`verdict line, got: ${report.split("\n")[0]}`,
	);
	check(
		report.includes("Requirements: R1: met; R2: unmet; R3: uncertain"),
		`per-requirement status, got: ${report}`,
	);
	check(
		report.includes("  [P1, security] src/auth.ts: tokens leaked in logs"),
		`finding line format, got: ${report}`,
	);
	check(
		report.includes("  [P2, design] src/api.ts: error paths swallow failures"),
		"second finding line",
	);
	...
	check(
		summary.includes("Review: fix — 2 finding(s).") &&
			summary.includes("R2: unmet"),
		`summary carries the review report, got: ${summary}`,
	);
	check(
		summary.includes("[P1, security] src/auth.ts: tokens leaked in logs"),
	...
	check(
		cleanReport === "Review: ship — 0 finding(s).\n  Requirements: R1: met",
	...
	const capped = renderReviewReport({
		verdict: "fix",
		findings: many,
		requirements: [],
	});
	...
	check(
		capped.includes("... and 5 more finding(s)"),
		`elision line, got: ${capped.split("\n").pop()}`,
	);
	...
	const atCap = renderReviewReport({
		verdict: "fix",
		findings: many.slice(0, 10),
		requirements: [],
	});
	check(!atCap.includes("... and"), "at-cap report has no elision line");

	// Empty requirements array: no Requirements line.
	const noReqs = renderReviewReport({
		verdict: "fix",
		findings: [],
		requirements: [],
	});
	check(
		noReqs === "Review: fix — 0 finding(s).",
		"no requirements → no Requirements line",
	);

	console.log(
		"✓ renderReviewReport: verdict + requirement status + finding lines, capped at MAX_REVIEW_FINDINGS_IN_SUMMARY",
	);
}
...
function testRenderInPlace(errors: string[]): void {
	...
	// pi-tui's Text keeps its content private (no getter) — read it through
	// a structural cast so the reuse contract stays assertable hermetically.
	const textOf = (c: unknown): string => (c as { text: string }).text;
	const first = renderInPlace(ctx, "plan(full): work(m)");
	check(
		textOf(first) === "plan(full): work(m)",
		"first render creates a Text with the content",
	);
	ctx.lastComponent = first;
	const second = renderInPlace(ctx, "plan(full): work(m)\n1/1 workers done");
	check(
		second === first,
		"partial re-render must return the SAME component (no stacking)",
	);
	check(
		textOf(second) === "plan(full): work(m)\n1/1 workers done",
		"reused component carries the new content",
	);

	// A missing lastComponent (first render) creates a fresh Text.
	const fresh = renderInPlace({}, "x");
	check(
		fresh !== first && textOf(fresh) === "x",
		"no previous component → fresh Text",
	);

	console.log(
		"✓ renderInPlace: reuses the previous component in place; fresh Text on first render",
	);
}
...
function testRunPlan(errors: string[]): void {
	...
	check(
		JSON.stringify(names(full)) ===
			JSON.stringify(["prewalk", "work", "review"]),
		`full plan phases, got ${JSON.stringify(names(full))}`,
	);
	check(
		JSON.stringify(models(full)) ===
			JSON.stringify(["prov/strong", "prov/fast", "prov/strong"]),
	...
	check(
		JSON.stringify(names(free)) === JSON.stringify(["work"]),
		`free plan omits review, got ${JSON.stringify(names(free))}`,
	);

	// prewalk == execute auto-skips the prewalk phase (mirrors isPrewalkActive).
	const skipped = buildRunPlan({
		tier: "x",
		prewalkModel: "prov/fast",
		executeModel: "prov/fast",
		review: false,
	});
	check(
		JSON.stringify(names(skipped)) === JSON.stringify(["work"]),
	...
	const fallback = buildRunPlan({
		tier: "x",
		executeModel: "prov/fast",
		review: true,
	});
	check(
		fallback.phases[1]?.model === "prov/fast",
		"review model falls back to the execute model",
	);
	...
	check(
		withReviewWall.reviewWallTimeoutMs === 20 * 60_000,
		"plan carries the review wall",
	);
	check(
		renderPlanLine(withReviewWall) ===
			"plan(full): work(prov/fast) → review(prov/strong) · review wall 20m",
	...
	check(
		renderPlanLine(goalsAndWall) ===
			"plan(full): work(prov/fast) → review(prov/strong) · goals: keep it small · review wall 10m",
	...
	check(
		!renderPlanLine(noReviewWall).includes("review wall"),
		"no review phase → no review-wall clause",
	);
	const noReviewWallValue = buildRunPlan({
		tier: "full",
		executeModel: "prov/fast",
		review: true,
	});
	check(
		!renderPlanLine(noReviewWallValue).includes("review wall"),
		"review phase without a wall value → no clause",
	);
	...
	const withGoals = buildRunPlan({
		tier: "full",
		executeModel: "prov/fast",
		goals: "block algorithmic feeds",
	});
	check(
		withGoals.goals === "block algorithmic feeds",
		"plan carries the resolved goals",
	);
	check(
		renderPlanLine(withGoals) ===
			"plan(full): work(prov/fast) · goals: block algorithmic feeds",
		`plan line with goals clause, got: ${renderPlanLine(withGoals)}`,
	);
	const longGoals =
		"keep the feed chronological and free of engagement-baiting ranking models everywhere";
	const truncated = renderPlanLine(
		buildRunPlan({ tier: "full", executeModel: "prov/fast", goals: longGoals }),
	);
	check(
		truncated.endsWith(
			"goals: keep the feed chronological and free of engagement-baitin…",
		),
		`long goals truncated on the plan line, got: ${truncated}`,
	);
	check(
		renderGoalsClause(undefined) === "" &&
			renderGoalsClause("") === "" &&
			renderGoalsClause("   ") === "",
		"absent/blank goals render no clause",
	);
	check(
		renderGoalsClause("keep it small") === "goals: keep it small",
		"goals clause prefix",
	);
	check(
		truncateGoals("a".repeat(60)) === "a".repeat(60) &&
			truncateGoals("a".repeat(61)) === "a".repeat(57) + "…",
	...
	check(
		truncateGoals("  padded  goal  ") === "padded goal",
		"whitespace collapsed + trimmed",
	);
	check(
		!renderPlanLine(full).includes("goals:"),
		"absent goals → no goals clause (backward compatible)",
	);

	console.log(
		"✓ buildRunPlan: phase sequence + per-phase models; tier-gated omissions",
	);
}
...
function testProgressStateAndRender(errors: string[]): void {
	...
	check(
		state.startMs === 1000 && state.workers.get(0)?.phaseStartMs === 1000,
		"state records the run start + per-worker phase start",
	);
	let text = buildProgressText(state, 1000);
	check(
		text.startsWith(
			"plan(full): prewalk(prov/strong) → work(prov/fast) → review(prov/strong)",
		),
	...
	check(
		text.includes(
			"⏳ worker-1: prewalk 0s | total 0s | no checklist yet | 0 turns, 0s idle",
		),
	...
	const goalsText = buildProgressText(
		createProgressState(1, goalsPlan, 1000),
		1000,
	);
	check(
		goalsText
			.split("\n")[0]!
			.includes(
				"plan(full): prewalk(prov/strong) → work(prov/fast) → review(prov/strong) · goals: block algorithmic feeds before launch",
			),
		`goals clause on the widget plan line, got: ${goalsText.split("\n")[0]}`,
	);
	const plainText = buildProgressText(
		createProgressState(1, fullPlan, 1000),
		1000,
	);
	check(
		!plainText.includes("goals:"),
		"widget plan line without goals stays plain",
	);

	// Tier without prewalk (economy): plan line omits prewalk; workers start in work.
	const economyPlan = buildRunPlan({
		tier: "economy",
		executeModel: "prov/fast",
		reviewModel: "prov/fast",
		review: true,
	...
	check(
		eText.startsWith("plan(economy): work(prov/fast) → review(prov/fast)"),
		`economy plan line, got: ${eText.split("\n")[0]}`,
	);
	check(
		eText.includes("⏳ worker-1: work 0s | total 0s | no checklist yet"),
		"economy worker starts in work phase",
	);

	// Tier without review (free): plan line omits review entirely.
	const freePlan = buildRunPlan({
		tier: "free",
		executeModel: "prov/fast",
		review: false,
	});
	...
	check(
		fText.startsWith("plan(free): work(prov/fast)"),
		`free plan line, got: ${fText.split("\n")[0]}`,
	);
	...
	check(
		!text.includes("no checklist yet"),
		"initialized checklist replaces the placeholder",
	);
	applyProgressEvent(state, { type: "checklist", done: 3, total: 5 }, 3000);
	text = buildProgressText(state, 3000);
	check(
		text.includes("checklist 3/5"),
		`checklist after check-offs, got: ${text}`,
	);
	...
	applyProgressEvent(
		state,
		{ type: "merge", conflicts: [], commit_id: "c8e4de5f", files_changed: 19 },
		5000,
	);
	text = buildProgressText(state, 5000);
	check(
		text.includes(
			"merged 1 worker commit(s) → c8e4de5f (19 file(s) changed vs base; worker commits consumed)",
		),
	...
	applyProgressEvent(
		metaState,
		{
			...
			metas: [
				{
					goal: "Fix the Rust side of the bot",
					scope: ["rust/bot/src/convert.rs", "tests/convert_integration.rs"],
				},
			],
		},
		6000,
	...
	check(
		text.includes(
			"worker-1 → Fix the Rust side of the bot [rust/bot/src/convert.rs, tests/convert_integration.rs]",
		),
	...
	applyProgressEvent(
		metaState,
		{
			type: "tool_start",
			toolName: "edit",
			args: "{path:rust/bot/src/convert.rs}",
		},
		7000,
	);
	text = buildProgressText(metaState, 7000);
	check(
		text.includes("⎈ edit: {path:rust/bot/src/convert.rs}"),
		`live tool line, got: ${text}`,
	);
	...
	// prewalk phase, no swap, no review phase) — what /survey dispatches.
	{
		...
		const t = buildProgressText(
			createProgressState(1, analysisPlan, 1000),
			1000,
		);
		check(
			t.startsWith("plan(full): work(qwen-token-plan/qwen3.8-max-preview)"),
		...
		check(
			!t.includes("→ prewalk") && !t.includes("→ review"),
			"analysis plan omits prewalk + review phases",
		);
	}

	// C: the tier wall shows as total-clock headroom.
	const wallPlan = buildRunPlan({
		tier: "economy",
		executeModel: "prov/fast",
		wallTimeoutMs: 25 * 60_000,
	});
	const wallText = buildProgressText(
		createProgressState(1, wallPlan, 1000),
		2000,
	);
	check(
		wallText.includes("total 1s/25m"),
		`wall headroom in the total clock, got: ${wallText}`,
	);
	check(
		!buildProgressText(state, 8000).includes("total 7s/"),
		"no wall suffix when the plan carries no wall",
	);
	...
	check(
		JSON.stringify(
			extractFileScope(
				"touch docs/pi-task-design.md and extensions/task/worker.ts, then verify",
			),
		) ===
	...
	check(
		extractFileScope(
			"no paths here — just prose about https://example.com/x.md and 0.83.0",
		).length === 0,
		"extractFileScope filters URLs + version tokens",
	);
	check(
		extractFileScope("a.ts a.ts b.rs c.md d.txt e.go").length === 5,
	...
	check(
		text.includes("3 turns, 12s idle"),
		`idle rendering, got: ${text.split("\n")[2]}`,
	);
	check(
		text.includes("prewalk 15s | total 15s"),
		`phase + total elapsed, got: ${text.split("\n")[2]}`,
	);
	...
	applyProgressEvent(
		state,
		{ type: "tool_end", toolName: "edit", isError: true },
		17000,
	);
	check(
		state.workers.get(0)?.phase === "prewalk",
		"errored edit must not transition prewalk → work",
	);
	applyProgressEvent(
		state,
	...
	check(
		state.workers.get(0)?.phase === "work",
		"successful write transitions prewalk → work",
	);
	text = buildProgressText(state, 17000);
	check(
		text.includes("⏳ worker-1: ✓prewalk → work 0s | total 16s |"),
	...
	check(
		state.workers.get(0)?.phase === "review",
		"review_start transitions work → review",
	);
	check(
		state.workers.get(0)?.phaseStartMs === 20000,
		"review_start restarts the phase clock",
	);
	...
	check(
		text.includes("1/1 workers done"),
		`done header after yield, got: ${text.split("\n")[1]}`,
	);
	check(
		text.includes(
			"⏳ worker-1: ✓prewalk → ✓work → ✓review 1s | total 20s | checklist 3/5 | 3 turns, 0s idle",
		),
	...
	check(
		text.includes(
			"⏳ worker-1: ✓prewalk → ✓work → ✓review 1s | total 4s | no checklist yet | 3 turns, 0s idle",
		),
		`R3: frozen turns/checklist + live review elapsed, got: ${text.split("\n")[2]}`,
	);
	const laterReview = buildProgressText(s4, 9000);
	check(
		laterReview.includes("✓prewalk → ✓work → ✓review 6s | total 9s"),
	...
	check(
		t2.includes("✓ worker-1: ✓work 9s | total 9s | no checklist yet | 1 turn"),
	...
	check(
		text.includes(
			"⏳ worker-1: work 0s | total 0s | no checklist yet | 4 turns, 0s idle",
		),
		"worker-1 running line",
	);
	check(
		text.includes(
			"⏳ worker-2: work 0s | total 0s | no checklist yet | 0 turns, 0s idle",
		),
		"worker-2 fresh line",
	);
	...
	applyProgressEvent(
		state,
		{ type: "workspace_created", index: 0, dir: "/x" },
		21000,
	);
	applyProgressEvent(state, { type: "merge", index: 0, conflicts: [] }, 21000);
	applyProgressEvent(
		state,
		{ type: "review", verdict: "ship", findings: 0, decision: "ship" },
		21000,
	);
	applyProgressEvent(state, null, 21000);
	applyProgressEvent(state, { type: "turn" }, 21000);
	check(
		buildProgressText(state, 21000) === before,
		"unknown events change nothing",
	);

	console.log(
		"✓ buildProgressText: plan render, checklist, phases, checkmarks, durations, liveness, review freeze",
	);
}
...
function testSummaryMetrics(errors: string[]): void {
	...
	check(
		formatDuration(0) === "0s",
		`formatDuration(0), got ${formatDuration(0)}`,
	);
	check(
		formatDuration(42000) === "42s",
		`formatDuration(42000), got ${formatDuration(42000)}`,
	);
	check(
		formatDuration(65000) === "1m05s",
		`formatDuration(65000), got ${formatDuration(65000)}`,
	);
	check(
		formatDuration(60000) === "1m",
		`formatDuration(60000), got ${formatDuration(60000)}`,
	);
	check(
		formatDuration(120000) === "2m",
		`formatDuration(120000), got ${formatDuration(120000)}`,
	);
	check(
		formatDuration(-5) === "0s",
		`formatDuration clamps negatives, got ${formatDuration(-5)}`,
	);
	...
	const manifest: RunManifest = {
		...
		config: {
			budget: "full",
			prewalk_model: "prov/strong",
			execute_model: "prov/fast",
			review_model: "prov/strong",
			swap_trigger: "first-edit",
			checklist: true,
			review_forked: true,
			sandbox: false,
			shape: "code",
	...
	check(
		single.includes("task done in 1m05s · $0.0075 · 1/1 verified (full)"),
		`single manifest: one-line summary (duration from totals.duration_ms, cost/verify/tier from the manifest), got: ${single.split("\n")[0]}`,
	);
	check(
		single.includes("Tokens: 1000 in / 500 out."),
		`single manifest: tokens sum phases, got: ${single}`,
	);
	check(
		single.includes("$0.0075"),
		`single manifest: cost in the summary line, got: ${single}`,
	);
	check(
		!single.includes("Took ") && !single.includes("Cost:"),
		"single manifest: duration/cost not duplicated (R4)",
	);
	check(
		!single.includes("task done in 1s"),
		"manifest latency beats result.durationMs",
	);
	...
	const parallelManifest: RunManifest = {
		...
		merge: {
			...
			overlaps: [],
			worker_count: 2,
			merged_commit_id: "c8e4de5f",
			files_changed: 19,
		},
	};
	const parallel = summarizeResult(
		fakeResult({
	...
	check(
		parallel.includes(
			"merged 2 worker commit(s) → c8e4de5f (19 file(s) changed vs base; worker commits consumed)",
		),
		`parallel summary reports the merge outcome, got: ${parallel.split("\n")[1]}`,
	);
	check(
		!parallel.includes("1 commit(s),"),
		"parallel summary does not use the single-worker commit phrasing",
	);
	...
	check(
		parSummary.includes("task done in 45s"),
		`parallel no-manifest: duration from durationMs, got: ${parSummary}`,
	);
	check(
		parSummary.includes("Tokens: 1000 in / 500 out."),
		`parallel no-manifest: last snapshots aggregated, got: ${parSummary}`,
	);
	check(
		parSummary.includes("Cost: $0.003."),
		`parallel no-manifest: usage cost summed, got: ${parSummary}`,
	);
	...
	check(
		bare.includes("task done in 1m05s"),
		`no-manifest single: duration still shown, got: ${bare}`,
	);
	check(
		!bare.includes("Tokens:"),
		`no-manifest single: no tokens, got: ${bare}`,
	);
	...
	check(
		noCostSummary.includes("Tokens: 10 in / 5 out."),
		`tokens derived without cost, got: ${noCostSummary}`,
	);
	check(
		!noCostSummary.includes("Cost:"),
		`cost omitted when usage lacks it, got: ${noCostSummary}`,
	);

	// deriveRunMetrics directly: no manifest + no usage → duration-only shape.
	const metrics = deriveRunMetrics(fakeResult());
	check(
		metrics.durationMs === 65000 &&
	...
	check(
		manifestMetrics.tokensIn === 1000 &&
			manifestMetrics.tokensOut === 500 &&
			manifestMetrics.costUsd === 0.0075,
	...
	const withProgress = failureMessageWithProgress(
		"Worker wall-timeout: ...",
		"plan(economy): work(prov/fast)\n1/1 workers done",
	);
	check(
		withProgress.startsWith("Worker wall-timeout") &&
			withProgress.includes("Last progress:\nplan(economy)"),
		`failure message carries the last progress, got: ${withProgress}`,
	);
	check(
		failureMessageWithProgress("boom", "") === "boom",
		"empty progress leaves the message bare",
	);

	console.log(
		"✓ summarizeResult/deriveRunMetrics: one-line summary + tokens, manifest preferred, usage fallback, duration-only degradation; failureMessageWithProgress",
	);
}
...
function testWorkflowContract(errors: string[]): void {
	...
	check(
		workflowContractBlock({ workflowContract: true }) === text,
		"enabled config → the contract text",
	);
	check(
		workflowContractBlock({ workflowContract: false }) === "",
		"disabled config → no block",
	);
	...
	check(
		/Batch independent tool calls/.test(eff) &&
			/never make sequential calls/.test(eff),
		"efficiency: batch independent tool calls into one turn",
	);
	check(
		/head -c ~2000/.test(eff) && /never cat whole files/.test(eff),
		"efficiency: truncate large outputs",
	);
	check(
		/re-run an identical command/i.test(eff) && /smaller query/.test(eff),
		"efficiency: don't re-run identical commands",
	);
	check(
		/at most twice/.test(eff) && /targeted test files/.test(eff),
	...
	console.log(
		"✓ efficiencyContractText: pure efficiency block (batch / truncate / no re-runs / test-sparing / fewest turns)",
	);

	console.log(
		"✓ workflowContract: pure ~120-150-word block (goals / plan-first / delegate-by-default / orientation-only / CONTEXT.md shared language / spec discipline), config-gated",
	);
}
...
function testCompletionSummary(errors: string[]): void {
	...
	const mk = (overrides: Partial<RunManifest> = {}): RunManifest => ({
		...
		config: {
			budget: "economy",
			prewalk_model: "prov/m",
			execute_model: "prov/m",
			review_model: "prov/m",
			swap_trigger: "first-edit",
			checklist: true,
			review_forked: false,
	...
	const line = completionSummaryLine({
		success: true,
		manifest: wall,
		durationMs: 1,
	...
	check(
		completionSummaryLine({
			...
			durationMs: 0,
			verificationFailures: [],
		}) === "task done in 1m · $0.006 · 5/5 verified (economy)",
		"inverted timestamps → totals.duration_ms fallback",
	);
	...
	const failed = completionSummaryLine({
		success: false,
		manifest: fail,
		durationMs: 0,
		verificationFailures: [1, 2, 3],
	});
	check(
		failed === "task failed in 1m · $0.006 · 2/5 verified (economy)",
	...
	const withManifest = fakeResult({
		manifest: mk({
			received_at: "2026-08-06T10:00:00.000Z",
			completed_at: "2026-08-06T10:00:42.000Z",
		}),
	});
	const summary = summarizeResult(withManifest);
	check(
		summary.split("\n")[0] ===
			"task done in 42s · $0.006 · 5/5 verified (economy)",
		`summary first line, got: ${summary.split("\n")[0]}`,
	);
	check(
		summary.includes("Tokens: 100 in / 50 out."),
		`tokens stay, got: ${summary}`,
	);
	...
	check(
		failedSummary.split("\n")[0] ===
			"task failed in 1m · $0.006 · 2/5 verified (economy)",
		`failure first line, got: ${failedSummary.split("\n")[0]}`,
	);
	check(
		failedSummary.includes("Task failed: 1 commit(s), tests failing"),
	...
	check(
		completionSummaryLine({
			...
			durationMs: 0,
			verificationFailures: [],
		}).includes("pre-dispatch: 3.5k tokens"),
	...
	check(
		completionSummaryLine({
			...
			durationMs: 0,
			verificationFailures: [],
		}).includes("pre-dispatch: 800 tokens"),
		"pre-dispatch renders raw tokens below 1000",
	);
	for (const absent of [undefined, 0]) {
		const noClause = completionSummaryLine({
			success: true,
			manifest: mk(absent === undefined ? {} : { main_session_tokens: absent }),
			durationMs: 0,
			verificationFailures: [],
		});
		check(
			!noClause.includes("pre-dispatch"),
			`no pre-dispatch clause when main_session_tokens is ${absent}`,
		);
	}
	check(
		completionSummaryLine({
	...
	const preSummary = summarizeResult(
		fakeResult({
			manifest: mk({
				received_at: "2026-08-06T10:00:00.000Z",
				completed_at: "2026-08-06T10:00:42.000Z",
				main_session_tokens: 12345,
			}),
		}),
	);
	const preSummaryFirstLine = preSummary.split("\n")[0]!;
	check(
		preSummaryFirstLine.includes("pre-dispatch: 12.3k tokens"),
		`summarizeResult first line carries the clause, got: ${preSummaryFirstLine}`,
	);
	const noPreSummary = summarizeResult(fakeResult({ manifest: mk() }));
	check(
		!noPreSummary.includes("pre-dispatch"),
		"summarizeResult omits the clause when the manifest lacks it",
	);
	...
	check(
		formatTokenCount(0) === "0 tokens" &&
			formatTokenCount(999) === "999 tokens",
		"raw token counts below 1000",
	);
	check(
		formatTokenCount(1000) === "1k tokens" &&
			formatTokenCount(12000) === "12k tokens" &&
			formatTokenCount(120000) === "120k tokens",
		"integer k trims the decimal",
	);
	check(
		formatTokenCount(12345) === "12.3k tokens" &&
			formatTokenCount(12355) === "12.4k tokens",
		"one-decimal k rounding",
	);

	console.log(
		"✓ completionSummaryLine: wall-clock latency · cost · verify status · tier; summarizeResult first line (R1/R2/R4)",
	);
}
...
export function runTests(): Promise<void> {
	const errors: string[] = [];
	console.log(
		"── test-index: schema locking + budget resolution + override persistence + sub_specs precedence + result mapping + progress ──",
	);
