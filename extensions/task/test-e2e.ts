...
import { attachJsonlReader, runWorker, CHECKLIST_EXTENSION_PATH, DEFAULT_WORKER_SYSTEM_PROMPT, spawnWorkerSession } from "./worker.ts";
...
const TASK_EXTENSION_PATH = join(dirname(fileURLToPath(import.meta.url)), "index.ts");
...
function assertSteering(signals: number[], label: string): void {
	console.log(`  ${label} remaining sequence:`, signals.join(" → ") || "(none)");
	const meaningful = signals.filter((n) => n > 0);
	if (meaningful.length === 0) {
		errors.push(`[${label}] checklist was never initialized (no remaining:N with N>0)`);
		return;
	}
	const max = Math.max(...meaningful);
	if (max < 2) errors.push(`[${label}] expected checklist with >= 2 items, max remaining was ${max}`);
	if (signals[signals.length - 1] !== 0) {
		errors.push(`[${label}] expected final remaining to be 0, got ${signals[signals.length - 1]}`);
	}
	const firstMax = signals.indexOf(max);
	const tail = signals.slice(firstMax);
	for (let i = 1; i < tail.length; i++) {
		if (tail[i] > tail[i - 1]) {
			errors.push(`[${label}] remaining increased after init: ${signals.join(",")}`);
			break;
		}
	}
}
...
async function section1RunWorker(): Promise<void> {
	...
	try {
		...
		console.log(`  turns: ${result.usage.turns}, files: ${JSON.stringify(result.yield.files_changed)}`);

		check(result.yield.files_changed.includes("hello.txt"),
		...
		check(result.usage.turns >= 1, `turns should be >= 1, got ${result.usage.turns}`);
	} finally {
	...
	try {
		...
		try {
			...
			if (err.message.includes("abort")) {
				console.log("  ✓ abort rejected with:", err.message);
			} else {
				errors.push(`[2] unexpected error: ${err.message}`);
			}
		}
	} finally {
		rmSync(dir, { recursive: true, force: true });
...
async function section3ChecklistAndPrewalk(): Promise<void> {
	...
	try {
		...
		session.onEvent((e: any) => {
			if (e.type === "extension_ui_request" && e.method === "setStatus") {
				if (e.statusKey === "checklist") checklistSignals.push(parseRemaining(e.statusText));
		...
		check(result.yield.files_changed.includes("README.md"),
		...
		check(readme.includes("## Features") && readme.includes("Updated by the task worker"),
			`README should contain both additions, got: ${JSON.stringify(readme.slice(0, 200))}`);
		check(result.yield.commit_ids.length > 0, "[3] commit_ids should be non-empty");
		check(result.yield.deviations.length === 0, `[3] expected no deviations, got ${JSON.stringify(result.yield.deviations)}`);

		// Prewalk extension pruning: active → pruned at the first edit, stays pruned.
		if (!planningStates.includes("active")) {
			errors.push(`[3] expected at least one "active" planning state, got: ${planningStates.join(",")}`);
		}
		if (planningStates[planningStates.length - 1] !== "pruned") {
			errors.push(`[3] expected final planning state "pruned", got: ${planningStates.join(",")}`);
		}
		const firstPruned = planningStates.indexOf("pruned");
		if (firstPruned !== -1 && planningStates.slice(firstPruned).some((s) => s !== "pruned")) {
			errors.push(`[3] planning state flapped after pruning: ${planningStates.join(",")}`);
		}
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
...
async function section4ExecuteTask(): Promise<void> {
	...
	try {
		...
		console.log(`  success: ${result.success} | tests: ${result.tests} | commits: ${JSON.stringify(result.commits)}`);

		check(result.success, "[4] success should be true");
		check(result.tests === "passing", `[4] tests should be "passing", got "${result.tests}"`);
		check(result.commits.length > 0, "[4] commits should be non-empty");
		check(result.files_changed.includes("hello.txt"),
		...
		check(result.worker.usage.turns >= 1, `[4] turns should be >= 1, got ${result.worker.usage.turns}`);
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
}
...
`;
		...
		console.log(`  success: ${result.success} | workers: ${result.workers?.length} | conflicts: ${JSON.stringify(result.conflicts)}`);
		for (const [i, w] of (result.workers ?? []).entries()) {
			console.log(`  worker-${i}: turns=${w.usage.turns} reads=${w.usage.reads} commits=${JSON.stringify(w.yield.commit_ids)}`);
		}

		check(result.success, "[5] success should be true");
		check(result.tests === "passing", `[5] tests should be "passing", got "${result.tests}"`);
		check(result.conflicts !== undefined && result.conflicts.length === 0,
			`[5] no merge conflicts expected, got ${JSON.stringify(result.conflicts)}`);
		check(result.workers !== undefined && result.workers.length === 2,
			`[5] expected 2 workers, got ${result.workers?.length}`);
		check(result.workers !== undefined && result.workers.every((w) => w.usage.turns >= 1),
			"[5] each worker should have usage.turns >= 1");
		for (const f of ["a.txt", "b.txt"]) {
			check(result.files_changed.includes(f),
		...
		check(result.commits.length === 1,
			`[5] expected exactly 1 (merged base) commit id, got ${JSON.stringify(result.commits)}`);
		if (result.commits.length === 1) {
			try {
				execSync(`jj log -r ${result.commits[0]} --no-graph`, { cwd: dir, stdio: "pipe" });
			} catch {
				errors.push(`[5] returned commit id ${result.commits[0]} does not resolve via jj log -r`);
			}
		}
...
async function section6RepoMap(): Promise<void> {
	...
	for (const [rel, content] of Object.entries({
		...
		"src/utils.ts": "export function clamp(n: number, lo: number, hi: number): number {\n  return Math.max(lo, Math.min(hi, n));\n}\n",
	...
	try {
		...
		check(map.files.length === 4, `[6] expected 4 files in map, got ${map.files.length}`);
		check(Array.isArray(map.entryPoints), "[6] entryPoints should be an array");
		const summarized = map.files.filter((f) => f.summary && f.summary.length > 0).length;
		console.log(`  build: ${map.files.length} files, ${summarized} with summaries`);
		console.log(`  entryPoints: ${JSON.stringify(map.entryPoints)}`);
		for (const f of map.files) console.log(`    ${f.path} [${f.role ?? "?"}] ${(f.summary ?? "").slice(0, 80)}`);
		check(summarized >= 2, `[6] expected most files to have LLM summaries, got ${summarized}/${map.files.length}`);

		// Cache hit: same tree → returned as-is (generated unchanged proves no rebuild)
		const map2 = await buildMap(dir, { model: MODEL });
		check(map2.generated === map.generated, "[6] cache hit should return the cached map (generated unchanged)");
		...
		writeFileSync(join(dir, "src/calculator.ts"), 'export function add(a: number, b: number): number {\n  return a + b;\n}\nexport function multiply(a: number, b: number): number {\n  return a * b;\n}\n');
		const map3 = await buildMap(dir, { model: MODEL });
		check(map3.treeHash !== map.treeHash, "[6] treeHash should change after edit");
		...
		if (calc3 && calc0) {
			check(calc3.contentHash !== calc0.contentHash, "[6] edited file contentHash should change");
		}
		const utils3 = map3.files.find((f) => f.path === "src/utils.ts");
		const utils0 = before.get("src/utils.ts");
		check(!!utils3 && !!utils0 && utils3.summary === utils0.summary,
			"[6] unchanged file should keep its cached summary");
		console.log("  incremental rebuild ✓ (changed file re-annotated, unchanged preserved)");
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
}
...
async function section7Review(): Promise<void> {
	...
	try {
		...
		console.log(`  success: ${result.success} | tests: ${result.tests} | verdict: ${result.review?.verdict}`);
		console.log(`  findings: ${result.review?.findings.length ?? 0} | requirements: ${JSON.stringify(result.review?.requirements)}`);
		for (const f of result.review?.findings ?? []) {
			console.log(`    [${f.priority}] (${f.category}) ${f.file}: ${f.description.slice(0, 70)}`);
		}
		console.log(`  fixLoop: ${JSON.stringify(result.fixLoop)}`);

		check(result.review !== undefined, "[7] review should be present when review enabled");
		check(result.fixLoop !== undefined, "[7] fixLoop metadata should be present");
		check(result.tests === "passing", `[7] tests should be passing, got ${result.tests}`);
		check(result.success === true, "[7] clean task should ship (success true)");
		if (result.review) {
			check(["ship", "fix", "escalate"].includes(result.review.verdict), `[7] verdict should be valid, got ${result.review.verdict}`);
			check(result.review.verdict === "ship", `[7] clean task should verdict ship, got ${result.review.verdict}`);
			check(Array.isArray(result.review.findings), "[7] findings should be an array");
			check(Array.isArray(result.review.requirements) && result.review.requirements.length >= 1,
				"[7] reviewer should report at least one requirement status");
		}
		if (result.fixLoop) {
			check(result.fixLoop.fixesDispatched === 0,
...
async function section8FixLoop(): Promise<void> {
	...
	try {
		...
		check(result.fixLoop !== undefined, "[8] fixLoop metadata should be present");
		check(result.tests === "passing", `[8] tests should converge to passing, got ${result.tests}`);
		check(result.success === true, "[8] fix loop should converge to success");
		check(result.files_changed.includes("done.txt"),
			`[8] done.txt should exist after the loop, got ${JSON.stringify(result.files_changed)}`);
		if (result.fixLoop) {
			console.log(`  fix workers dispatched: ${result.fixLoop.fixesDispatched}`);
			check(result.fixLoop.fixesDispatched >= 1,
...
async function section9Metrics(): Promise<void> {
	...
	try {
		...
		console.log(`  success: ${result.success} | verdict: ${result.review?.verdict}`);
		console.log(`  manifestPath: ${result.manifestPath}`);
		if (result.manifest) {
			console.log(`  run_id: ${result.manifest.run_id} | spec_hash: ${result.manifest.task.spec_hash}`);
			console.log(`  phases.execute: ${JSON.stringify(result.manifest.phases.execute)}`);
			console.log(`  phases.review: ${JSON.stringify(result.manifest.phases.review)}`);
			console.log(`  totals: ${JSON.stringify(result.manifest.totals)}`);
		}

		check(result.manifest !== undefined, "[9] TaskResult.manifest should be present");
		check(result.manifestPath !== undefined && existsSync(result.manifestPath!), "[9] manifest should be persisted");

		if (result.manifest) {
			const m = result.manifest;
			check(/^\d{8}T\d{4}-[0-9a-f]{4}$/.test(m.run_id), `[9] run_id format, got ${m.run_id}`);
			check(/^[0-9a-f]{12}$/.test(m.task.spec_hash), `[9] spec_hash format, got ${m.task.spec_hash}`);
			check(m.config.execute_model === MODEL, `[9] config.execute_model set, got ${m.config.execute_model}`);
			check(m.config.review_forked === true, "[9] review_forked should be true");
			check(m.phases.execute !== undefined && m.phases.execute.turns >= 1,
				`[9] phases.execute present with turns, got ${JSON.stringify(m.phases.execute)}`);
			check(m.phases.review !== null && m.phases.review !== undefined, "[9] phases.review should be present");
			if (m.phases.review) {
				check(m.phases.review.forked === true, "[9] review forked should be true");
				check(typeof m.phases.review.by_priority === "object", "[9] review by_priority should be an object");
				check(typeof m.phases.review.context_inherited_tokens === "number", "[9] context_inherited_tokens present");
			}
			check(m.totals.cost_usd > 0, `[9] totals.cost_usd should be > 0, got ${m.totals.cost_usd}`);
			check(typeof m.totals.read_duplication_tokens === "number", "[9] read_duplication_tokens present");
			check(m.totals.duration_ms > 0, `[9] totals.duration_ms should be > 0, got ${m.totals.duration_ms}`);
		}

		// Persisted file parses back to a manifest with the same run_id.
		if (result.manifestPath) {
			const onDisk = JSON.parse(readFileSync(result.manifestPath, "utf-8"));
			check(onDisk.run_id === result.manifest?.run_id, "[9] persisted manifest round-trips the run_id");
		}
	} finally {
		rmSync(dir, { recursive: true, force: true });
...
async function runTaskToolConversation(opts: {
	...
	prompt: string;
	budgetFlag?: string;
}): Promise<{ details: any | null; exitCode: number }> {
	const args = [
		"--mode", "json",
		"-e", TASK_EXTENSION_PATH,
...
async function section10TaskTool(): Promise<void> {
	...
	try {
		...
		const single = await runTaskToolConversation({ cwd: dir, prompt: SINGLE_TOOL_PROMPT, budgetFlag: "free" });
		check(single.exitCode === 0, `[10] conversational run should exit 0, got ${single.exitCode}`);
		check(single.details !== null, "[10] the model should have called the task tool");
		if (single.details) {
			check(single.details.success === true, `[10] task success, got ${JSON.stringify(single.details)}`);
			check(single.details.tests === "passing", `[10] tests passing, got ${single.details.tests}`);
			check(Array.isArray(single.details.commits) && single.details.commits.length > 0,
				"[10] commits should be non-empty");
			check(single.details.files_changed?.includes("hello.txt"),
				`[10] files_changed should include hello.txt, got ${JSON.stringify(single.details.files_changed)}`);
			check(single.details.verification?.passed === true, "[10] verification should have passed");
			// Phase 10: the manifest carries the resolved tier — the run is
			// budget-locked to free, so config.budget must say "free" (not
			// the "default" placeholder).
			check(single.details.metrics?.config?.budget === "free",
		...
		const par = await runTaskToolConversation({ cwd: dir, prompt: SUBSPECS_TOOL_PROMPT, budgetFlag: "free" });
		check(par.exitCode === 0, `[10b] sub_specs run should exit 0, got ${par.exitCode}`);
		check(par.details !== null, "[10b] the model should have called task with sub_specs");
		if (par.details) {
			check(par.details.success === true, `[10b] sub_specs success, got ${JSON.stringify(par.details)}`);
			check(par.details.conflicts !== undefined && par.details.conflicts.length === 0,
				`[10b] no merge conflicts expected, got ${JSON.stringify(par.details.conflicts)}`);
			const files: string[] = par.details.files_changed ?? [];
			check(files.includes("a.txt") && files.includes("b.txt"),
...
async function main(): Promise<void> {
	console.log(`test-e2e: real pi + real LLM (hardcoded model: ${MODEL} — the only model in this suite)`);
