...
export function runTests(): Promise<void> {
	...
	// ─── buildReviewPrompt ───
	{
		...
		check(
			prompt.includes("report_findings"),
			"prompt should instruct report_findings",
		);
		check(
			prompt.includes("## Spec") && prompt.includes("Do X"),
			"prompt should carry the spec",
		);
		check(
			prompt.includes("## Final diff") && prompt.includes("+line"),
			"prompt should carry the diff in a fence",
		);
		check(
			prompt.includes("## Worker summary") && prompt.includes("did the thing"),
			"prompt should carry the summary",
		);
		check(
			!prompt.toLowerCase().includes("deviation"),
			"no deviations section when deviations empty",
		);
	}
	{
		const prompt = buildReviewPrompt({
			...
			diff: "d",
			summary: "m",
			deviations: ["skipped R2", "used lib Y"],
		});
		check(
			prompt.includes("## Worker-declared deviations"),
			"deviations section present when non-empty",
		);
		check(
			prompt.includes("- skipped R2") && prompt.includes("- used lib Y"),
			"deviations listed",
		);
	}
	...
	// ─── mergeReviewOutcomes (parallel review axes) ───
	{
		...
		const single = mergeReviewOutcomes([
			{ result: mk({ verdict: "fix" }), usage: { cost_usd: 0.01 } },
		]);
		check(
			single.result.verdict === "fix" && single.costUsd === 0.01,
			"single axis passes through verdict + cost",
		);
		...
		const merged = mergeReviewOutcomes([
			...
			},
			{
				result: mk({
					verdict: "fix",
					findings: [finding("F2", "P1")],
					requirements: [
						{ id: "R1", status: "uncertain" },
						{ id: "R2", status: "unmet" },
					],
				}),
				usage: { cost_usd: 0.02 },
		...
		check(
			merged.result.findings.length === 2 &&
				merged.result.findings.some((f) => f.id === "F2"),
			"findings concatenated across axes",
		);
		check(
			merged.result.requirements.find((r) => r.id === "R1")?.status ===
				"uncertain",
			"R1 worst status (uncertain > met) wins",
		);
		check(
			merged.result.requirements.find((r) => r.id === "R2")?.status === "unmet",
			"R2 unmet survives",
		);
		check(
			Math.abs(merged.costUsd - 0.03) < 1e-9,
			"review cost summed across axes",
		);
		...
		check(
			empty.result.verdict === "ship" &&
				empty.result.findings.length === 0 &&
				empty.costUsd === 0,
			"empty outcomes → ship, no findings, zero cost",
		);
		console.log(
			"✓ mergeReviewOutcomes: verdict worst, findings concat, requirements worst-per-id, cost summed",
		);
	}

	{
		...
		if (ok.ok)
			check(ok.result === REVIEW, "settled result is the captured report");
	}
	{
		...
		if (!fail.ok) {
			check(
				fail.error.message.includes(
					"Reviewer exited (code 2) without reporting findings.",
				),
				`error includes exit code, got: ${fail.error.message}`,
			);
			check(
				fail.error.message.includes("stderr: boom"),
				`error includes stderr, got: ${fail.error.message}`,
			);
		}
	}
	{
		...
		if (!quiet.ok)
			check(
				...
				`empty stderr omitted, got: ${quiet.error.message}`,
			);
	}
	{
		...
		if (!long.ok) {
			...
			check(
				detail.length === 500,
				`stderr sliced to 500, got ${detail.length}`,
			);
		}
	}
	...
	//    review no-progress window is shorter than the worker's
	{
		...
		check(
			ordered,
			`watchdog ordering violated: first-call ${REVIEW_FIRST_EVENT_TIMEOUT_MS} < no-progress ${REVIEW_NO_PROGRESS_TIMEOUT_MS} < wall ${REVIEW_WALL_TIMEOUT_MS}`,
		);
		check(
	...
	{
		const msg = reviewNoProgressErrorMessage(
			REVIEW_NO_PROGRESS_TIMEOUT_MS,
			REVIEW_WALL_TIMEOUT_MS,
		);
		check(
			msg.includes("no progress"),
			`no-progress message names the cause, got: ${msg}`,
		);
		check(
			msg.includes("5m"),
			`no-progress message names the window, got: ${msg}`,
		);
		check(
			msg.includes("20m"),
			`no-progress message names the wall limit, got: ${msg}`,
		);
	}
	...
	{
		const msg = firstEventTimeoutErrorMessage(
			REVIEW_FIRST_EVENT_TIMEOUT_MS,
			REVIEW_WALL_TIMEOUT_MS,
		);
		check(
			msg.includes("first model call"),
			`first-event message names the stalled call, got: ${msg}`,
		);
		check(
			msg.includes("no events"),
			`first-event message names the cause, got: ${msg}`,
		);
		check(
			msg.includes("3m"),
			`first-event message names the deadline, got: ${msg}`,
		);
	}

	if (errors.length > 0) {
		return Promise.reject(
			new Error("test-review failed:\n  ✗ " + errors.join("\n  ✗ ")),
		);
	}
	console.log(
		"✓ review: buildReviewPrompt assembly + settleReview branches + watchdog decisions",
	);
