...
function testSkeletonAndCache(errors: string[]): void {
	...
	try {
		...
		check(
			!!calc && calc.lang === "typescript" && calc.symbols.includes("add"),
		...
		check(
			!!util &&
				util.lang === "python" &&
				util.symbols.includes("helper") &&
				util.symbols.includes("Helper"),
			`utils.py skeleton wrong: ${JSON.stringify(util)}`,
		);
		const gd = byPath.get("main.gd");
		check(
			!!gd && gd.lang === "gdscript" && gd.symbols.includes("_ready"),
		...
		const fakeMap: CodebaseMap = {
			...
			patterns: [],
			testLayout: "",
			files: buildSkeleton(dir),
		};
		saveMap(dir, fakeMap);
		const loaded = loadCachedMap(dir);
		check(
			!!loaded && loaded.treeHash === h3 && loaded.mode === "full",
			"cache roundtrip failed",
		);
		const cacheFile = mapCachePath(dir);
		check(existsSync(cacheFile), `cache file should exist at ${cacheFile}`);
		check(
			!existsSync(join(dir, ".pi")),
			"map cache must not create anything inside the project repo",
		);
		const tracked = execSync("jj file list", { cwd: dir, encoding: "utf8" });
		check(
			!tracked.includes("codebase-map"),
			"no cache files should be tracked in the repo",
		);
	} finally {
	...
	console.log(
		"✓ skeleton langs/symbols/binaries, hash stable+changes, cache outside the repo",
	);
}
...
function testCachePathDerivation(errors: string[]): void {
	...
	try {
		...
		check(
			pa.startsWith(root + "/"),
			`cache path ${pa} should be under ${root}`,
		);
		check(
			pb.startsWith(root + "/"),
			`cache path ${pb} should be under ${root}`,
		);
		check(
			pa === mapCachePath(a),
			"same project dir must map to the same cache path",
		);
		check(
			pa !== pb,
			"different project dirs must map to different cache paths",
		);
	} finally {
	...
	console.log(
		"✓ cache path: under agent-dir cache root, stable per dir, distinct across dirs",
	);
}
...
function testSlicingAndInjection(errors: string[]): void {
	...
	const files = [
		{
			...
			loc: 3,
			contentHash: "a",
			summary: "basic arithmetic operations (add)",
		},
		{
			...
			loc: 3,
			contentHash: "b",
			summary: "renders output to console",
		},
		{
			path: "src/storage.ts",
			lang: "typescript",
	...
	const map: CodebaseMap = {
		...
		patterns: ["ts modules"],
		testLayout: "tests/ via tests/run_all.sh",
		files,
	};

	const rel = sliceRelevant(
		map,
		"Add a multiply operation to the arithmetic module",
	);
	check(
		rel.some((f) => f.path === "src/calculator.ts"),
	...
	check(
		prompt.includes("Read the files above first"),
		"prompt missing guidance line",
	);
...
function testFormatMapOverview(errors: string[]): void {
	...
	check(
		overview.includes("Entry points: src/main.ts"),
		"overview missing entry points",
	);
	check(
		overview.includes("Patterns: ts modules; mvc"),
		"overview missing patterns",
	);
	check(
		overview.includes("Test layout: tests/ via npm test"),
		"overview missing test layout",
	);
	check(
		!overview.includes("- "),
		"overview should NOT include the file list (that is the codebase_map tool's job)",
	);

	// Empty map → empty string (callers skip the injection entirely).
	const empty = formatMapOverview({
		...map,
		entryPoints: [],
		patterns: [],
		testLayout: "",
	...
	check(
		empty === "",
		`empty map should produce "", got ${JSON.stringify(empty)}`,
	);

	console.log(
		"✓ formatMapOverview: entry points + patterns + test layout, no file list",
	);
}
...
function testParseAnnotation(errors: string[]): void {
	...
	check(
		fenced.entryPoints.length === 1 && fenced.entryPoints[0] === "src/main.ts",
		`fenced entryPoints wrong: ${JSON.stringify(fenced.entryPoints)}`,
	);
	check(
		fenced.patterns.length === 1 && fenced.patterns[0] === "ts modules",
		`fenced patterns wrong: ${JSON.stringify(fenced.patterns)}`,
	);
	check(
		fenced.files.length === 1 &&
			fenced.files[0]!.path === "a.ts" &&
			fenced.files[0]!.role === "core" &&
			fenced.files[0]!.summary === "does a",
		`fenced files wrong: ${JSON.stringify(fenced.files)}`,
	);

	// Braces-slicing: prose before/after the JSON object
	const sliced = parseAnnotation(
		'Sure, the map is {"entryPoints": ["x.ts"], "files": []} — hope that helps.',
	);
	check(
		sliced.entryPoints.length === 1 && sliced.entryPoints[0] === "x.ts",
	...
	check(
		partial.files.length === 1 &&
			partial.files[0]!.role === "" &&
			partial.files[0]!.summary === "",
		`partial file entry defaults wrong: ${JSON.stringify(partial.files)}`,
	);
	const nonArray = parseAnnotation(
		'{"entryPoints": "not-an-array", "files": "nope"}',
	);
	check(
		nonArray.entryPoints.length === 0 && nonArray.files.length === 0,
		"non-array fields should default to []",
	);
...
function testConfigLoader(errors: string[]): void {
	...
	check(
		defaults.injectWorkers === true,
		"default injectWorkers should be true",
	);
	check(defaults.sliceLimit === 15, "default sliceLimit should be 15");
	check(
		defaults.workflowContract === true,
		"workflowContract should default ON (missing key)",
	);
	...
	check(
		cfg.annotationModel === "test/model",
		`annotationModel should parse, got ${cfg.annotationModel}`,
	);
	...
	check(
		cfg.workflowContract === false,
		"the agent-dir config can disable the workflow contract",
	);
	...
	check(
		loadRepoMapConfig(onPath).workflowContract === true,
		"explicit workflow_contract = true parses",
	);
	const badPath = join(tmp, "repo-map-bad.toml");
	writeFileSync(badPath, '[injection]\nworkflow_contract = "on"\n');
	check(
		loadRepoMapConfig(badPath).workflowContract === true,
		"invalid workflow_contract degrades to the default ON",
	);
	...
	try {
		...
		check(
			invalid.mode === "full" && invalid.sliceLimit === 15,
			"invalid TOML should fall back to defaults",
		);
		check(
			warns.length === 1,
			`invalid TOML should warn exactly once, got ${warns.length}: ${warns.join("; ")}`,
		);
		...
		check(
			warns.length === 1,
			`missing file must NOT warn, got ${warns.length}`,
		);
	} finally {
	...
	console.log(
		"✓ config loader: defaults + temp-file parse, invalid warns once / missing silent",
	);
}
...
async function testSkeletonMode(errors: string[]): Promise<void> {
	...
	try {
		...
		check(
			fresh.mode === "skeleton",
			"fresh build should produce a skeleton map",
		);
		check(
			!fresh.files.some((f) => f.summary),
			"skeleton mode should have no summaries on fresh build",
		);
		check(
			fresh.entryPoints.length === 0,
			"skeleton mode should have no entryPoints on fresh build",
		);
		...
		check(
			hit.generated === "t0" && hit.mode === "full",
			"skeleton request should hit the cached full map",
		);
		...
		check(
			calc?.summary === "cached summary of src/calculator.ts",
		...
		check(
			renderer !== undefined && !renderer.summary,
			`edited file should have no summary in skeleton mode, got ${JSON.stringify(renderer?.summary)}`,
		);
		check(
			skel.entryPoints.length === 1 &&
				skel.entryPoints[0] === "src/calculator.ts",
	...
	console.log(
		"✓ skeleton mode: no summaries fresh, cache reuse, never annotates",
	);
}
...
async function testFailureSemantics(errors: string[]): Promise<void> {
	...
	try {
		...
		const loaded = loadCachedMap(dir);
		check(
			loaded !== null && loaded.mode === "skeleton",
		...
		// tree hash.
		check(
			isCacheUsableFor(loaded, "full", treeHash) === false,
			"full request must reject a skeleton cache (annotation retry)",
		);
		check(
			isCacheUsableFor(loaded, "skeleton", treeHash) === true,
			"skeleton request must accept a skeleton cache",
		);
		check(
			isCacheUsableFor(loaded, "full", "other-hash") === false,
			"tree-hash mismatch must invalidate the cache",
		);
		check(
			isCacheUsableFor({ ...skeletonMap, mode: "full" }, "full", treeHash) ===
				true,
			"full request must accept a full cache",
		);
		check(
			isCacheUsableFor(null, "full", treeHash) === false,
		...
		try {
			const failed = await buildMap(
				dir,
				{ mode: "full", model: "m" },
				{
					annotate: () => Promise.reject(new Error("boom")),
				},
			);
			check(
				failed.mode === "skeleton",
				`failed annotation must yield a skeleton map, got "${failed.mode}"`,
			);
			const persisted = loadCachedMap(dir);
			check(
				persisted !== null && persisted.mode === "skeleton",
				`failed annotation must persist a skeleton map, got ${persisted?.mode}`,
			);
			check(
				warns.length === 0,
				`annotation failure must be silent, got warns: ${warns.join("; ")}`,
			);
		} finally {
			console.warn = origWarn;
		}
		...
		check(
			retriedPersisted !== null && retriedPersisted.mode === "full",
	...
	console.log(
		"✓ failure semantics: skeleton round-trips; failed annotation persists skeleton, retry yields full",
	);
}
...
export async function runTests(): Promise<void> {
	const errors: string[] = [];
	console.log(
		"── test-repo-map: skeleton/hash/cache, slicing, annotation parse, config, skeleton mode, failure semantics ──",
	);
