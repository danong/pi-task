...
function testSpecParsing(errors: string[]): void {
	...
	check(
		good.requirements.length === 2,
		`expected 2 requirements, got ${good.requirements.length}`,
	);
	check(
		good.verification.length === 1,
		`expected 1 verification command, got ${good.verification.length}`,
	);
	...
	for (const [markdown, expected] of cases) {
		try {
			parseSpec(markdown);
			errors.push(
				`expected SpecError mentioning "${expected}" for: ${markdown.trim().split("\n")[0]}`,
			);
		} catch (e) {
			if (!(e instanceof SpecError))
				errors.push(`expected SpecError, got ${(e as Error).constructor.name}`);
			else if (!e.message.includes(expected))
				errors.push(
					`SpecError should mention "${expected}", got: ${e.message}`,
				);
		}
	}
...
function testSplitSpec(errors: string[]): void {
	...
	const p0 = parts[0];
	const p1 = parts[1];
	if (!p0 || !p1) throw new Error("splitSpec(2) must return 2 parts");

	check(
		p0.includes("R1: one") && p0.includes("R3: three"),
		`worker 0 should get R1, R3 (round-robin), got: ${p0}`,
	);
	check(
		p1.includes("R2: two") && p1.includes("R4: four"),
		`worker 1 should get R2, R4 (round-robin), got: ${p1}`,
	);
	check(
		!p0.includes("R2: two") && !p1.includes("R1: one"),
		"requirements must not leak across workers",
	);
	check(
		p0.includes("## Goal") && p0.includes("## Verification"),
		"sub-task should keep Goal and Verification sections",
	);
	check(
		p0.includes("## Scope") &&
			p0.toLowerCase().includes("implement only the requirements listed"),
...
function testAggregateSubSpecs(errors: string[]): void {
	...
	check(
		agg.requirements.length === 3,
		`expected 3 unioned requirements, got ${agg.requirements.length}`,
	);
	check(
		agg.verification.length === 2,
		`expected 2 unioned verification commands, got ${agg.verification.length}`,
	);
	check(
		agg.goal === "Create a.txt.",
		`goal should be the first sub-spec's, got "${agg.goal}"`,
	);
	check(
		agg.verification.includes("test -f a.txt") &&
			agg.verification.includes("test -f b.txt"),
	...
	try {
		...
		errors.push(
			"aggregateSubSpecs should reject a sub-spec without Verification",
		);
	} catch (e) {
		if (!(e instanceof SpecError))
			errors.push(`expected SpecError, got ${(e as Error).constructor.name}`);
		else if (!e.message.includes("sub_specs[1]"))
			errors.push(`error should name sub_specs[1], got: ${e.message}`);
	}
	...
	try {
		...
		errors.push(
			"aggregateSubSpecs should reject a sub-spec without Requirements",
		);
	} catch (e) {
		if (!(e instanceof SpecError))
			errors.push(`expected SpecError, got ${(e as Error).constructor.name}`);
		else if (!e.message.includes("sub_specs[0]"))
			errors.push(`error should name sub_specs[0], got: ${e.message}`);
	}
...
async function testRunVerification(errors: string[]): Promise<void> {
	...
	try {
		...
		check(
			...
			`expected 1 failure, got ${fail.failures.length}`,
		);
		const failF = fail.failures[0];
		if (!failF) throw new Error("exit-3 run must have a failure entry");
		check(failF.exitCode === 3, `exit code should be 3, got ${failF.exitCode}`);
		check(failF.command === "exit 3", "failure should carry the command");

		// Output captured (stdout + stderr joined)
		const noisy = await runVerification(
			["echo oops && echo more 1>&2 && exit 1"],
			dir,
			5000,
		);
		const noisyF = noisy.failures[0];
		if (!noisyF) throw new Error("failing run must have a failure entry");
		check(
			noisyF.exitCode === 1,
			`exit code should be 1, got ${noisyF.exitCode}`,
		);
		check(
			noisyF.output.includes("oops") && noisyF.output.includes("more"),
			`output should capture stdout+stderr, got: ${JSON.stringify(noisyF.output)}`,
		);
		...
		check(
			multi.failures.length === 2,
			`expected 2 failures, got ${multi.failures.length}`,
		);
		const multiF0 = multi.failures[0];
		const multiF1 = multi.failures[1];
		if (!multiF0 || !multiF1)
			throw new Error("multi-command run must have 2 failures");
		check(
			multiF0.exitCode === 7 && multiF1.exitCode === 1,
		...
		check(
			empty.passed === true && empty.commands === 0,
			"empty command list should pass",
		);

		// Timeout path: sleep killed → exit code 124
		const timeout = await runVerification(["sleep 5"], dir, 300);
		check(
			timeout.failures[0]?.exitCode === 124,
			`timeout should report exit code 124, got ${timeout.failures[0]?.exitCode}`,
		);
		check(
			timeout.timed_out === true,
			"timed_out flag set when a command hits its bound",
		);
		check(
			pass.timed_out === undefined || pass.timed_out === false,
			"timed_out not set on a clean pass",
		);
	} finally {
...
function testFixLoop(errors: string[]): void {
	...
	check(
		isBlocker({ priority: "P0" } as Finding) &&
			isBlocker({ priority: "P1" } as Finding),
		"P0/P1 are blockers",
	);
	check(
		!isBlocker({ priority: "P2" } as Finding) &&
			!isBlocker({ priority: "P3" } as Finding),
		"P2/P3 are not blockers",
	);
	check(
		blockersOf(reviewWith("P0", "P2", "P1", "P3")).length === 2,
		"blockersOf keeps only P0/P1",
	);
	...
	check(
		...
		"clean (pass, no findings) → ship",
	);
	check(
		decideFixLoop({
			testsPass: true,
			review: reviewWith("P2", "P3"),
			fixesUsed: 0,
			maxFixes: 2,
		}) === "ship",
		"pass + only P2/P3 → ship",
	);
	...
	check(
		...
		"failing tests + budget → fix",
	);
	check(
		decideFixLoop({
			testsPass: true,
			review: reviewWith("P1"),
			fixesUsed: 1,
			maxFixes: 2,
		}) === "fix",
		"P1 blocker + budget → fix",
	);
	...
	check(
		...
		"failing + no budget → escalate",
	);
	check(
		decideFixLoop({
			testsPass: true,
			review: reviewWith("P0"),
			fixesUsed: 2,
			maxFixes: 2,
		}) === "escalate",
		"P0 + no budget → escalate",
	);
	check(
		decideFixLoop({
			testsPass: false,
	...
	check(
		prompt.includes("## Spec") && prompt.includes("R1: x"),
		"fix prompt carries the spec",
	);
	check(
		prompt.includes("## Verification failures") &&
			prompt.includes("npm test") &&
			prompt.includes("FAIL boom"),
		"fix prompt carries failures",
	);
	check(
		prompt.includes("## Review findings") && prompt.includes("[P0]"),
		"fix prompt carries P0/P1 findings",
	);
	check(prompt.includes("yield()"), "fix prompt instructs yield");

	const noFail = buildFixPrompt({
		specMarkdown: "s",
		failures: [],
		findings: blockersOf(reviewWith("P1")),
	});
	check(
		!noFail.includes("Verification failures"),
		"no failures section when failures empty",
	);
	const noFind = buildFixPrompt({
		specMarkdown: "s",
		failures: [],
		findings: [],
	});
	check(
		!noFind.includes("Review findings"),
		"no findings section when findings empty",
	);
	...
	check(
		capped.includes("line 0") && capped.includes("more lines truncated"),
		"verbose output is capped with a pointer",
	);
	const short = capFixOutput("one line");
	check(
		short === "one line" && !short.includes("truncated"),
...
function testParseDiffStat(errors: string[]): void {
	...
	check(
		parseDiffStat(newFile).insertions === 2 &&
			parseDiffStat(newFile).deletions === 0,
		"new-file diff: only content lines count",
	);

	// Empty / header-only / binary diffs count nothing.
	check(
		parseDiffStat("").insertions === 0 && parseDiffStat("").deletions === 0,
		"empty diff → 0/0",
	);
	const headersOnly = "--- a/x\n+++ b/x\n@@ -1,1 +1,1 @@\n";
	check(
		parseDiffStat(headersOnly).insertions === 0 &&
			parseDiffStat(headersOnly).deletions === 0,
		"header-only diff → 0/0",
	);
	const binary =
		"diff --git a/img.png b/img.png\nnew file mode 100644\nindex 0000000..1111111\nBinary files differ\n";
	check(
		parseDiffStat(binary).insertions === 0 &&
			parseDiffStat(binary).deletions === 0,
		"binary diff → 0/0",
	);
	...
	check(
		ms.insertions === 3 && ms.deletions === 1,
		`multi-file sums (3/1), got ${ms.insertions}/${ms.deletions}`,
	);

	console.log(
		"✓ parseDiffStat: added/removed line counts (hunk headers excluded)",
	);
}
...
function testClassifyOverlapDiffs(errors: string[]): void {
	...
	check(
		classifyOverlapDiffs(commentDiffs) === "comment-only",
	...
	check(
		classifyOverlapDiffs(prefixDiffs) === "comment-only",
		"# -- ; comment prefixes → comment-only",
	);
	const wsDiffs = ["--- a/a.ts\n+++ b/a.ts\n-   \n+    \n"];
	check(
		classifyOverlapDiffs(wsDiffs) === "comment-only",
		"whitespace-only lines → comment-only",
	);
	check(
		classifyOverlapDiffs([]) === "comment-only",
		"empty diffs → comment-only (vacuous)",
	);
	...
	check(
		classifyOverlapDiffs(headerOnly) === "comment-only",
		"headers/context only → comment-only",
	);
	...
	check(
		classifyOverlapDiffs(codeDiffs) === "substantive",
		"one worker's code line → substantive",
	);
	check(
		classifyOverlapDiffs(["--- a/x\n+++ b/x\n+code()\n"]) === "substantive",
		"added code line → substantive",
	);
	check(
		classifyOverlapDiffs(["Binary files differ\n"]) === "substantive",
		"binary diff → substantive",
	);
	// A comment-only worker plus a code-changing worker → substantive.
	const mixedComment = commentDiffs[0];
	const mixedCode = codeDiffs[1];
	if (!mixedComment || !mixedCode)
		throw new Error("mixed fixture entries must exist");
	check(
		classifyOverlapDiffs([mixedComment, mixedCode]) === "substantive",
		"mixed comment + code → substantive",
	);

	console.log(
		"✓ classifyOverlapDiffs: comment/whitespace → union path, code/binary → substantive (R5)",
	);
}
...
function testNoYieldFailureClass(errors: string[]): void {
	...
	check(
		isNoYieldFailure(err),
		"abort error with code no_yield → no-yield class",
	);
	check(
		err.message.includes("turns: 11"),
		"abort error message is decorated (multi-line) — message matching cannot be the contract",
	);
	check(
		!isNoYieldFailure(new Error(NO_YIELD_FAILURE)),
	...
	check(
		...
		"other failure codes → not no-yield class",
	);
	check(
		isNoYieldFailure(
			buildAbortError({
		...
		"null-code external abort → not no-yield class",
	);
	console.log(
		"✓ isNoYieldFailure: structured diagnostics.cause matching (message-decorated abort rejections)",
	);
}
...
function testFinalizationIncomplete(errors: string[]): void {
	...
	check(
		isFinalizationIncomplete(null) === false,
		"null progress → not finalization-incomplete",
	);
	check(isFinalizationIncomplete({ done: 0, total: 3 }) === false, "0/3 → not");
	check(
		isFinalizationIncomplete({ done: 2, total: 3 }) === false,
		"2/3 (mid-work) → not",
	);
	check(
		isFinalizationIncomplete({ done: 0, total: 0 }) === false,
	...
	check(
		isFinalizationIncomplete({ done: 3, total: 3 }) === true,
		"3/3 → finalization-incomplete",
	);
	check(
		isFinalizationIncomplete({ done: 5, total: 3 }) === true,
	...
	check(
		classifyWorkerFailures([]) === "abort",
		"no failed workers → abort (vacuous)",
	);
	check(
		classifyWorkerFailures([null]) === "abort",
		"unclassified failed worker → abort",
	);
	check(
		classifyWorkerFailures([{ done: 1, total: 2 }]) === "abort",
		"mid-work failed worker → abort",
	);
	check(
		classifyWorkerFailures([
			{ done: 2, total: 2 },
	...
	check(
		classifyWorkerFailures([{ done: 2, total: 2 }, null]) === "abort",
	...
	console.log(
		"✓ finalization-incomplete classification: all-done-at-abort drives the merge attempt (R2)",
	);
}
...
function testRecoveryGuide(errors: string[]): void {
	...
	check(
		guide.includes("jj workspace list"),
		"guide names the workspace discovery command",
	);
	check(
		guide.includes("jj log -r all()"),
		"guide names the commit discovery command",
	);
	check(
		guide.includes("jj rebase -s"),
		"guide stacks the workspaces with jj rebase",
	);
	check(
		guide.includes("jj squash --from"),
		"guide squashes the stacked commits into the base",
	);
	check(
		guide.includes("Re-resolve ids AFTER every command"),
		"guide warns about stale ids after rebase",
	);
	...
	check(
		guide.includes("description-less"),
		"guide warns that description-less commits refuse push",
	);
	check(guide.includes("jj abandon"), "guide abandons the AI base/stubs");
	check(
		guide.includes("empty description"),
		"guide tells how to verify no empty descriptions remain",
	);

	// Add-vs-delete conflict warning (:ours/:theirs, not mid-stack abandon).
	check(
		guide.includes("Add-vs-delete"),
		"guide warns about add-vs-delete conflicts",
	);
	check(
		guide.includes(":ours") && guide.includes(":theirs"),
		"guide resolves via :ours/:theirs",
	);
	check(
		guide.includes("mid-stack abandon"),
		"guide warns against mid-stack abandon",
	);

	// The rescued uncommitted state is named where it lives.
	check(
		guide.includes("bbbb") && guide.includes("pi-task-ab1-0"),
		"guide names the rescue commit + workspace",
	);

	console.log(
		"✓ recovery guide: stacking + stub-abandon + add-vs-delete warnings (R4)",
	);
}
...
function testReviewGate(errors: string[]): void {
	...
	if (!analysisShape)
		throw new Error("DEFAULT_TASK_SHAPES.analysis must exist");

	// Analysis shape (review: []): NEVER forks — persona, flag, or both.
	{
		const none = resolveReviewGate({}, analysisShape);
		const flag = resolveReviewGate({ review: true }, analysisShape);
		const persona = resolveReviewGate(
			{ persona: "adversarial" },
			analysisShape,
		);
		const both = resolveReviewGate(
			{ review: true, persona: "adversarial" },
			analysisShape,
		);
	...
	// Code shape (declares axes): the flag or a persona forks; nothing → no review.
	{
		...
		const none = resolveReviewGate({}, codeShape);
		check(
			none.enabled === false &&
		...
		const flag = resolveReviewGate({ review: true }, codeShape);
		check(
			flag.enabled === true && flag.skipped === false,
			"code + review flag: forks",
		);
		const persona = resolveReviewGate({ persona: "adversarial" }, codeShape);
		check(
			persona.enabled === true && persona.skipped === false,
			"code + persona: forks",
		);
		const off = resolveReviewGate({ review: false }, codeShape);
		check(
			off.enabled === false && off.requested === false,
			"code + explicit false: not requested",
		);
	}
	...
	// with axes still forks; the batch shape (no axes) never does.
	{
		const codeShape = DEFAULT_TASK_SHAPES.code;
		if (!codeShape) throw new Error("DEFAULT_TASK_SHAPES.code must exist");
		const custom = { ...codeShape, review: ["adversarial"] };
		check(
			resolveReviewGate({ review: true }, custom).enabled === true,
			"custom shape declaring axes still forks",
		);
		check(
			resolveReviewGate({ persona: "adversarial" }, custom).enabled === true,
			"custom shape declaring axes forks on a persona too",
		);
		const batchShape = DEFAULT_TASK_SHAPES.batch;
		if (!batchShape) throw new Error("DEFAULT_TASK_SHAPES.batch must exist");
		check(
			resolveReviewGate({ review: true, persona: "adversarial" }, batchShape)
				.enabled === false,
	...
	console.log(
		"✓ resolveReviewGate: axes are the fork precondition (analysis never forks; code/custom-axes do)",
	);
}
...
function testReviewAxes(errors: string[]): void {
	...
	// regardless of how many axes the shape declares.
	{
		...
		check(
			JSON.stringify(names(axes)) === JSON.stringify(["adversarial"]),
			`default code shape must fork ONE adversarial reviewer, got ${JSON.stringify(names(axes))}`,
		);
		check(
			axes[0] === DEFAULT_PERSONA,
			"the default fork IS the DEFAULT_PERSONA",
		);
		check(
			axes.length === 1 && axes[0]?.output.kind === "findings",
			"exactly one findings fork",
		);
		// Undefined shape → the code default, still one adversarial fork.
		check(
			JSON.stringify(names(resolveReviewAxes(undefined, undefined))) ===
	...
	{
		const axes = resolveReviewAxes(
			PARALLEL_REVIEW_PERSONA,
			DEFAULT_TASK_SHAPES.code,
		);
		check(
			JSON.stringify(names(axes)) ===
		...
		const axeslessShape = DEFAULT_TASK_SHAPES.code;
		if (!axeslessShape) throw new Error("DEFAULT_TASK_SHAPES.code must exist");
		const emptyAxes = resolveReviewAxes(PARALLEL_REVIEW_PERSONA, {
			...axeslessShape,
			review: [],
		});
		check(
			JSON.stringify(names(emptyAxes)) === JSON.stringify(["adversarial"]),
		...
		check(
			getPersona(PARALLEL_REVIEW_PERSONA) === undefined,
			'"parallel" is not a registered persona',
		);
	}
	...
	// A single named persona → exactly that one axis (never the full set).
	{
		...
		check(
			JSON.stringify(names(adv)) === JSON.stringify(["adversarial"]),
		...
		check(
			JSON.stringify(names(arch)) === JSON.stringify(["architecture"]),
		...
		const unknown = resolveReviewAxes(
			"not-a-persona",
			DEFAULT_TASK_SHAPES.code,
		);
		check(
			JSON.stringify(names(unknown)) === JSON.stringify(["adversarial"]),
	...
	// orchestrator uses for personas: effectiveAxes.map((p) => p.name)).
	{
		check(
			JSON.stringify(
				names(resolveReviewAxes(undefined, DEFAULT_TASK_SHAPES.code)),
			) === JSON.stringify(["adversarial"]),
		...
		check(
			JSON.stringify(
				names(
					resolveReviewAxes(PARALLEL_REVIEW_PERSONA, DEFAULT_TASK_SHAPES.code),
				),
			) === JSON.stringify(["standards", "spec-fidelity", "architecture"]),
	...
	console.log(
		"✓ resolveReviewAxes: default = one adversarial fork; 'parallel' = full axis set; named = one axis",
	);
}
...
function testVerificationLifecycle(errors: string[]): void {
	...
	check(
		outputSignature(long).length === BASELINE_SIGNATURE_CAP,
		"long output → capped",
	);
	...
	const entry = (
		exitCode: number,
		signature: string,
	): VerificationBaselineEntry => ({ command: "c", exitCode, signature });
	check(
		isBrokenVerificationCommand(entry(127, "not found")) === true,
		"exit 127 (command not found) → broken",
	);
	check(
		isBrokenVerificationCommand(
			entry(2, "bash: line 1: syntax error near unexpected token"),
		) === true,
		"exit 2 + syntax error → broken",
	);
	check(
		isBrokenVerificationCommand(entry(2, "grep: no match")) === false,
		"exit 2 without syntax error → NOT broken",
	);
	check(
		isBrokenVerificationCommand(entry(1, "FAIL some test")) === false,
		"exit 1 (legit red) → NOT broken",
	);
	check(
		isBrokenVerificationCommand(entry(0, "")) === false,
		"exit 0 → NOT broken",
	);
	...
	check(
		split.specDefectSuspected.length === 1 &&
			split.specDefectSuspected[0]?.command === "badgrep",
		`baseline-identical failure → spec defect, got ${JSON.stringify(split.specDefectSuspected.map((f) => f.command))}`,
	);
	check(
		split.actionable.length === 2,
		`the rest are actionable, got ${split.actionable.length}`,
	);
	// A failure whose OUTPUT changed vs baseline is actionable (the change mattered).
	const split2 = classifyVerificationFailures(
		[{ command: "badgrep", exitCode: 1, output: "now it matches something" }],
		baseline,
	);
	check(
		split2.actionable.length === 1 && split2.specDefectSuspected.length === 0,
		"output changed vs baseline → actionable",
	);
}

async function testVerificationBaselineCapture(
	errors: string[],
): Promise<void> {
	const check = (cond: boolean, msg: string): void => {
		if (!cond) errors.push(msg);
	...
	try {
		...
		const healthy0 = baseline[0];
		const healthy1 = baseline[1];
		if (!healthy0 || !healthy1)
			throw new Error("baseline must record both commands");
		check(
			healthy0.exitCode === 1,
			`legit red recorded (test -f missing), got exit ${healthy0.exitCode}`,
		);
		check(healthy1.exitCode === 0, "green recorded");

		// A broken command (exit 127) rejects the dispatch BEFORE any worker.
		let threw = "";
		try {
			await captureVerificationBaseline(
		...
		check(
			threw.includes("broken") &&
				threw.includes("definitely-not-a-real-cmd-xyz"),
		...
		check(
			threw2.includes("broken"),
			`syntax error rejects dispatch, got: ${threw2.slice(0, 80)}`,
		);
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
...
function testDisputeAdjudication(errors: string[]): void {
	...
	check(
		JSON.stringify(adjudicated.upheld) === JSON.stringify(["badgrep"]),
		`only the baseline-identical dispute is upheld, got ${JSON.stringify(adjudicated.upheld)}`,
	);
	check(
		adjudicated.rejected.length === 2 &&
			adjudicated.rejected.some((d) => d.command === "flaky"),
		`differs-from-baseline and non-failing disputes are rejected, got ${JSON.stringify(adjudicated.rejected.map((d) => d.command))}`,
	);
	check(
		adjudicateDisputes([], failures, baseline).upheld.length === 0,
		"no disputes → nothing upheld",
	);
}
...
function testTurnBudget(errors: string[]): void {
	...
	check(
		decideTurnBudgetAction(500, undefined, false) === "none",
		"no budget → none",
	);
...
export async function runTests(): Promise<void> {
	const errors: string[] = [];
	console.log(
		"── test-orchestrator: spec parse + splitSpec + aggregateSubSpecs + runVerification + fix loop ──",
	);
