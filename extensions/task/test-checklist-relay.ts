...
function makeFakeSession(): FakeSession {
	...
	const session = {
		...
		setModel(): Promise<Record<string, unknown>> {
...
export function runTests(): Promise<void> {
	...
	// 1. Reducer: init → total known; done/status fold into done count
	{
		...
		const i2 = reduceChecklistRelayEvent(
			state,
			end("c1", { items: ["a", "b", "c", "d", "e"] }),
		);
		check(
			i2.update !== null && i2.update.done === 0 && i2.update.total === 5,
			`init should report 0/5, got ${JSON.stringify(i2.update)}`,
		);
		check(
			state.total === 5 && state.remaining === 5,
			"state carries total/remaining after init",
		);

		const d1 = reduceChecklistRelayEvent(state, start("c2", "done"));
		check(d1.update === null, "done start emits nothing");
		const d2 = reduceChecklistRelayEvent(
			state,
			end("c2", { index: 0, remaining: 4 }),
		);
		check(
			d2.update !== null && d2.update.done === 1 && d2.update.total === 5,
		...
		const s2 = reduceChecklistRelayEvent(
			state,
			end("c3", { remaining: 2, items: ["c", "d"] }),
		);
		check(
			s2.update !== null && s2.update.done === 3 && s2.update.total === 5,
			`status should report 3/5, got ${JSON.stringify(s2.update)}`,
		);

		reduceChecklistRelayEvent(state, start("c4", "status"));
		const s4 = reduceChecklistRelayEvent(state, end("c4", { remaining: 0 }));
		check(
			s4.update !== null && s4.update.done === 5 && s4.update.total === 5,
	...
	// 2. Reducer: non-checklist tools, errored calls, and duplicate marks are ignored
	{
		...
		const irrelevant = reduceChecklistRelayEvent(state, {
			type: "tool_execution_end",
			toolName: "edit",
			toolCallId: "r2",
		});
		check(irrelevant.update === null, "non-checklist tools emit nothing");
		const noCallId = reduceChecklistRelayEvent(state, {
			type: "tool_execution_end",
			toolName: "checklist",
		});
		check(
			noCallId.update === null,
			"checklist end without toolCallId emits nothing",
		);

		const bad = reduceChecklistRelayEvent(
			state,
			end("r3", { index: 99, remaining: 2 }, true),
		);
		check(bad.update === null, "errored checklist call emits nothing");

		reduceChecklistRelayEvent(state, start("r4", "done"));
		const dup2 = reduceChecklistRelayEvent(
			state,
			end("r4", { index: 0, alreadyDone: true }),
		);
		check(
			dup2.update === null,
			"duplicate mark (alreadyDone, no remaining) emits nothing",
		);

		const orphan = reduceChecklistRelayEvent(
			state,
			end("r5", { remaining: 1 }),
		);
		check(
			orphan.update === null,
			"end without a correlated start emits nothing",
		);
	...
	{
		const state = createChecklistRelayState();
		reduceChecklistRelayEvent(state, start("s1", "done"));
		const stray2 = reduceChecklistRelayEvent(
			state,
			end("s1", { index: 0, remaining: 1 }),
		);
		check(
			stray2.update === null && state.total === null,
			"remaining without known total emits nothing",
		);

		reduceChecklistRelayEvent(state, start("s2", "init"));
		reduceChecklistRelayEvent(state, end("s2", { items: ["a", "b", "c"] }));
		reduceChecklistRelayEvent(state, start("s3", "init"));
		const reinit2 = reduceChecklistRelayEvent(
			state,
			end("s3", { items: ["x"] }),
		);
		check(
			reinit2.update !== null &&
				reinit2.update.total === 1 &&
				reinit2.update.done === 0,
	...
	// 4. Attachment: event stream through the fake session drives onChecklist
	{
		...
		const ctrl = attachChecklistRelay(fake.session, {
			onChecklist: (p) => updates.push(p),
		});
		...
		check(
			updates[0]?.done === 0 && updates[0]?.total === 4,
			`init update should be 0/4, got ${JSON.stringify(updates[0])}`,
		);
		...
		check(
			updates.length === 3,
			`three updates after two done calls, got ${updates.length}`,
		);
		check(
			updates[2]?.done === 2,
			`latest update should be 2/4, got ${JSON.stringify(updates[2])}`,
		);
	...
	// 5. Observer-only: the relay never sends commands or requests
	{
		...
		check(
			...
			`relay must be observer-only, sent ${JSON.stringify(fake.commands)}`,
		);
	...
	if (errors.length > 0) {
		return Promise.reject(
			new Error("test-checklist-relay failed:\n  ✗ " + errors.join("\n  ✗ ")),
		);
	}
	console.log(
		"✓ reducer reconstruction + observer-only attachment, detach, zero commands",
	);
	return Promise.resolve();
}
