...
import { BUDGET_TIERS, DEFAULT_BUDGET_TIERS, type BudgetTierConfig } from "./config.ts";
...
function summary(rows: RunRow[]): RunSummary {
	const sorted = [...rows].sort((a, b) => (a.runId < b.runId ? -1 : a.runId > b.runId ? 1 : 0));
	return {
		rows: sorted,
		count: rows.length,
...
export async function runTests(): Promise<void> {
	...
	{
		check(BENCH_SPECS.length >= 2 && BENCH_SPECS.length <= 3,
		...
		for (const spec of BENCH_SPECS) {
			...
			check(parsed.requirements.length >= 1,
				`[${spec.id}] should have >= 1 requirement`);
			check(parsed.verification.length >= 1,
			...
			for (const [tier, b] of Object.entries(spec.baseline)) {
				check(b.durationMs > 0 && b.durationMs <= 60_000,
	...
	// ─── buildBenchPlan: all tiers, totals, per-run expectations ───
	{
		...
		for (const t of plan.tiers) {
			check(t.runs.length === BENCH_SPECS.length, `tier ${t.tier} should plan every spec`);
			for (const r of t.runs) {
				const b = baselineFor(BENCH_SPECS.find((s) => s.id === r.specId)!, t.tier);
				check(r.expectedDurationMs === b.durationMs && r.expectedCostUsd === b.costUsd,
					`[${t.tier} × ${r.specId}] plan expectations match the baseline`);
				expectedTotal += b.durationMs;
			}
			check(t.expectedDurationMs === t.runs.reduce((a, r) => a + r.expectedDurationMs, 0),
				`tier ${t.tier} expected duration is the sum of its runs`);
		}
		check(plan.totalExpectedDurationMs === expectedTotal, "total expected duration = sum of all run baselines");
		check(plan.totalExpectedCostUsd === plan.tiers.reduce((a, t) => a + t.expectedCostUsd, 0),
	...
	// ─── buildBenchPlan: tier filter ───
	{
		...
		check(plan.tiers.length === 1 && plan.tiers[0].tier === "full", "filter keeps only the named tier");
		check(plan.totalRuns === BENCH_SPECS.length, `filtered totalRuns, got ${plan.totalRuns}`);

		const none = buildBenchPlan({ tiers: DEFAULT_BUDGET_TIERS, tierOrder: [...BUDGET_TIERS], tierFilter: ["nope"] });
		check(none.tiers.length === 0 && none.totalRuns === 0, "unknown tier filter → empty plan");
	}
	...
	// ─── buildBenchPlan: custom tiers fall back to the default baseline ───
	{
		...
		check(full.runs[0].expectedDurationMs === 8_000 && full.runs[0].expectedCostUsd === 0.002,
			"known tier uses its own baseline entry");
		check(custom.runs[0].expectedDurationMs === 10_000 && custom.runs[0].expectedCostUsd === 0.001,
			"custom tier without a baseline entry falls back to default");
	}

	// ─── projectForSpec ───
	{
		check(projectForSpec("bench-", "hello") === "bench-hello", "project naming");
	...
	// ─── compareToBaselines: no data yet → runs 0, no regressions ───
	{
		...
		const rows = compareToBaselines({ plan, projectPrefix: "bench-", summaries: {} });
		check(rows.length === BENCH_SPECS.length, "one row per planned run");
		check(rows.every((r) => r.runs === 0), "no summaries → runs 0");
		check(rows.every((r) => r.durationRatio === 0 && r.costRatio === 0), "no data → ratios 0");
		check(rows.every((r) => r.regressions.length === 0), "no data → no regressions");
		check(rows.every((r) => r.latest === null), "no data → latest null");
	}
	...
	// ─── compareToBaselines: averages, ratios, tier matching ───
	{
		...
		const summaries = {
			"bench-hello": summary([
				runRow({ runId: "20260805T0001-abcd", durationMs: baseline.durationMs, costUsd: baseline.costUsd }),
				// A same-tier second run (repeat bench) → averaged in. 3x duration →
				// avg 2x baseline, which exceeds the 1.5x duration threshold.
				runRow({ runId: "20260805T0002-abcd", durationMs: baseline.durationMs * 3, costUsd: baseline.costUsd }),
		...
		check(row.latest?.runId === "20260805T0002-abcd", "latest = most recent matching run");
		const avgDur = (baseline.durationMs + baseline.durationMs * 3) / 2;
		check(Math.abs(row.avgDurationMs - avgDur) < 1e-9, `avg duration, got ${row.avgDurationMs}`);
		check(Math.abs(row.durationRatio - avgDur / baseline.durationMs) < 1e-9, "duration ratio = avg/baseline");
		check(row.regressions.length === 1 && row.regressions[0].startsWith("duration"),
		...
		const rows = compareToBaselines({
			...
			summaries: {
				"bench-custom-spec": summary([
					...
					runRow({ project: "bench-custom-spec", runId: "20260805T0001-abcd", durationMs: b.durationMs * 3, costUsd: b.costUsd * 5 }),
					runRow({ project: "bench-custom-spec", runId: "20260805T0002-abcd", durationMs: b.durationMs, costUsd: b.costUsd, verifyPassed: false }),
				]),
			},
		});
		const row = rows[0];
		check(row.runs === 2 && row.verifyPassRate === 0.5, `verifyPassRate, got ${row.verifyPassRate}`);
		check(Math.abs(row.costRatio - 3) < 1e-9, `cost ratio, got ${row.costRatio}`);
		check(row.regressions.some((r) => r.startsWith("cost")), "3x cost trips the cost regression");
		check(row.regressions.some((r) => r.startsWith("verify failed (1/2")), "verify failure is a regression");
		check(row.regressions.some((r) => r.startsWith("duration")), "2x duration trips the duration regression");
		check(REGRESSION_DURATION_FACTOR === 1.5 && REGRESSION_COST_FACTOR === 2.0,
	...
	// ─── rendering ───
	{
		...
		check(planText.includes("planned runs") && planText.includes(String(plan.totalRuns)),
			"plan lists the run count");
		check(planText.includes("estimated:"), "plan lists the estimate line");
		for (const r of plan.tiers[0].runs) {
			check(new RegExp(`${r.tier}\\s+×\\s+${r.specId}`).test(planText), `plan lists ${r.tier} × ${r.specId}`);
		}

		const rows: BenchComparisonRow[] = [
			{
				specId: "hello", tier: "full", runs: 0, latest: null,
				avgDurationMs: 0, avgCostUsd: 0,
				expectedDurationMs: 30_000, expectedCostUsd: 0.002,
				durationRatio: 0, costRatio: 0, verifyPassRate: 0, regressions: [],
			},
			{
				specId: "hello", tier: "full", runs: 1,
				latest: { runId: "20260805T0001-abcd", durationMs: 60_000, costUsd: 0.004, verifyPassed: true },
				avgDurationMs: 60_000, avgCostUsd: 0.004,
				expectedDurationMs: 30_000, expectedCostUsd: 0.002,
				durationRatio: 2, costRatio: 2, verifyPassRate: 1,
		...
		check(reportText.includes("⚠") && reportText.includes("regression thresholds"), "regressions flagged + threshold note");
		check(reportText.includes("1 regression(s)"), "report headline counts regressions");
	}

	// ─── parseBenchArgs ───
	{
		const plain = parseBenchArgs([]);
		check(plain.tierFilter.length === 0 && !plain.dryRun && !plain.help, "empty argv → defaults");
		const full = parseBenchArgs(["--tier", "full", "--dry-run"]);
		check(full.tierFilter.length === 1 && full.tierFilter[0] === "full" && full.dryRun, "--tier + --dry-run parsed");
		const repeat = parseBenchArgs(["--tier", "full", "--tier", "free"]);
		check(JSON.stringify(repeat.tierFilter) === JSON.stringify(["full", "free"]), "--tier repeatable");
		const metrics = parseBenchArgs(["--metrics-dir", "/tmp/x", "--help"]);
		check(metrics.metricsDir === "/tmp/x" && metrics.help, "--metrics-dir + --help parsed");
		let threw = false;
		try {
	...
	if (errors.length > 0) {
		throw new Error("test-bench-regression failed:\n  ✗ " + errors.join("\n  ✗ "));
	}
	console.log("✓ bench-regression: canned specs, plan assembly, baseline comparison, rendering, args");
}
