 ...
 * worker/reviewer subprocess (via --extension). When
 * PI_TASK_ENABLE_SESSION_ID=1 it injects a top-level `session_id` string
 * into each outgoing provider payload so a run's traffic is
 * attributable/correlatable downstream (OpenRouter dashboard) — which
 * cheapens debugging by grouping a run's requests. Disabled by default
 ...
 *      worker/reviewer spawns when enabled, so every subprocess of a run shares one id
 ...
 * truncated); the payload otherwise passes through unchanged. No logs.
 *
 * No enable flag or no identifier → the extension is a strict no-op
 * (zero overhead, zero injection). The field lands only under
 * before_provider_request — it never enters prompt text, so the
 * deterministic-prefix/cache rule is preserved.
 */
...
/** Enable flag — injection only when PI_TASK_ENABLE_SESSION_ID=1 (disabled by default). */
export const ENABLE_SESSION_ID_ENV_VAR = "PI_TASK_ENABLE_SESSION_ID";
...
export default function (pi: ExtensionAPI) {
	pi.on(
		...
		(event: { payload: unknown }, ctx: ExtensionContext) => {
			if (process.env[ENABLE_SESSION_ID_ENV_VAR] !== "1") return;
			...
			if (sessionId.length > SESSION_ID_MAX_LENGTH) return; // dropped, never truncated
