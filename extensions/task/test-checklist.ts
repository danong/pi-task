...
export function runTests(): Promise<void> {
	...
	{
		const capped = createChecklistState(
			Array.from({ length: 15 }, (_, i) => `item ${i}`),
		);
		check(
			capped.items.length === 12,
			`should truncate to 12 items, got ${capped.items.length}`,
		);
		check(
			capped.items.every((i) => !i.done),
			"fresh items should all be undone",
		);
		check(
			capped.items[0]!.text === "item 0" &&
				capped.items[11]!.text === "item 11",
			"truncation should keep the first items",
		);

		const small = createChecklistState(["a", "b"]);
		check(
			small.items.length === 2,
			`should keep 2 items, got ${small.items.length}`,
		);

		const custom = createChecklistState(["a", "b", "c"], 2);
		check(
			custom.items.length === 2,
			`maxItems override should cap at 2, got ${custom.items.length}`,
		);
	}
	...
	// 2. markChecklistDone happy path
	{
		...
		if (result.ok) {
			...
			check(
				result.remaining === 2,
				`remaining should be 2, got ${result.remaining}`,
			);
		}
	...
	// 3. Duplicate mark → ok:false, state unchanged
	{
		...
		if (!dup.ok) {
			check(
				dup.error.includes("already done"),
				`dup error should mention "already done", got: ${dup.error}`,
			);
		}
		check(
			state.items[0]!.done === true && checklistRemaining(state) === 1,
			"dup mark must not change state",
		);
	}
	...
	// 4. Out-of-range marks → ok:false with index guidance
	{
		...
		for (const bad of [-1, 3, 99]) {
			...
			if (!result.ok) {
				check(
					result.error.includes(`Index ${bad} invalid (expected 0..2)`),
					`error should guide the caller, got: ${result.error}`,
				);
				check(
					result.alreadyDone === false,
					`index ${bad} must not be flagged alreadyDone`,
				);
			}
		}
		check(
			checklistRemaining(state) === 3,
			"no out-of-range mark may change state",
		);
	}
	...
	// 5. checklistRemaining
	{
		...
		check(
			checklistRemaining(state) === 3,
			`fresh remaining should be 3, got ${checklistRemaining(state)}`,
		);
		markChecklistDone(state, 0);
		markChecklistDone(state, 1);
		check(
			checklistRemaining(state) === 1,
			`remaining should be 1, got ${checklistRemaining(state)}`,
		);
	}
	...
	// 6. checklistStatusText format
	{
		...
		markChecklistDone(state, 1);
		check(
			checklistStatusText(state) === "remaining:1",
			`status text should be "remaining:1", got ${checklistStatusText(state)}`,
		);
		check(
			checklistStatusText(createChecklistState([])) === "remaining:0",
			"empty checklist should report remaining:0",
		);
	}
	...
	// 8. shouldInjectChecklistReminder matrix
	{
		...
		check(
			shouldInjectChecklistReminder(false, remaining) === false,
			"no first edit → no injection",
		);
		check(
			shouldInjectChecklistReminder(true, null) === false,
			"null state → no injection",
		);
		check(
			shouldInjectChecklistReminder(true, done) === false,
			"zero remaining → no injection",
		);
		check(
			shouldInjectChecklistReminder(false, null) === false,
			"no first edit + null state → no injection",
		);
		check(
			shouldInjectChecklistReminder(true, remaining) === true,
			"first edit + remaining items → inject",
		);
	}

	if (errors.length > 0) {
		return Promise.reject(
			new Error("test-checklist failed:\n  ✗ " + errors.join("\n  ✗ ")),
		);
	}
	console.log(
		"✓ truncation, mark/dup/out-of-range, remaining, status, reminder, inject gate",
	);
