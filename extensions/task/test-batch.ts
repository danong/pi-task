...
function jj(args: string[], cwd: string): string {
	return execFileSync("jj", args, { cwd, encoding: "utf8", stdio: ["pipe", "pipe", "pipe"], env: jjEnv() });
}
...
const BATCH_SHAPE = DEFAULT_TASK_SHAPES.batch;

function specWith(requirements: string, verification: string): Spec {
	return parseSpec(`## Goal\nG\n## Requirements\n${requirements}\n## Verification\n- ${verification}\n`);
}
...
function latestBatchState(metricsDir: string, project: string): BatchJobState | null {
	const projDir = join(metricsDir, project);
	if (!existsSync(projDir)) return null;
	const files = readdirSync(projDir).filter((n) => n.endsWith(".batch.json")).sort();
	if (files.length === 0) return null;
	return JSON.parse(readFileSync(join(projDir, files[files.length - 1]), "utf-8")) as BatchJobState;
}
...
function testContracts(errors: string[]): void {
	...
	// passes through.
	check(stripCodeFences('```json\n{"a": 1}\n```') === '{"a": 1}', "json fence stripped");
	check(stripCodeFences('```\nplain\n```') === "plain", "bare fence stripped");
	check(stripCodeFences("  plain text  ") === "plain text", "plain text trimmed only");

	// text contract: non-empty passes, empty fails.
	check(validateBatchOutput({ kind: "text" }, "hello").ok === true, "text contract accepts non-empty");
	check(validateBatchOutput({ kind: "text" }, "   ").ok === false, "text contract rejects blank");

	// json contract: any parseable JSON passes.
	check(validateBatchOutput({ kind: "json" }, '[1, 2]').ok === true, "json contract accepts arrays");
	check(validateBatchOutput({ kind: "json" }, "not json").ok === false, "json contract rejects malformed");
	const fenced = validateBatchOutput({ kind: "json" }, '```json\n{"ok": true}\n```');
	check(fenced.ok === true && (fenced as { value: unknown }).value !== undefined,
		"json contract accepts fenced output");

	// json_object contract: object + required keys; arrays and scalars fail.
	const obj = { kind: "json_object", requiredKeys: ["a", "b"] } as const;
	check(validateBatchOutput(obj, '{"a": 1, "b": 2}').ok === true, "json_object accepts a complete object");
	const missing = validateBatchOutput(obj, '{"a": 1}');
	check(missing.ok === false && (missing as { error: string }).error.includes("b"),
		`json_object rejects missing keys (${(missing as { error: string }).error})`);
	check(validateBatchOutput(obj, "[1, 2]").ok === false, "json_object rejects arrays");
	check(validateBatchOutput(obj, '"str"').ok === false, "json_object rejects scalars");
	check(validateBatchOutput({ kind: "json_object" }, "{}").ok === true,
		"json_object without requiredKeys accepts any object");

	console.log("✓ batch contracts: fence stripping + text/json/json_object validation");
}
...
function testItemBuilding(errors: string[]): void {
	...
	check(requirementId("R1: create x", 0) === "R1", `requirementId R1, got ${requirementId("R1: create x", 0)}`);
	check(requirementId("R12. do y", 1) === "R12", "requirementId R12");
	check(requirementId("1. do y", 0) === "req-1", "requirementId falls back to req-<index>");

	// buildBatchPrompt: self-contained single-turn with the output contract.
	const prompt = buildBatchPrompt("Create x.", "R1: create x");
	check(prompt.includes("## Goal") && prompt.includes("Create x."), "prompt carries the goal");
	check(prompt.includes("## Requirement") && prompt.includes("R1: create x"), "prompt carries the requirement");
	check(prompt.includes("Output contract") && prompt.includes('"files"') && prompt.includes("single JSON object"),
		"prompt embeds the typed output contract");
	check(prompt.includes("no tools") && prompt.includes("single-turn"), "prompt declares single-turn/no-tools");
	...
	check(items[0].customId === "R1" && items[1].customId === "R2", "customIds are the R-ids");
	check(items.every((i) => i.contract.kind === "json_object" && i.prompt.length > 0),
	...
	const ok = extractBatchFiles(
		{ requirement: "R1", files: [{ path: "a/b.txt", content: "hi" }], summary: "s" },
		"R1",
	);
	check(ok.length === 1 && ok[0].path === "a/b.txt" && ok[0].content === "hi", "valid files extracted");
	const bad = (value: unknown, needle: string): boolean => {
		try {
			extractBatchFiles(value, "R1");
			return false;
		} catch (err) {
			return err instanceof BatchError && err.code === "invalid_output" && err.message.includes(needle);
		}
	};
	check(bad("nope", "not a JSON object"), "non-object output rejected");
	check(bad({}, "files"), "missing files key rejected");
	check(bad({ files: [{ path: "/etc/passwd", content: "x" }] }, "not repo-relative"), "absolute path rejected");
	check(bad({ files: [{ path: "../x.txt", content: "x" }] }, "escapes the repo"), ".. path rejected");
	check(bad({ files: [{ path: "a/../x.txt", content: "x" }] }, "escapes the repo"), "embedded .. rejected");
	check(bad({ files: [{ path: "C:\\x.txt", content: "x" }] }, "not repo-relative"), "windows drive rejected");
	check(bad({ files: [{ path: "a\\b.txt", content: "x" }] }, "not repo-relative"), "backslash rejected");
	check(bad({ files: [{ path: "", content: "x" }] }, "path must be a non-empty string"), "empty path rejected");
	check(bad({ files: [{ path: "a.txt", content: 7 }] }, "content must be a string"), "non-string content rejected");
	check(bad({ files: [{ path: "a.txt" }] }, "content must be a string"), "missing content rejected");
	...
	const merged = mergeBatchFiles([
		...
		{ customId: "R2", files: [{ path: "b.txt", content: "2" }, { path: "a.txt", content: "1" }] },
	]);
	check(merged.length === 2, `union dedupes identical duplicates, got ${merged.length}`);
	let conflict = false;
	try {
		mergeBatchFiles([
			{ customId: "R1", files: [{ path: "a.txt", content: "1" }] },
		...
		conflict = err instanceof BatchError && err.code === "invalid_output"
			&& err.message.includes("R1") && err.message.includes("R2");
	}
	check(conflict, "conflicting duplicate paths are a typed error (never silent last-wins)");

	console.log("✓ batch item building: R-ids, single-turn prompts, file extraction + path safety");
}
...
function testJobState(errors: string[]): void {
	...
	try {
		const state: BatchJobState = {
			...
			prompts: [{ custom_id: "R1", prompt: "p", contract: { kind: "json_object", requiredKeys: ["files"] } }],
			items: [
		...
		const path = writeBatchJobState(state, { metricsDir: dir, project: "proj" });
		check(path === batchJobStatePath(dir, "proj", "run-1"), "state path matches <metricsDir>/<project>/<run>.batch.json");
		const back = readBatchJobState(dir, "proj", "run-1");
		check(back !== null && back.job_id === "job-1" && back.status === "in_progress"
			&& back.items.length === 2 && back.prompts[0].custom_id === "R1",
			"state file round-trips (job id, prompts, per-item status)");
		check(back!.items[1].status === "invalid" && back!.items[1].error === "missing key",
			"per-item failure details survive the round-trip");

		// incompleteItems: the recoverable subset.
		check(incompleteItems(state).length === 1 && incompleteItems(state)[0].custom_id === "R2",
			"incompleteItems names the recoverable subset");
		check(incompleteItems({ ...state, items: [{ custom_id: "R1", status: "completed" }] }).length === 0,
			"incompleteItems is empty for a green job");
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
	console.log("✓ batch job state: <metricsDir>/<project>/<run>.batch.json round-trip + recoverable subset");
}
...
async function testLaneRoundTrip(errors: string[]): Promise<void> {
	...
	try {
		...
		const provider = new FakeBatchProvider({
			outputs: {
				R1: JSON.stringify({ requirement: "R1", files: [{ path: "a.txt", content: "1" }], summary: "a" }),
		...
		check(provider.submitCalls === 1 && provider.jobId === "fake-batch-1", "one submit with the fake job id");
		...
		check(lane.outputs.R1 !== undefined && (lane.outputs.R1 as { files: unknown[] }).files.length === 1,
			"validated parsed outputs keyed by customId");
		check(lane.usage.prompt_tokens === 20 && lane.usage.completion_tokens === 10 && lane.usage.cost_usd === 0.0002,
			`usage aggregates across items, got ${JSON.stringify(lane.usage)}`);
		check(events.includes("batch_submitted") && events.includes("batch_status") && events.includes("batch_collected"),
			`lane events flow (${events.join(",")})`);
		check(provider.pollCalls >= 2, `fake polls advance in_progress → completed, got ${provider.pollCalls} polls`);

		// Job state persisted: job id + prompts + per-item status.
		const state = readBatchJobState(dir, "proj", lane.runId);
		check(state !== null && state.status === "completed" && state.job_id === "fake-batch-1"
			&& state.items.length === 2 && state.items[0].status === "completed",
			"job state file records the completed job");
		check(state!.prompts.length === 2 && state!.prompts[0].custom_id === "R1"
			&& state!.prompts[0].prompt.includes("Output contract"),
			"job state records the submitted prompts + contracts");
		check(state!.usage !== undefined && state!.usage.cost_usd === 0.0002, "job state records aggregate usage");
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
...
async function testLaneFailures(errors: string[]): Promise<void> {
	...
	const run = async (provider: FakeBatchProvider, extra: Record<string, unknown> = {}): Promise<unknown> => {
		try {
	...
	try {
		...
		const invalid = await run(new FakeBatchProvider({ outputs: { R1: "not json at all" } }));
		check(invalid instanceof BatchError && invalid.code === "items_incomplete",
			"contract violation → BatchError items_incomplete");
		check((invalid as BatchError).detail?.items?.length === 1
		...
		check(invalidState !== null && invalidState.status === "failed"
		...
		check(invalidState!.job_id === "fake-batch-1", "job state keeps the job id for the failed items");

		// Provider-level item failure → status "failed".
		const failed = await run(new FakeBatchProvider({ itemErrors: { R1: "provider exploded" } }));
		check(failed instanceof BatchError && failed.code === "items_incomplete"
			&& (failed as BatchError).detail?.items?.[0].status === "failed"
			&& (failed as BatchError).detail?.items?.[0].error === "provider exploded",
		...
		check(missing instanceof BatchError && (missing as BatchError).detail?.items?.[0].status === "missing",
			"item absent from the results payload → missing status");

		// Job reached a failed terminal phase → job_failed.
		const jobFailed = await run(new FakeBatchProvider({ terminalPhase: "failed" }));
		check(jobFailed instanceof BatchError && jobFailed.code === "job_failed"
		...
		const submitFailed = await run(new FakeBatchProvider({ submitError: "provider down" }));
		check(submitFailed instanceof BatchError && submitFailed.code === "submit_failed",
		...
		check(aborted instanceof BatchError && aborted.code === "aborted"
		...
		let t = 0;
		const timedOut = await run(new FakeBatchProvider({ completeAfterPolls: 100 }), {
			jobTimeoutMs: 500,
			now: () => t,
			sleep: async () => {
				t += 1000;
		...
		check(timedOut instanceof BatchError && timedOut.code === "poll_timeout"
			&& timedOut.message.includes("fake-batch-1"),
			`poll timeout → poll_timeout naming the still-live job (${(timedOut as Error).message})`);
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
	console.log("✓ typed lane failures: submit_failed, job_failed, poll_timeout, aborted, items_incomplete");
}
...
function testOpenRouterWire(errors: string[]): void {
	...
	const provider = new OpenRouterBatchProvider({ apiKey: "test-key", fetchImpl: fetchImpl as typeof fetch });

	// Submit: POST /chat/batches with the pinned request shape.
	queue.push({ status: 200, body: JSON.stringify({ id: "batch_abc", status: "validating" }) });
	void provider
		.submit("google/gemini-3.7-flash:batch", [
			{ customId: "R1", prompt: "do it", contract: { kind: "text" } },
		])
		.then((r) => {
			check(r.jobId === "batch_abc" && r.phase === "validating", "submit maps id + phase");
			check(calls[0].url.endsWith("/chat/batches"), `submit URL, got ${calls[0].url}`);
			check(calls[0].init.method === "POST", "submit method POST");
			check(calls[0].init.headers.Authorization === "Bearer test-key", "submit carries the bearer key");
			const body = JSON.parse(calls[0].init.body ?? "{}") as {
				model: string;
				items: Array<{ custom_id: string; body: { messages: Array<{ role: string; content: string }> } }>;
			};
			check(body.model === "google/gemini-3.7-flash:batch", "submit body carries the batch model id");
			check(body.items.length === 1 && body.items[0].custom_id === "R1"
				&& body.items[0].body.messages[0].role === "user"
				&& body.items[0].body.messages[0].content === "do it",
	...
	queue.push({
		status: 200,
		body: JSON.stringify({ id: "batch_abc", status: "in_progress", request_counts: { total: 2, completed: 1, failed: 0 } }),
	});
	void provider.status("batch_abc").then((s) => {
		check(s.phase === "in_progress" && s.counts.total === 2 && s.counts.completed === 1,
			"status maps phase + request_counts");
		check(calls[1].url.endsWith("/chat/batches/batch_abc"), `status URL, got ${calls[1].url}`);
	});
	queue.push({ status: 200, body: JSON.stringify({ id: "x", status: "weird", request_counts: {} }) });
	void provider.status("x").then((s) => {
		check(s.phase === "in_progress", "unknown status maps to in_progress (forward compatible)");
	});
	...
	void provider.results("batch_abc").then((raw) => {
		check(raw.length === 2, `unparseable JSONL lines skipped, got ${raw.length}`);
		check(raw[0].customId === "R1" && raw[0].ok === true && raw[0].text === '{"files": []}'
			&& raw[0].promptTokens === 100 && raw[0].completionTokens === 50 && raw[0].costUsd === 0.001,
			"ok item maps content + usage + cost");
		check(raw[1].customId === "R2" && raw[1].ok === false && raw[1].statusCode === 422
			&& raw[1].error === "item rejected",
			"errored item maps status_code + error");
		// calls: 0 submit · 1 status(batch_abc) · 2 status(x) · 3 results
		check(calls[3].url.endsWith("/chat/batches/batch_abc/results"), `results URL, got ${calls[3].url}`);
	});
	...
	void provider.status("batch_abc").catch((err: unknown) => {
		check(err instanceof BatchError && err.code === "http_error" && err.message.includes("401"),
	...
	const noKey = new OpenRouterBatchProvider({ fetchImpl: fetchImpl as typeof fetch });
	void noKey.submit("m", [{ customId: "R1", prompt: "p", contract: { kind: "text" } }]).catch((err: unknown) => {
		check(err instanceof BatchError && err.code === "no_api_key", "missing api key → no_api_key");
	});
	if (savedKey !== undefined) process.env.OPENROUTER_API_KEY = savedKey;

	// The queue is drained by the async handlers above — flush microtasks.
	void Promise.resolve().then(() => {
		if (queue.length > 0) errors.push(`unconsumed mock responses: ${queue.length}`);
	});
	console.log("✓ OpenRouter wire: submit/status/results request shape + JSONL mapping (mock fetch)");
}
...
function testRouteRun(errors: string[]): void {
	...
	check(routeRun(undefined).kind === "interactive", "undefined shape → interactive");
	check(routeRun(DEFAULT_TASK_SHAPES.code).kind === "interactive", "code shape → interactive");
	check(routeRun({ ...DEFAULT_TASK_SHAPES.code, channel: "flex" }).kind === "interactive",
	...
	check(p.kind === "invalid" && p.reason.includes("parallel"), "batch + parallel → invalid (reason names parallel)");
	const s = routeRun(BATCH_SHAPE, { hasSubSpecs: true });
	check(s.kind === "invalid" && s.reason.includes("sub_specs"), "batch + sub_specs → invalid (reason names sub_specs)");
	check(routeRun(BATCH_SHAPE, { parallel: 1, hasSubSpecs: false }).kind === "batch",
		"batch + parallel 1 + no sub_specs → batch");

	console.log("✓ routeRun: batch channel routes to the lane; parallel/sub_specs are configuration errors");
}
...
async function testExecuteBatchLane(errors: string[]): Promise<void> {
	...
	try {
		...
		const provider = new FakeBatchProvider({
			outputs: {
				...
				R2: JSON.stringify({ requirement: "R2", files: [], summary: "commit via lane" }),
			},
		});
		const events: string[] = [];
		const result: TaskResult = await executeTask({
			...
			batch: { model: "google/gemini-3.7-flash:batch", pollIntervalMs: 1, jobTimeoutMs: 60_000 },
			metricsDir,
			project: "batch-test",
		...
		check(result.success === true && result.tests === "passing", "green batch run succeeds");
		check(result.files_changed.length === 1 && result.files_changed[0] === "hello.txt",
			`files_changed carries the applied file, got ${JSON.stringify(result.files_changed)}`);
		check(existsSync(join(dir, "hello.txt")) && readFileSync(join(dir, "hello.txt"), "utf-8") === "hi",
			"the batch output was applied to the working copy");
		check(result.commits.length === 1 && /^[0-9a-f]{40}$/.test(result.commits[0]),
			`the batch commit id is returned, got ${JSON.stringify(result.commits)}`);
		check(result.batch?.jobId === "fake-batch-1" && result.batch?.items === 2,
			"result carries the batch record (job id + item count)");
		check(result.worker.usage.turns === 2 && result.worker.usage.cost_usd === 0.0002
		...
		check(jj(["log", "-r", "@-", "-T", "description", "--no-graph"], dir).trim().startsWith("batch(task):"),
			"the batch commit carries the lane message");
		check(jj(["log", "-r", "@-", "-T", "author.name()", "--no-graph"], dir).trim() === "Pi (exec)",
			`the batch commit is AI-authored, got ${jj(["log", "-r", "@-", "-T", "author.name()", "--no-graph"], dir).trim()}`);
		check(jj(["diff", "--from", "@-", "--to", "@", "--summary"], dir).trim() === "",
		...
		check(result.manifest !== undefined && result.manifest.phases.verify.source === "batch"
			&& result.manifest.config.channel === "batch" && result.manifest.config.shape === "batch",
			"manifest records verify.source batch + channel batch");
		check(result.manifest!.phases.execute.model === "google/gemini-3.7-flash:batch"
			&& result.manifest!.phases.execute.turns === 2 && result.manifest!.phases.execute.cost_usd === 0.0002,
			"manifest execute phase aggregates the lane usage");
		check(result.manifestPath !== undefined && existsSync(result.manifestPath!), "manifest persisted");
		const state = readBatchJobState(metricsDir, "batch-test", result.manifest!.run_id);
		check(state !== null && state.job_id === "fake-batch-1" && state.status === "completed"
			&& state.items.every((i) => i.status === "completed"),
			"job state file persisted next to the manifest (job id + per-item status)");
		check(existsSync(batchJobStatePath(metricsDir, "batch-test", result.manifest!.run_id)),
		...
		try {
			initRepo(failDir);
			const badProvider = new FakeBatchProvider({ outputs: { R1: "not json" } });
			...
			check(caught instanceof BatchError && caught.code === "items_incomplete",
				"contract violation surfaces as typed items_incomplete");
			check(jj(["log", "-r", "@-", "-T", "description", "--no-graph"], failDir).trim() === "init",
				"failed batch run restores the working copy (no dangling commits)");
			check(jj(["diff", "--from", "@-", "--to", "@", "--summary"], failDir).trim() === "",
			...
			const failures = readdirSync(projDir).filter((n) => n.endsWith(".failure.json")).sort();
			check(failures.length >= 1, "failure artifact written for the failed lane run");
			const artifact = JSON.parse(
				readFileSync(join(projDir, failures[failures.length - 1]), "utf-8"),
			) as { kind: string; recovery?: string; cause: string };
			check(artifact.kind === "batch" && artifact.recovery !== undefined
		...
		for (const opts of [
			...
			check(caught instanceof Error && caught.message.includes("batch channel"),
	...
	console.log("✓ executeTask batch channel: real jj repo — apply → commit (AI) → verify → manifest + state, typed failures");
}

export async function runTests(): Promise<void> {
	const errors: string[] = [];
	console.log("── test-batch: batch lane — contracts, fake round-trip, OpenRouter wire, routing ──");
