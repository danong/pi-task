...
import { pruneReviewContext, DEFAULT_DROP_TOOLS, type ReviewMessage } from "./prune.ts";
...
export async function runTests(): Promise<void> {
	...
	const assistant = (...content: unknown[]): ReviewMessage => ({ role: "assistant", content });
	const text = (t: string) => ({ type: "text", text: t });
	const thinking = (t: string) => ({ type: "thinking", thinking: t });
	const call = (name: string, id = "c1") => ({ type: "toolCall", id, name, arguments: {} });
	const custom = (): ReviewMessage => ({ role: "custom", customType: "pi-task-checklist", data: {} });

	// 1. Empty in → empty out
	check(pruneReviewContext([]).length === 0, "empty input should prune to empty");

	// 2. User messages (the task) are kept verbatim
	{
		const out = pruneReviewContext([user("build the thing")]);
		check(out.length === 1 && out[0].role === "user" && out[0].content === "build the thing",
	...
	// 3. Factual tool results kept; biased/commitment tool results dropped
	{
		...
		check(JSON.stringify(names) === JSON.stringify(["read", "bash", "grep"]),
	...
	{
		const out = pruneReviewContext([assistant(text("I'll approach this by..."), thinking("let me plan"), call("read", "r1"))]);
		check(out.length === 1, "assistant message with a kept tool call should survive");
		const blocks = out[0].content as any[];
		check(blocks.length === 1 && blocks[0].type === "toolCall" && blocks[0].name === "read",
	...
	// 5. Assistant message that is ONLY reasoning is dropped entirely
	{
		...
		check(out.length === 0, "pure-reasoning assistant message should be dropped");
	}

	// 6. Biased tool calls dropped from assistant messages; empty → dropped
	{
		const out = pruneReviewContext([assistant(call("edit"), call("checklist"))]);
		check(out.length === 0, "assistant message with only biased tool calls should be dropped");
	}

	// 7. Custom extension state entries dropped
	{
		const out = pruneReviewContext([custom(), user("task")]);
		check(out.length === 1 && out[0].role === "user", "custom entries should be dropped, user kept");
	}
	...
	// 8. Realistic mixed conversation → expected pruned shape
	{
		...
		const shape = out.map((m) =>
			m.role === "toolResult"
				...
				: m.role === "assistant"
					? `assistant[${(m.content as any[]).map((b) => b.name ?? b.type).join(",")}]`
					: m.role,
		);
		check(JSON.stringify(shape) === JSON.stringify(["user", "assistant[read]", "toolResult:read"]),
			`pruned shape wrong, got ${JSON.stringify(shape)}`);
	}

	// 9. dropTools override changes what is pruned
	{
		const out = pruneReviewContext([toolResult("edit", "x"), toolResult("read", "y")], { dropTools: ["read"] });
		const names = out.map((m) => m.toolName);
		check(JSON.stringify(names) === JSON.stringify(["edit"]),
	...
	// 10. Malformed assistant content (non-array) → dropped, no throw
	{
		...
		check(out.length === 0, "assistant with non-array content should be dropped without throwing");
	}

	// 11. DEFAULT_DROP_TOOLS is the documented set
	check(JSON.stringify([...DEFAULT_DROP_TOOLS].sort()) === JSON.stringify(["checklist", "edit", "write", "yield"]),
	...
	// 12. Input array is not mutated (pure)
	{
		...
		check(JSON.stringify(input) === before, "pruneReviewContext must not mutate its input");
	}

	if (errors.length > 0) {
		throw new Error("test-prune failed:\n  ✗ " + errors.join("\n  ✗ "));
	}
	console.log("✓ pruning: keeps task+reads/bash, drops reasoning/edits/checklist/custom, pure");
}
